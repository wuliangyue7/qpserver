...
bool CTableFrameSink::OnEventGameStart()
{
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    {
      GameStart.cbCardData[i]=0;
    }
    else
    {
      GameStart.cbCardData[i]=m_cbHandCardData[i][1];
    }
  }
  ...
  if(wPlayerCount>=2)
  {
    ...
    if(m_wCurrentUser==wChairID)
    {
      ...
      for(WORD i=1; i<GAME_PLAYER; i++)
      {
        ...
        //状态判断
        if(m_cbPlayStatus[wCurrentUser]==FALSE)
        {
          continue;
        }
...
VOID CTableFrameSink::ControlUserCard()
{
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    {
      if((g_lMaxWinScore!=0L)&&(pUserTracing->lWinScore>g_lMaxWinScore))
      {
        bSwitchCard=true;
      }
      if((g_lMaxVarietycore!=0L)&&(pUserTracing->lVarietyScore>g_lMaxVarietycore))
      {
        bSwitchCard=true;
      }
    }
    ...
    {
      DWORD dwUserID=0L;
      if((g_BlackListIDMap.Lookup(pIServerUserItem->GetUserID(),dwUserID)==TRUE)&&(dwUserID!=0L))
      {
        bSwitchCard=true;
      }
    }
    ...
    if(bSwitchCard==false)
    {
      ...
      if((g_UserAddressMap.Lookup(pIServerUserItem->GetClientAddr(),dwUserAddress)==TRUE)&&(dwUserAddress!=0))
      {
        bSwitchCard=true;
      }
    }
...
VOID CTableFrameSink::OnEventScoreAlter(DWORD dwUserID, LONGLONG lVarietyScore)
{
  ...
  if(lVarietyScore>0L)
  {
    pUserTracing->lWinScore+=lVarietyScore;
  }
