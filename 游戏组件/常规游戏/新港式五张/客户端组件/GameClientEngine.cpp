...
bool CGameClientEngine::OnSubGameStart(VOID * pData, WORD wDataSize)
{
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    {
      //发牌用户
      if(wStartChairID==INVALID_CHAIR)
      {
        wStartChairID=wViewChairID;
      }
  ...
  //if ((IsLookonMode()==true)||(wViewChairID!=MYSELF_VIEW_ID))
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if(m_bXiaZhu)
        {
          if(pIClientUserItem->GetGender()==GENDER_FEMALE)
          {
            PlayGameSound(AfxGetInstanceHandle(),TEXT("XIAZHU_1"));
          }
          else
          {
            PlayGameSound(AfxGetInstanceHandle(),TEXT("XIAZHU_0"));
          }
        }
        else
        {
          if(pIClientUserItem->GetGender()==GENDER_FEMALE)
          {
            PlayGameSound(AfxGetInstanceHandle(),TEXT("ADD_SCORE_1"));
          }
          else
          {
            PlayGameSound(AfxGetInstanceHandle(),TEXT("ADD_SCORE_0"));
          }
        }
...
bool CGameClientEngine::PerformAutoStart()
{
  ...
  //状态判断
  if((IsLookonMode()==true)||(m_pIClientKernel->GetClockID()==IDI_START_GAME))
  {
    return false;
  }
...
VOID CGameClientEngine::PerformGameConclude()
{
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    {
      //过虑判断
      if(m_GameEndPacket.cbCardData[i]==0x00)
      {
        continue;
      }
