...
//  if(num == 0)
//  {
//    fp=fopen("虎虎生威机器人.log","w");
//  }
//  else
//  {
//    fp=fopen("虎虎生威机器人.log","a");
//  }
//  if(fp == NULL)
//    return ;
//  num++;
...
bool  CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
  switch(nTimerID)
  {
    case IDI_CHECK_BANKER:    //检查上庄
    {
      ...
      {
        CString cs;
        cs.Format(TEXT("异常ID：%d"),IDI_CHECK_BANKER);
    ...
    case IDI_REQUEST_BANKER:  //申请上庄
    {
      ...
      {
        CString cs;
        cs.Format(TEXT("异常ID：%d"),IDI_REQUEST_BANKER);
        CTraceService::TraceString(cs,TraceLevel_Exception);
      }
    ...
    case IDI_GIVEUP_BANKER:   //申请下庄
    {
      ...
      {
        CString cs;
        cs.Format(TEXT("异常ID：%d"),IDI_GIVEUP_BANKER);
        CTraceService::TraceString(cs,TraceLevel_Exception);
      }
    ...
    case IDI_BANK_OPERATE:    //银行操作
    {
      ...
      {
        CString cs;
        cs.Format(TEXT("异常ID：%d"),IDI_BANK_OPERATE);
        CTraceService::TraceString(cs,TraceLevel_Exception);
      }
    }
    default:
...
bool  CAndroidUserItemSink::OnEventGameMessage(WORD wSubCmdID, VOID * pBuffer, WORD wDataSize)
{
  ...
  {
    CString cs;
    cs.Format(TEXT("游戏消息ID：%d"),wSubCmdID);
    CTraceService::TraceString(cs,TraceLevel_Exception);
  }
...
void CAndroidUserItemSink::ReadConfigInformation(TCHAR szFileName[], TCHAR szRoomName[], bool bReadFresh)
{
  ...
  //列表人数
  m_nMinRobotBankerCount = GetPrivateProfileInt(szRoomName, TEXT("RobotListMinCount"), 3, szConfigFileName);
