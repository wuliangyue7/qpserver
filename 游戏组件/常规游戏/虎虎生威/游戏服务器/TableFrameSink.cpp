...
void Debug(char *text,...)
{
  ...
  if(num == 0)
  {
    fp=fopen("虎虎生威库存.log","w");
  }
  else
  {
    fp=fopen("虎虎生威库存.log","a");
  }
  if(fp == NULL)
  {
    return ;
...
void MakeStirngToStirng(char *buf,int len,char *outbuf,int &Outlen)
{
  ...
  while(1)
  {
    iPos = Findstr(buf,_TEXT(','),(INT)strlen(buf));
    if(iPos==-1)
    {
      char tmp[256]= {0};
      char *strtmp = buf;
...
CTableFrameSink::CTableFrameSink()
{
  ...
  m_lMaxPercent=0l;
  m_DispatchGold=0l;
  m_lStorageMax[0] = m_lStorageMax[1] = 0;
  m_wStorageMul[0] = m_wStorageMul[1] = 0;
  ...
  switch(cbReason)
  {
    case GER_NORMAL:    //常规结束
    {
      ...
      if(m_pServerControl!=NULL&&m_pServerControl->ExcuteControl(&cbCard))
      {
        ...
        m_StorageStart = m_StorageStart - (m_StorageStart * ((NeedDeductStorage()) ? m_StorageDeduct : 0) / 1000);
      ...
      else
      {
        ...
        {
          bSystemLost = (m_StorageStart>=m_lStorageMax[1])?(rand()%100 < m_wStorageMul[1]):(rand()%100 < m_wStorageMul[0]);
        }
      ...
      for(WORD wUserIndex = 0; wUserIndex < GAME_PLAYER; ++wUserIndex)
      {
        ...
        //记录玩家总输赢
        m_lUserTotalScore[wUserIndex]+=m_lUserWinScore[wUserIndex];


        //玩家总分
        LONGLONG lUserBetScore = 0;
        for(BYTE j=0; j<AREA_COUNT+1; j++)
        {
          lUserBetScore+=m_lUserJettonScore[j][wUserIndex];
        }
        if(lUserBetScore!=0)
        {
          GameEnd.lServiceScore = m_lServiceScore;
        }
        else
        {
          GameEnd.lServiceScore = 0;
        }
        //设置成绩
        GameEnd.lUserScore=m_lUserWinScore[wUserIndex]-GameEnd.lServiceScore;
  ...
  switch(cbGameStatus)
  {
    case GAME_STATUS_FREE:      //空闲状态
    {
      ...
      if(CUserRight::IsGameCheatUser(pIServerUserItem->GetUserRight()))
      {
        ...
        CopyMemory(StorageInfo.wStorageMul,m_wStorageMul,sizeof(m_wStorageMul));
        CopyMemory(StorageInfo.lUserTotalScore,m_lUserTotalScore,sizeof(m_lUserTotalScore));
        m_pITableFrame->SendTableData(wChiarID,SUB_S_ADMIN_STORAGE_INFO,&StorageInfo,sizeof(StorageInfo));
        m_pITableFrame->SendLookonData(wChiarID,SUB_S_ADMIN_STORAGE_INFO,&StorageInfo,sizeof(StorageInfo));
      }
      return bSuccess;
    }
    ...
    case GS_GAME_END:     //结束状态
    {
      ...
      StatusPlay.cbGameStatus=pIServerUserItem->GetUserStatus()==US_PLAYING?m_pITableFrame->GetGameStatus():GS_GAME_END;
...
bool  CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
  ...
  switch(wSubCmdID)
  {
    ...
    case SUB_C_ADMIN_COMMDN:
    {
      ...
      CString strBuf;
      if(pData->cbControlAppType == C_CA_UPDATE)
      {
        strBuf.Format(TEXT("重新设置下注区域:%d,设置控制次数:%d\n"), pData->cbControlArea,pData->cbControlTimes);
        AdminOperateInfo(strBuf,pIServerUserItem);
      }
      else if(pData->cbControlAppType == C_CA_SET)
      {
    ...
    case SUB_C_STORAGE:
    {
      ...
      m_StorageDeduct=FreshStorage->lStorageDeuct;
    ...
    case SUB_C_STORAGEINFO:
    {
      ...
      if(wDataSize!=sizeof(CMD_C_StorageInfo))
      {
        return false;
      }
      ...
      CMD_C_StorageInfo *pStorage = (CMD_C_StorageInfo *)pDataBuffer;
      CopyMemory(m_lStorageMax,pStorage->lMaxStorage,sizeof(m_lStorageMax));
      CopyMemory(m_wStorageMul,pStorage->wStorageMul,sizeof(m_wStorageMul));
      return true;
    }
  }
...
bool CTableFrameSink::ProbeJudge(bool& bSystemLost)
{
  ...
  // 11/11/2013 [Wan.MY]
  // 这里添加送分判断
  if(m_StorageStart > m_lStorageMax[0] && bSystemLost && lSystemScore != 0)
  {
    // 如果系统还是赢钱，返回生成下一个
    if(lSystemScore > 0)
    {
    ...
    // 够输，OK
    if(m_StorageStart + lSystemScore >= 0)
    {
      m_StorageStart += lSystemScore;
      m_StorageStart = m_StorageStart - (m_StorageStart * ((NeedDeductStorage()) ? m_StorageDeduct : 0) / 1000);
      return true;
  ...
  //如果是玩家做庄，控制其输的下限不超过身上金币的百分比
  if(!bIsBankerAndroidUser && m_wBankerUser != INVALID_CHAIR)
  {
    //玩家输赢
    LONGLONG lUserScore = 0l;
    IServerUserItem * pIBankerUserItem = m_pITableFrame->GetTableUserItem(m_wBankerUser);
    SCORE lBankerScore = pIBankerUserItem->GetUserScore();
    ...
    for(WORD wChairID = 0; wChairID < GAME_PLAYER; wChairID++)
    {
      ...
      for(WORD wArea = 1; wArea <= AREA_COUNT; ++wArea)
      {
        if(bcResulteOut[wArea-1]>0)
        {
          lUserScore -= (pUserScore[wArea][wChairID] *(bcResulteOut[wArea-1]));
        }
        else
        {
          if(bcResulteOut[wArea-1]==0)
          {
            lUserScore += pUserScore[wArea][wChairID];
          }
        }
      }
    }
    ...
    if(lSystemScore == 0)
    {
      if(lBankerScore >= m_lBankerGold || lUserScore > 0)
      {
        return true;
      }
      else
      {
        SCORE lLeftGold = m_lBankerGold - m_lBankerGold * m_lUserBankerPercent / 100;
        if(lBankerScore >= lLeftGold)
        {
          LONGLONG lLoseGold = lBankerScore - lLeftGold;
          if(lUserScore + lLoseGold >= 0)
          {
            return true;
          }
          else
    ...
    {
      SCORE lLeftGold = m_lBankerGold - m_lBankerGold * m_lUserBankerPercent / 100;
      SCORE lLeftScore = m_StartScore - m_StartScore * m_DispatchGold / 100;
      if(lBankerScore - lLeftGold < 0)
      {
        if(lSystemScore < 0)
        {
          m_StorageStart += lSystemScore;
      ...
      {
        if(m_StorageStart >= lLeftScore)
        {
          LONGLONG lLoseGold = m_StorageStart - lLeftScore;
          if(lSystemScore + lLoseGold >= 0)
          {
            m_StorageStart += lSystemScore;
            m_StorageStart = m_StorageStart - (m_StorageStart * ((NeedDeductStorage()) ? m_StorageDeduct : 0) / 1000);
            return true;
...
void CTableFrameSink::WriteInfo(LPCTSTR pszString)
{
  ...
  CStdioFile myFile;
  CString strFileName;
  strFileName.Format(TEXT("虎虎生威.txt"));
...
//  m_cbCompareTime=pCustomConfig->CustomGeneral.cbGuessTime;
  ...
  if(m_StorageDeduct<0)
  {
    m_StorageDeduct=0;
  }
  if(m_StorageDeduct>1000)
  {
    m_StorageDeduct=1000;
  }
  ...
  if(m_wStorageMul[0]<0 || m_wStorageMul[0]>100)
  {
    m_wStorageMul[0]=80;
  }
  m_wStorageMul[1]=pCustomConfig->CustomGeneral.StorageMul2;
  if(m_wStorageMul[1]<0 || m_wStorageMul[1]>100)
  {
    m_wStorageMul[1]=80;
  }

  m_StorageDispatch = 20;
  m_lServiceScore = m_pGameServiceOption->lServiceScore;
...
void CTableFrameSink::AdminOperateInfo(LPCTSTR pszString,IServerUserItem *pIServerUserItem)
{
  ...
  CString strFileName;
  strFileName.Format(TEXT("虎虎生威操作日志.txt"));
