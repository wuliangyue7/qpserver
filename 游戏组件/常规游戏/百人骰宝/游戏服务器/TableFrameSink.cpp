  ...
  switch(wSubCmdID)
  {
    ...
    case SUB_C_STORAGE:
    {
      ...
      for(WORD wUserID = 0; wUserID < GAME_PLAYER; wUserID++)
      {
        ...
        if(RQ_REFRESH_STORAGE == FreshStorage->cbReqType && pIServerUserItem->GetChairID() != wUserID)
        {
          continue;
        }
    ...
    case SUB_C_CONTROL_WIN:
    {
      ...
      if(wDataSize!=sizeof(CMD_C_ControlWinLose))
      {
        return false;
      }
...
bool CTableFrameSink::OnUserPlaceJetton(WORD wChairID, BYTE cbJettonArea, SCORE lJettonScore)
{
  ...
  for(int nAreaIndex=0; nAreaIndex<AREA_COUNT; ++nAreaIndex)
  {
    lAllJettonCount += m_lAllUserBet[nAreaIndex];
  }
...
SCORE CTableFrameSink::CalculateScore(bool bProbeJudgeScore)
{
  ...
  for(WORD wChairID=0; wChairID<GAME_PLAYER; wChairID++)
  {
    ...
    for(BYTE cbAreaIndex =0; cbAreaIndex<AREA_COUNT; cbAreaIndex++)
    {
      ...
      if(m_lUserAllBet[cbAreaIndex][wChairID]<=0)
      {
        continue;
      }
      if(nWinMultiple[cbAreaIndex]>0)
...
void CTableFrameSink::GetStockScore(SCORE *lAndroidScore,SCORE *lUserScore,BYTE bDice[])
{
  ...
  for(BYTE i=0; i<GAME_PLAYER; i++)
  {
    ...
    if(pIServerUser == NULL || i == m_wCurrentBanker)
    {
      continue;
    }
...
BYTE CTableFrameSink::ControlUserWin()
{
  ...
  for(WORD wChairID=0; wChairID<GAME_PLAYER; wChairID++)
  {
    ...
    if(m_lQueryGameID!=pIServerUserItem->GetGameID())
    {
      continue;
    }
...
void CTableFrameSink::ReadConfigInformation()
{
  ...
  if(m_dwTimeGameFree < TIME_FREE || m_dwTimeGameFree > 99)
  {
    m_dwTimeGameFree = TIME_FREE;
  }
  if(m_dwTimeGameBet < TIME_PLACE_JETTON || m_dwTimeGameBet > 99)
  {
    m_dwTimeGameBet = TIME_PLACE_JETTON;
  }
  if(m_dwTimeGameEnd < TIME_GAME_END || m_dwTimeGameEnd > 99)
  {
    m_dwTimeGameEnd = TIME_GAME_END;
  }
