  ...
  }
  {
    ...
    switch(m_cbGameStatus)
    {
      case GAME_STATUS_FREE:
        m_PngFlagsTip.DrawImage(pDCBuff,nWidth/2+140, 107, nTipWidth, nTipHeight, 0,0);
        break;
      case GS_PLAYER_BET:
        m_PngFlagsTip.DrawImage(pDCBuff,nWidth/2+140, 107, nTipWidth, nTipHeight, nTipWidth,0);
        break;
      case GS_GAME_END:
        m_PngFlagsTip.DrawImage(pDCBuff,nWidth/2+140, 107, nTipWidth, nTipHeight, nTipWidth*2,0);
        break;
      default:
        break;
    }
...
void CGameClientView::DrawGameRecord(CDC*pDC)
{
  ...
  CRect rcRecord(m_ptRecordPosEx.x,m_ptRecordPosEx.y-12,m_ptRecordPosEx.x+150,m_ptRecordPosEx.y);
...
BYTE CGameClientView::GetFramePngIndex(BYTE cbAreaIndex)
{
  ...
  if(cbAreaIndex>=2&&cbAreaIndex<=7)
  {
    return 2;
  }
  if(cbAreaIndex>=8&&cbAreaIndex<=13)
  {
    return 3;
  }
  if(cbAreaIndex>=15&&cbAreaIndex<=28)
  {
    return 5;
  }
  if(cbAreaIndex>=29&&cbAreaIndex<=43)
  {
    return 6;
  }
  if(cbAreaIndex>44&&cbAreaIndex<49)
  {
    return 8;
  }
  return 0xFF;
...
void CGameClientView::OnTimeGameEnd()
{
  ...
  {
    m_pGameClientEngine->PlayGameSound(AfxGetInstanceHandle(),TEXT("END_WIN"));
  }
  else if(m_lMeCurGameScore<0)
  {
    m_pGameClientEngine->PlayGameSound(AfxGetInstanceHandle(),TEXT("END_LOST"));
  }
  else
  {
    m_pGameClientEngine->PlayGameSound(AfxGetInstanceHandle(),TEXT("END_DRAW"));
  }
