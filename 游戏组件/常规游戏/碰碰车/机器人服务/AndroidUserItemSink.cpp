...
bool  CAndroidUserItemSink::OnEventTimer(UINT nTimerID)
{
  switch(nTimerID)
  {
    case IDI_CHECK_BANKER:    //检查上庄
    {
      ...
      if(m_wCurrentBanker == INVALID_CHAIR||m_nListUserCount<m_nMinRobotBankerCount)
      {
        //空庄
        m_nWaitBanker++;
    ...
    case IDI_BANK_OPERATE:    //银行操作
    {
      ...
      LONGLONG lRobotScore = pUserItem->GetUserScore();
      {
        ...
        {
          SCORE lScore=rand()%(m_lRobotBankGetScoreBanker - m_lRobotBankGetScore)+ m_lRobotBankGetScore;
          if(lScore > 0)
          {
...
bool  CAndroidUserItemSink::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonOther, VOID * pData, WORD wDataSize)
{
  switch(cbGameStatus)
  {
    case GAME_STATUS_FREE:      //空闲状态
    {
      ...
      CopyMemory(&m_CustomAndroidConfig,&(pStatusFree->CustomAndroidConfig),sizeof(m_CustomAndroidConfig));
      ReadConfigInformation(m_CustomAndroidConfig);
      ...
      {
        if(m_bRobotBanker   && m_stlApplyBanker < m_nRobotApplyBanker)
        {
          //合法判断
          IServerUserItem *pIUserItemBanker = m_pIAndroidUserItem->GetMeUserItem();
          if(pIUserItemBanker->GetUserScore() > m_lBankerCondition)
    ...
    case GS_GAME_END:   //结束状态
    {
      ...
      CopyMemory(&m_CustomAndroidConfig,&(pStatusPlay->CustomAndroidConfig),sizeof(m_CustomAndroidConfig));
      ReadConfigInformation(m_CustomAndroidConfig);
...
//读取配置
VOID CAndroidUserItemSink::ReadConfigInformation(tagCustomAndroid& customAndroidConfig)
{
  //每盘刷新
  m_bRefreshCfg = TRUE;
  //筹码限制
  m_lRobotJettonLimit[0] = customAndroidConfig.lRobotMinJetton;
  m_lRobotJettonLimit[1] = customAndroidConfig.lRobotMaxJetton;
  ...
  //次数限制
  m_nRobotBetTimeLimit[0] = customAndroidConfig.lRobotMinBetTime;
  m_nRobotBetTimeLimit[1] = customAndroidConfig.lRobotMaxBetTime;
  ...
  //是否坐庄
  m_bRobotBanker = customAndroidConfig.nEnableRobotBanker;

  //坐庄次数
  m_nRobotBankerCount = customAndroidConfig.lRobotBankerCount;

  //空盘重申
  m_nRobotWaitBanker = customAndroidConfig.lRobotWaitBanker;

  //最多个数
  m_nRobotApplyBanker = customAndroidConfig.lRobotApplyBanker;

  //降低限制
  m_bReduceJettonLimit = 0;

  //列表人数
  m_nMinRobotBankerCount = customAndroidConfig.lRobotListMinCount;

  //分数限制
  m_lRobotScoreRange[0] = customAndroidConfig.lRobotGetCondition;
  m_lRobotScoreRange[1] = customAndroidConfig.lRobotSaveCondition;
  ...
  //提款数额
  m_lRobotBankGetScore = customAndroidConfig.lRobotGetMin;

  //提款数额 (庄家)
  m_lRobotBankGetScoreBanker = customAndroidConfig.lRobotGetMax;

  //存款倍数
  m_nRobotBankStorageMul = customAndroidConfig.lRobotBankStoMul;
