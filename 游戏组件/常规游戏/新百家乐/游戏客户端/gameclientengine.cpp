...
bool CGameClientEngine::OnEventGameMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    ...
    case SUB_S_USER_SCORE_NOTIFY: //金币变化
    {
      ...
      if(wDataSize!=sizeof(CMD_S_UserScoreNotify))
      {
        return false;
      }
...
bool CGameClientEngine::OnSubPlaceBet(const void * pBuffer, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_S_PlaceBet))
  {
    return false;
  }
...
void CGameClientEngine::UpdateButtonContron()
{
  ...
  if(bEnablePlaceBet)
  {
    ...
    if(lCurrentBet>lLeaveScore)
    {
      ...
      {
        m_GameClientView.SetCurrentBet(10000000L);
      }
      else if(lLeaveScore>=5000000L)
      {
        m_GameClientView.SetCurrentBet(5000000L);
      }
      else if(lLeaveScore>=1000000L)
      {
        m_GameClientView.SetCurrentBet(1000000L);
      }
      else if(lLeaveScore>=100000L)
      {
        m_GameClientView.SetCurrentBet(100000L);
      }
      else if(lLeaveScore>=10000L)
      {
        m_GameClientView.SetCurrentBet(10000L);
      }
      else if(lLeaveScore>=1000L)
      {
        m_GameClientView.SetCurrentBet(1000L);
      }
      else
      {
        m_GameClientView.SetCurrentBet(0L);
  ...
  if(!IsLookonMode())
  {
    ...
    if(m_wBankerUser == GetMeChairID() && GetGameStatus() != GAME_SCENE_FREE)
    {
      bEnableCancel=false;
    }
...
LRESULT CGameClientEngine::OnApplyBanker(WPARAM wParam, LPARAM lParam)
{
  //合法判断
  IClientUserItem* pMeUserItem = GetMeUserItem();
  if(pMeUserItem->GetUserScore() < m_lApplyBankerCondition)
  {
    return true;
  }
  if(m_lPlayFreeSocre < m_lApplyBankerCondition)
  {
    return true;
  }
  ...
  //当前判断
  if(m_wBankerUser == GetMeChairID() && bApplyBanker)
  {
    return true;
  }
