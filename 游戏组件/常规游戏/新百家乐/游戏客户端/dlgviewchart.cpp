...
void CDlgViewChart::DrawChart(CDC *pDC)
{
  ...
  while(nIndex != m_nRecordLast && (m_nRecordLast != m_nRecordFirst))
  {
    ...
    {
      m_PngTwopairFlag.DrawImage(pDC,nPosX+nHorCount*nGridWidth+2,nPosY+nVerCount*nGridHeight+2,nWidth,nHeight,0,0);
    }
    if(ClientGameRecord.bPlayerTwoPair)
    {
      m_PngTwopairFlag.DrawImage(pDC,nPosX+(nHorCount+1)*nGridWidth-3-nWidth,nPosY+(nVerCount+1)*nGridHeight-3-nHeight,nWidth,nHeight,nWidth,0);
    }
...
void CDlgViewChart::SetGameRecord(const tagClientGameRecord &ClientGameRecord)
{
  ...
  m_GameRecordCount++;
  if(m_TraceGameRecordCount>=100)
  {
    m_TraceGameRecordCount=0;
  }
  if(m_GameRecordCount>=80)
  {
    m_GameRecordCount=0;
  }
