...
bool CGameClientEngine::OnSubGameStart(VOID * pData, WORD wDataSize)
{
  ...
  //环境设置
  if((IsLookonMode()==false)&&(m_bTrustee==false))
  {
    ActiveGameFrame();
  }
...
bool CGameClientEngine::OnSubGameConclude(VOID * pData, WORD wDataSize)
{
  ...
  if(lMeScore<0L)
  {
    PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_LOSE"));
  }
  if(lMeScore==0L)
  {
    PlayGameSound(AfxGetInstanceHandle(),TEXT("GAME_CONCLUDE"));
  }
...
bool CGameClientEngine::VerdictOutCard()
{
  ...
  if(GetGameStatus()!=GAME_SCENE_PLAY)
  {
    return false;
  }
  ...
  if(cbShootCount>0L)
  {
    ...
    if(m_GameLogic.GetCardType(cbCardData,cbShootCount)==CT_ERROR)
    {
      return false;
    }
...
bool CGameClientEngine::PerformAutoStart()
{
  ...
  //状态判断
  if((pIMySelfUserItem==NULL)||(pIMySelfUserItem->GetUserStatus()==US_READY))
  {
    return false;
  }
  if((IsLookonMode()==true)||(m_pIClientKernel->GetClockChairID()==IDI_START_GAME))
  {
    return false;
  }
...
VOID CGameClientEngine::PlayOutCardSound(WORD wChairID, BYTE cbCardData[], BYTE cbCardCount)
{
  ...
  if((wChairID>=GAME_PLAYER)||(cbCardCount==0))
  {
    return;
  }
...
VOID CGameClientEngine::OnTimer(UINT nIDEvent)
{
  switch(nIDEvent)
  {
    case IDI_MOST_CARD:     //最大扑克
    {
      ...
      if(m_wMostCardUser==INVALID_CHAIR)
      {
        return;
      }
...
{
  //状态判断
  if(m_GameClientView.m_btOutCard.IsWindowEnable()==false)
  {
    return 0;
  }
...
{
  //状态判断
  if(m_GameClientView.m_btPassCard.IsWindowEnable()==false)
  {
    return 0;
  }
...
LRESULT CGameClientEngine::OnMessageDispatchFinish(WPARAM wParam, LPARAM lParam)
{
  ...
  if(GetGameStatus()!=GAME_SCENE_CALL)
  {
    return 0L;
  }
