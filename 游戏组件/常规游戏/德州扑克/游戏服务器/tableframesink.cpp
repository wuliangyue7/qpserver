...
#define IDI_DELAY_TIME        5000      //延时时间
//////////////////////////////////////////////////////////////////////////
...
CTableFrameSink::CTableFrameSink()
{
  ...
  m_hControlInst = LoadLibrary(TEXT("DZShowHandServerControl.dll"));
  if(m_hControlInst)
  {
    typedef void * (*CREATE)();
...
{
  if(bLookonUser || pIServerUserItem->GetChairID()==INVALID_CHAIR)
  {
    return true;
  }
  for(WORD i=0; i<m_wPlayerCount; i++)
  {
    if(i==pIServerUserItem->GetChairID())
    {
      continue;
    }
...
bool CTableFrameSink::OnEventGameStart()
{
  ...
  if(m_pServerContro)
  {
    ...
    //信息结算
    m_pServerContro->ServerControl(cbHandCardData,m_cbCenterCardData, m_pITableFrame);
  }
  ...
  GameStart.wMinChipInUser = m_wMinChipInUser;
    ...
    case GER_NORMAL:    //常规结束
    {
      ...
      try
      {
        ...
        for(int i=0; i<m_wPlayerCount-1; i++)
        {
          ...
          {
            if(0 == lTotalScore[WinnerList[i].wWinerList[j]])
            {
              continue;
            }

            if(j>0 && lTotalScore[WinnerList[i].wWinerList[j]] -
               lTotalScore[WinnerList[i].wWinerList[j-1]] == 0)
            {
              continue;
            }
            ...
            for(int k=i+1; k<m_wPlayerCount; k++)
            {
              ...
              for(int l=0; l<WinnerList[k].bSameCount; l++)
              {
                ...
                LONGLONG lMinScore = 0;

                //上家得分数目
                LONGLONG lLastScore = ((j>0)?lTotalScore[WinnerList[i].wWinerList[j-1]]:0);
                if(j>0)
                {
                  ASSERT(lLastScore>0L);
                }
      ...
      GameEnd.cbDelayOverGame = bDelayOverGame?(IDI_DELAY_TIME/1000):0;
      //发送信息
...
bool CTableFrameSink::OnUserAddScore(WORD wChairID, LONGLONG lScore, bool bGiveUp)
{
  ...
  if(!bFinishTurn)
  {
    ...
    {
      LONGLONG bTemp = (m_lBalanceScore==m_lCellScore*2)?(m_lCellScore*2):((m_lBalanceScore-m_lCellScore*2)*2);
  ...
  CopyMemory(SendCard.cbCenterCardData,m_cbCenterCardData,(m_cbBalanceCount+2)*sizeof(BYTE));
