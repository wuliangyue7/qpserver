...
CGoldControl::CGoldControl()
{
  ...
  m_ImageMid.LoadFromResource(hInstance,IDB_MID);
  m_ImageRight.LoadFromResource(hInstance,IDB_RIGHT);
...
LONGLONG CGoldControl::GetGold()
{
  ...
  {
    lGold+=m_lGoldCell[i]*(LONGLONG)pow(10L,i);
  }
  return lGold;
}
...
void CGoldControl::DrawGoldCell(CDC * pDC, int nXPos, int nYPox, LONGLONG lGold)
{
  if(lGold<10L)
  {
    ...
    m_ImageNumber.BitBlt(pDC->m_hDC,nXPos,nYPox,IMAGE_WIDTH,IMAGE_HEIGHT,LONG(lGold)*IMAGE_WIDTH,0,SRCCOPY);
  }
  else
  {
...
void CGoldControl::OnLButtonUp(UINT nFlags, CPoint point)
{
  ...
  //if (m_lGoldCell[iCellPos]==9L)
  //{
  //  LONGLONG lAddGold=(LONGLONG)pow(10L,iCellPos)*9;
