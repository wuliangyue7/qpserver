...
public:
  ...
  //设置位置
  void SetMePost(WORD wMeChairID)
  {
  ...
  void SetOverTimer()
  {
    SetTimer(IDI_GAME_OVER,50,NULL);
  }
  //复位标志
  void SetResetTag(bool bReset=false)
  {
    m_bReset = bReset;
  }
  //复位标志
  void SetUserGender(WORD wViewChairID,WORD wGender)
  {
    m_wUserGender[wViewChairID] = wGender;
  }
  //结束提示框
  void SetNoScoreDlg()
  {
    SetTimer(IDI_NO_SCORE,50,NULL);
  }
  //结束牌型
