...
bool CGameClientEngine::OnEventSceneMessage(BYTE cbGameStatus, bool bLookonUser, VOID * pData, WORD wDataSize)
{
  ...
  switch(cbGameStatus)
  {
    ...
    case GAME_STATUS_PLAY:  //游戏状态
    {
      ...
      //设置变量
      m_wMinChipInUser = pStatusPlay->wMinChipInUser;
      ...
      m_GameClientView.SetDFlag(pStatusPlay->wDUser);
...
{
  ////TEST+
  //BYTE cbTestCard[] = {0x01,0x02,0x03,0x04,0x05};
  //m_GameClientView.m_CenterGoldView.SetGold(10000);
  //for (WORD i = 0;i<GAME_PLAYER;i++)
  //{
  //  m_GameClientView.SetUserTableScore(i, 100);
  //  m_GameClientView.m_SmallCardControl[i].SetCardData(MAX_COUNT);
  //  m_GameClientView.m_CardControl[i].SetCardData(cbTestCard,2);
  //}
  //m_GameClientView.m_CenterCardControl.SetCardData(cbTestCard,5);
  //return 0;
  ////TEST+
  ...
  //设置界面
  m_GameClientView.ShowOverCardType(INVALID_CHAIR,0);
...
LRESULT CGameClientEngine::OnShowHand(WPARAM wParam,LPARAM lParam)
{
  ...
  if(ShowHandObj.DoModal()==IDOK)
  {
    ...
    {
      IClientUserItem * pClientUserItem = GetTableUserItem(i);
      if(pClientUserItem!=NULL && pClientUserItem->GetUserStatus()==US_PLAYING)
      {
        break;
      }
    }
...
LRESULT CGameClientEngine::OnGameOver(WPARAM wParam,LPARAM lParam)
{
  ...
  else
  {
    ...
    for(WORD i = 0; i<GAME_PLAYER; i++)
    {
      ...
      {
        m_GameClientView.m_CardControl[i].SetCardData(m_cbHandCardData[i],2);
        m_GameClientView.ShowOverCardType(i, m_GameLogic.GetCardType(bTempData[i], MAX_CARD_COUNT));
  ...
  if(m_pIStringMessage!=NULL)
  {
    ...
    if(m_bReset)
    {
      for(WORD i=0; i<GAME_PLAYER; i++)
      {
        ...
        if(pClientUserItem!=NULL)
        {
          _sntprintf(szBuffer,CountArray(szBuffer),TEXT("%s：")SCORE_STRING,/*%s玩家\n得分:%ld*/
...
bool CGameClientEngine::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
  ...
  //加注信息
  m_lTableScore[pGameStart->wMinChipInUser] += m_lCellScore;
  m_lTableScore[pGameStart->wMaxChipInUser] = 2*m_lCellScore;
  m_lTotalScore[pGameStart->wMinChipInUser] =  m_lCellScore;
  m_lTotalScore[pGameStart->wMaxChipInUser] = 2*m_lCellScore;

  //设置界面
  m_GameClientView.SetDFlag(pGameStart->wDUser);
  m_GameClientView.DrawMoveAnte(pGameStart->wMinChipInUser,CGameClientView::AA_BASEFROM_TO_BASEDEST,m_lTotalScore[pGameStart->wMinChipInUser]);
  ...
  m_GameClientView.SetTotalScore(pGameStart->wMinChipInUser,m_lTotalScore[pGameStart->wMinChipInUser]);
...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
  ...
  {
    SetGameClock(GetMeChairID(),IDI_GAME_END_DELAY,pGameEnd->cbDelayOverGame);
  }
  else
  {
