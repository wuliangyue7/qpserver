...
CScoreView::CScoreView(void)
{
  ...
  //加载资源
  m_ImageBack.LoadFromResource(AfxGetInstanceHandle(),IDB_GAME_WIN);
}
...
void CScoreView::OnPaint()
{
  ...
  if(m_lGameScore > 0L)
  {
    ...
    m_ImageBack.TransDrawImage(&dc,0,0,m_ImageBack.GetWidth()/3,m_ImageBack.GetHeight(),0,0,RGB(255,0,255));
  }
  else if(m_lGameScore == 0L)
  {
    m_ImageBack.TransDrawImage(&dc,0,0,m_ImageBack.GetWidth()/3,m_ImageBack.GetHeight(),m_ImageBack.GetWidth()/3,0,RGB(255,0,255));
  }
  else
  {
    m_ImageBack.TransDrawImage(&dc,0,0,m_ImageBack.GetWidth()/3,m_ImageBack.GetHeight(),2*m_ImageBack.GetWidth()/3,0,RGB(255,0,255));
  }
  ...
  //用户积分
  _sntprintf(szBuffer,sizeof(szBuffer),TEXT("%ld"),m_lGameScore);
  ...
  //绘画信息
  _sntprintf(szBuffe,sizeof(szBuffer),TEXT("%ld"),m_bTimes);
  //dc.Ellipse(&rcScor);
...
//设置积分
void CScoreView::SetGameScore(WORD wChairID,  LONGLONG lScore)
{
  //设置变量
...
void CScoreView::OnTimer(UINT nIDEvent)
{
  ...
  if(DLG_IDI_CLOSE==nIDEvent)
  {
    ...
    if(m_bTimes<=0)
    {
      ...
      {
        m_bStart=false;
        ((CGameClientView *)(this->GetParent()))->PostEngineMessage(IDM_START_TIMES,0,0);
      }
    }
    else
    {
