...
WORD CCardControl::GetCardData(BYTE cbCardData[], BYTE cbBufferCount)
{
  ...
  if(cbBufferCount<m_cbCardCount)
  {
    return 0;
  }
...
WORD CCardControl::SwitchCardPoint(CPoint & MousePoint)
{
  ...
  if(m_bHorizontal==true)
  {
    ...
    if(wCardIndex>=m_cbCardCount)
    {
      wCardIndex=(m_cbCardCount-1);
    }
...
bool CCardControl::LoadResource(CWnd* pWnd)
{
  ...
  m_ImageCard.LoadImage(pWnd, hInstance,TEXT("PNG_CARD"));
  m_ImageCardMask.LoadImage(pWnd, hInstance,TEXT("CARD_MASK"));
  m_ImageWin.LoadImage(pWnd, hInstance,TEXT("PNG_MAX"));
