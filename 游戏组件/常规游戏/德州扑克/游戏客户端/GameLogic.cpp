...
BYTE CGameLogic::GetCardType(BYTE cbCardData[], BYTE cbCardCount)
{
  ...
  if(cbCardCount !=MAX_CENTERCOUNT)
  {
    return 0;
  }
...
BYTE CGameLogic::GetSameCard(const BYTE bCardData[],const BYTE bMaxCard[],BYTE bCardCount,BYTE bMaxCardCount,BYTE bResultData[])
{
  ...
  for(BYTE i=0; i<bCardCount; i++)
  {
    ...
    {
      if(bCardData[i]==bMaxCard[j])
      {
        bResultData[bTempCount++] = bMaxCard[j];
      }
    }
  }
  return bTempCount;
...
//查找最大
bool CGameLogic::SelectMaxUser(BYTE bCardData[GAME_PLAYER][MAX_CENTERCOUNT],UserWinList &EndResult,const LONGLONG lAddScore[])
{
  //清理数据
  ...
  {
    if(i==wWinnerID || bCardData[i][0]==0)
    {
      continue;
    }
  ...
  if(EndResult.bSameCount>1 && lAddScore!=NULL)
  {
    ...
    while((iSameCount--)>0)
    {
      LONGLONG lTempSocre = lAddScore[EndResult.wWinerList[iSameCount]];
