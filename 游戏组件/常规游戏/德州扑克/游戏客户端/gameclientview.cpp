...
CGameClientView::CGameClientView(void)
{
  ...
  ZeroMemory(m_wCardType,sizeof(m_wCardType));
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
  ...
  m_ImageCradType.LoadImage(this, hInstance, TEXT("CARD_TYPE"));
...
VOID CGameClientView::ResetGameView()
{
  ...
  ShowOverCardType(INVALID_CHAIR,0);
...
VOID CGameClientView::RectifyControl(INT nWidth, INT nHeight)
{
  ...
  //D位置
  m_ptD[0].x = nWidth/2+95;
  m_ptD[0].y = nHeight/2-97;
  m_ptD[1].x = nWidth/2+175;
  m_ptD[1].y = nHeight/2-55;
  m_ptD[2].x = nWidth/2+170;
  m_ptD[2].y = nHeight/2+25;
  m_ptD[3].x = nWidth/2+70;
  m_ptD[3].y = nHeight/2+55;
  m_ptD[4].x = nWidth/2-95;
  m_ptD[4].y = nHeight/2+55;
  m_ptD[5].x = nWidth/2-195;
  m_ptD[5].y = nHeight/2+25;
  m_ptD[6].x = nWidth/2-200;
  m_ptD[6].y = nHeight/2-55;
  m_ptD[7].x = nWidth/2-105;
  m_ptD[7].y = nHeight/2-97;
  for(INT i=0; i<GAME_PLAYER; i++)
  {
    m_ptD[i].y -=10;
  }
...
VOID CGameClientView::DrawGameView(CDC * pDC, INT nWidth, INT nHeight)
{
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    else if(pClientUserItem->GetGender() == 0)
    {
  ...
  for(WORD i=0; i<GAME_PLAYER; i++)
  {
    ...
    if(pClientUserItem!=NULL)
    {
      ...
      if(lLeaveScore<0)
      {
        lLeaveScore=pClientUserItem->GetUserScore();
      }
  ...
  for(int i = 0; i < GAME_PLAYER; ++i)
  {
    if(m_wCardType[i] != 0)
    {
      ...
      INT nXPos = ptCenterPoint.x - m_ImageCradType.GetWidth()/20;
      INT nYPos = ptCenterPoint.y - m_ImageCradType.GetHeight()+40;

      //绘画动作
      m_ImageCradType.DrawImage(pDC,nXPos, nYPos,
                                m_ImageCradType.GetWidth()/10, m_ImageCradType.GetHeight(), (m_wCardType[i]-1)*m_ImageCradType.GetWidth()/10, 0);
    }
  }
    ...
    else if(pCardStatus->wMoveCount>0)
    {
      ...
      switch(pCardStatus->iMoveType)
      {
        ...
        case TO_CENTER_CARD:    //中心扑克
        {
          ...
          if(pCardStatus->bCard==0 || cbCount>=MAX_CENTERCOUNT)
          {
            break;
          }
...
LPCTSTR CGameClientView::GetGlodString(LONGLONG lGold, TCHAR szString[])
{
  ...
  //调整字符
  if(szString[dwCharBit-1]==TEXT(','))
  {
    szString[dwCharBit-1]=0;
  }
...
//结束牌型
void CGameClientView::ShowOverCardType(WORD wViewChairID, BYTE cbCardType)
{
  if(wViewChairID != INVALID_CHAIR)
  {
    m_wCardType[wViewChairID] = cbCardType;
  }
  else
  {
    ZeroMemory(m_wCardType, sizeof(m_wCardType));
