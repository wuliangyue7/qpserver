...
VOID CJettonControl::LoadResource(CWnd *pWnd)
{
  //加载位图
  HINSTANCE hInst = AfxGetInstanceHandle();
  m_PngJetton.LoadImage(pWnd,hInst,TEXT("JETTON_VIEW"));
}
...
//设置筹码
VOID CJettonControl::SetScore(LONGLONG lScore)
{
  if(m_lScore != lScore)
  {
    m_lScore = lScore;
...
//添加筹码
void CJettonControl::AddScore(LONGLONG lScore)
{
  m_lScore += lScore;
  RectifyControl();
}
...
void CJettonControl::RectifyControl()
{
     ...
     m_lDrawScore != m_lScore)
  {
    ...
    {
      lDrawScore = m_lScore - m_lDrawScore;
    }
    m_lDrawScore = m_lScore;
    if(lDrawScore > 0L)
    {
      LONGLONG lJettons[] = {1L,5L,10L,50L,100L,500L,1000L,5000L,10000L,50000L,100000L,500000L,1000000L,5000000L};
