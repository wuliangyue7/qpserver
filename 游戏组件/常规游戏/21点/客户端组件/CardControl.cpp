...
void CCardControl::OnLButtonDown(UINT nFlags, CPoint Point)
{
  ...
  //状态判断
  if((m_bPositively==false)||(m_bDisplayHead==true))
  {
    return;
  }
  if((Point.x>(int)m_dwCardHSpace)||(m_CardDataArray.GetCount()==0))
  {
    return;
  }
  ...
  m_ImageCard.TransDrawImage(&ClientDC,0,0,nDrawWidth,m_CardSize.cy,dwXImagePos,dwYImagePos,RGB(255,0,255));
...
CPoint CCardControl::GetTailPos()
{
  ...
  switch(m_XCollocateMode)
  {
    case enXLeft:
    {
      ptTail.x=m_BenchmarkPos.x+nSpaceWidth;
      break;
    }
    case enXCenter:
    {
      ptTail.x=m_BenchmarkPos.x-ControlSize.cx/2+nSpaceWidth;
      break;
    }
    case enXRight:
    {
      ptTail.x=m_BenchmarkPos.x-ControlSize.cx+nSpaceWidth;
      break;
    }
  }
  ...
  switch(m_YCollocateMode)
  {
    case enYTop:
    {
      ptTail.y=m_BenchmarkPos.y;
      break;
    }
    case enYCenter:
    {
      ptTail.y=m_BenchmarkPos.y-ControlSize.cy/2;
      break;
    }
    case enYBottom:
    {
      ptTail.y=m_BenchmarkPos.y-ControlSize.cy;
      break;
    }
  }
...
void CCardControl::GetControlRect(CRect &rc)
{
  ...
  switch(m_XCollocateMode)
  {
    case enXLeft:
    {
      rc.left = m_BenchmarkPos.x;
      rc.right = m_BenchmarkPos.x+ControlSize.cx;
      break;
    }
    case enXCenter:
    {
      rc.left = m_BenchmarkPos.x-ControlSize.cx/2;
      rc.right = m_BenchmarkPos.x+ControlSize.cx/2;
      break;
    }
    case enXRight:
    {
      rc.left = m_BenchmarkPos.x-ControlSize.cx;
      rc.right = m_BenchmarkPos.x;
      break;
    }
  }
  ...
  switch(m_YCollocateMode)
  {
    case enYTop:
    {
      rc.top = m_BenchmarkPos.y;
      rc.bottom = m_BenchmarkPos.y+ControlSize.cy;
      break;
    }
    case enYCenter:
    {
      rc.top = m_BenchmarkPos.y-ControlSize.cy/2;
      rc.bottom = m_BenchmarkPos.y+ControlSize.cy/2;
      break;
    }
    case enYBottom:
    {
      rc.top = m_BenchmarkPos.y-ControlSize.cy;
      rc.bottom = m_BenchmarkPos.y;
      break;
    }
  }
