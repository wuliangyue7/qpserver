...
bool CGameServiceManager::RectifyParameter(tagGameServiceOption & GameServiceOption)
{
  ...
  if(GameServiceOption.wServerType&(GAME_GENRE_GOLD|SCORE_GENRE_POSITIVE))
  {
    /**
     *最小坐下为底分900倍（底分最小为1）
     *  分牌 1+12个加倍（1）*2+（1）*2 共4倍 陪3家12倍 最大下分为底注50倍12*50 600倍底注
     *  不计保险 因庄输保险为BJ时，BJ时庄赢牌
     *
     */
    GameServiceOption.lMinTableScore=__max(GameServiceOption.lCellScore*600L,GameServiceOption.lMinTableScore);
  }
