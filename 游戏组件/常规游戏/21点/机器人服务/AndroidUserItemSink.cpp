  ...
  try
  {
    switch(nTimerID)
    {
      ...
      case IDI_USER_ADD_SCORE:  //加注定时
      {
        ...
        int nRandScore = rand()%50+1;
        AddScore.lScore=nRandScore*m_lCellScore;
      ...
      {
        m_pIAndroidUserItem->SendSocketData(SUB_C_STOP_CARD);
        return true;
      }
      case IDI_DOUBLE_CARD:     //加倍定时
      {
        m_pIAndroidUserItem->SendSocketData(SUB_C_DOUBLE_SCORE);
        return true;
      }
      case IDI_SPLIT_CARD:      //分牌定时
      {
        m_pIAndroidUserItem->SendSocketData(SUB_C_SPLIT_CARD);
        return true;
      }
      case IDI_GET_CARD:
      {
        m_pIAndroidUserItem->SendSocketData(SUB_C_GET_CARD);
        return true;
      }
...
bool CAndroidUserItemSink::OnSubStopCard(const void * pBuffer, WORD wDataSize)
{
  ...
  if(pStopCard->bTurnBanker)
  {
    ...
    if(m_wBankerUser == wMeChairId)
    {
      if(m_wBankerUser != wMeChairId && m_GameLogic.GetCardValue(m_HandCardData[0][1]) == m_GameLogic.GetCardValue(m_HandCardData[0][0]))
      {
        //下注时间
        UINT nElapse=rand()%(4)+TIME_LESS;
        m_pIAndroidUserItem->SetGameTimer(IDI_SPLIT_CARD,nElapse);
...
bool CAndroidUserItemSink::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
  ...
  {
    UINT nElapse=rand()%(3)+TIME_LESS + 3;
    m_pIAndroidUserItem->SetGameTimer(IDI_START_GAME,nElapse);
  }
