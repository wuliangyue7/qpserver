...
//服务器控制
bool __cdecl CServerControlItemSink::ServerControl(BYTE cbHandCardData[GAME_PLAYER*2][11], ITableFrame * pITableFrame)
{
...
bool __cdecl CServerControlItemSink::ControlResult(BYTE cbControlCardData[GAME_PLAYER * 2][MAX_COUNT], BYTE cbCardCount[GAME_PLAYER*2], ROOMUSERCONTROL Keyroomusercontrol, ITableFrame * pITableFrame, WORD wBankerUser, BYTE cbPlayStatus[GAME_PLAYER])
{
  ...
  CopyMemory(cbUserCardData, cbControlCardData, sizeof(cbUserCardData));
  ...
  {
    //用户过滤
    if(cbPlayStatus[i]==FALSE || i == wBankerUser)
    {
      continue;
    }
  ...
  if(Keyroomusercontrol.userControl.control_type == CONTINUE_WIN)
  {
    ...
    CopyMemory(cbTempData, cbUserCardData[Keyroomusercontrol.roomUserInfo.wChairID * 2],MAX_COUNT);
    CopyMemory(cbUserCardData[Keyroomusercontrol.roomUserInfo.wChairID * 2], cbUserCardData[wWinUser*2],MAX_COUNT);
    CopyMemory(cbUserCardData[wWinUser*2], cbTempData,MAX_COUNT);

    //拷贝扑克
    CopyMemory(cbControlCardData, cbUserCardData, sizeof(cbUserCardData));
    ...
    for(WORD i=0; i<GAME_PLAYER; i++)
    {
      ...
      {
        if(m_GameLogic.GetCardLogicValue(cbControlCardData[i * 2][1]) < m_GameLogic.GetCardLogicValue(cbControlCardData[i * 2][0]))
        {
          BYTE cbTemp = cbControlCardData[i * 2][1];
          cbControlCardData[i * 2][1] = cbControlCardData[i * 2][0];
          cbControlCardData[i * 2][0] = cbTemp;
        }
      }
    }
  ...
  else if(Keyroomusercontrol.userControl.control_type == CONTINUE_LOST)
  {
    ...
    BYTE cbTempData[MAX_COUNT];
    CopyMemory(cbTempData, cbUserCardData[Keyroomusercontrol.roomUserInfo.wChairID * 2],MAX_COUNT);
    CopyMemory(cbUserCardData[Keyroomusercontrol.roomUserInfo.wChairID * 2], cbUserCardData[wMinUser*2],MAX_COUNT);
