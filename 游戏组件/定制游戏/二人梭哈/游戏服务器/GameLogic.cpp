...
BYTE CGameLogic::GetCardGenre(BYTE cbCardData[], BYTE cbCardCount)
{
  ...
  CopyMemory(cbCardTemp,cbCardData,sizeof(cbCardTemp));
  ...
  if(cbCardCount>4)
  {
    ...
    {
      //数据分析
      if(GetCardColor(cbCardTemp[i])!=cbFirstColor)
      {
        cbSameColor=false;
      }
      if(cbFirstValue!=(GetCardLogicValue(cbCardTemp[i])+i))
      {
        bLineCard=false;
      }
...
#ifdef  MAGIC_A_CARD
    //特殊
    if(!bLineCard && GetCardValue(cbCardTemp[0])==1)
    {
      //A J 10 9 8特殊
      BYTE cbMinusCount = cbCardCount==4?3:2;
      for(BYTE i=1; i<cbCardCount; i++)
      {
        if((cbFirstValue-cbMinusCount)!=(GetCardLogicValue(cbCardTemp[i])+i))
        {
          break;
        }
      }
  ...
  //对牌类型
  if((AnalyseResult.cbDoubleCount==1)&&(AnalyseResult.cbThreeCount==0))
  {
  ...
  {
    return (AnalyseResult.cbDoubleCount==1)?CT_HU_LU:CT_THREE_TIAO;
  }
...
bool CGameLogic::CompareCard(BYTE cbFirstCardData[], BYTE cbNextCardData[], BYTE cbCardCount)
{
  ...
  switch(cbFirstGenre)
  {
    ...
    case CT_THREE_TIAO:   //三条
    {
      ...
      {
        SortCardList(AnalyseResultNext.cbSignedLogicVolue,AnalyseResultNext.cbSignedCount);
        SortCardList(AnalyseResultFirst.cbSignedLogicVolue,AnalyseResultFirst.cbSignedCount);
      ...
      {
        SortCardList(AnalyseResultNext.cbSignedCardData,AnalyseResultNext.cbSignedCount);
        SortCardList(AnalyseResultFirst.cbSignedCardData,AnalyseResultFirst.cbSignedCount);
...
void CGameLogic::AnalysebCardData(const BYTE cbCardData[], BYTE cbCardCount, tagAnalyseResult & AnalyseResult)
{
  ...
  for(BYTE i=0; i<cbCardCount; i++)
  {
    ...
    bool bContiue=false;
    for(int nDouble=0; nDouble<MAX_COUNT; nDouble++)
    {
      if((cbCardData[i]==AnalyseResult.cbDoubleCardData[nDouble])||(cbCardData[i]==AnalyseResult.cbThreeCardData[nDouble])||(cbCardData[i]==AnalyseResult.cbFourCardData[nDouble]))
      {
        bContiue=true;
        break;
      }
