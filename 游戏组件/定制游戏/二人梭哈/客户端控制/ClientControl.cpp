...
//建立对象函数
extern "C" __declspec(dllexport) void * __cdecl CreateClientControlEx(CWnd *pParentWnd)
{
  //建立对象
  CClientControlItemSinkDlgEx * pServerControl = NULL;
  try
  {
    pServerControl = new CClientControlItemSinkDlgEx(pParentWnd);
    if(pServerControl == NULL)
    {
      throw TEXT("创建失败");
    }
