...
CGoldView::CGoldView()
{
  ...
  {
    m_bInit=true;
    m_ImageGold.LoadFromResource(GetModuleHandle(NULL),IDB_GOLD);
  }
...
//创建函数
void CGoldView::OnCreate(CWnd* pWnd)
{
  m_DFontEx.CreateFont(pWnd, TEXT("宋体"), 12, 800);
}
...
//绘画筹码
void CGoldView::DrawGoldView(CDC * pDC, int nXPos, int nYPos, COLORREF crColor)
{
  //绘画判断
  if(m_lGold<=0L)
  {
  ...
  m_ImageGold.TransDrawImage(pDC,nXPos1,nYPos1,nJettonWidth,nJettonHeight,nJettonWidth*m_nGoldIndex,0,RGB(255,0,255));

  //画数字
  TCHAR szNum[64];
  //_snprintf( szNum,CountArray(szNum),TEXT("%I64d"),m_lScore );
  GetGlodString(m_lGold,szNum);
  CRect rcNum(nXPos1,nYPos1,nXPos1+nJettonWidth,nYPos1+nJettonHeight);
  m_DFontEx.DrawText(pDC, szNum,rcNum,crColor,DT_VCENTER|DT_CENTER|DT_SINGLELINE|DT_END_ELLIPSIS);
