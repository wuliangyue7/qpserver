...
LPDIRECTSOUND CDirectSound::m_lpDirectSound=0;
...
BOOL CDirectSound::TestDevice()
{
  if(m_lpDirectSound == 0)
  {
    ...
    while(nRes < 5 && (hRes == DSERR_NODRIVER));
...
BOOL CDirectSound::IsEnd()
{
  if(! IsValid()||!IsEnabled())
  {
    return TRUE;  // no chance to play the sound ...
  }
