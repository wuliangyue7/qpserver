...
CCardControl::CCardControl()
{
  ...
  //m_ImageCard.LoadFromResource(hResInstance,IDB_CARD);

  m_PngCard.LoadImage(hResInstance,TEXT("PNG_CARD"));

  //获取大小
  m_CardSize.SetSize(m_PngCard.GetWidth()/13,m_PngCard.GetHeight()/5);
...
BYTE CCardControl::GetCardLogicValue(BYTE cbCardData)
{
  //扑克属性
  BYTE cbReturn=0;
  BYTE cbCardColor=MASK_COLOR&(cbCardData);
  BYTE cbCardValue=MASK_VALUE&(cbCardData);
...
//继续弹起
bool CCardControl::ContinueShootCard(const BYTE cbCardData[], WORD wCardCount)
{
...
WORD CCardControl::GetShootCard(tagShootCard shootCard[],WORD&wMaxCount)
{
  //变量定义
  wMaxCount=0;
  //搜索扑克
  for(DWORD i=0; i<m_wCardCount; i++)
  {
    //扑克分析
    if(m_CardItemArray[i].bShoot==true)
    {
      shootCard[wMaxCount].cbCardData=m_CardItemArray[i].cbCardData;
      shootCard[wMaxCount].dwCardIndex=i;
      wMaxCount++;
    }
  }
  return wMaxCount;
...
VOID CCardControl::DrawCardControl(CDC * pDC)
{
  ...
  for(WORD i=0; i<m_wCardCount; i++)
  {
    ...
    m_PngCard.DrawImage(pDC,OriginPoint.x+nXDrawPos,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,nXImagePos,nYImagePos,m_CardSize.cx,m_CardSize.cy);
    //绘画选择
    if((m_wStartIndex<=i)&&(i<=m_wEndIndex))
    {
      ...
      ImageCardSelect.DrawImage(pDC,OriginPoint.x+nXDrawPos,OriginPoint.y+nYDrawPos,m_CardSize.cx,m_CardSize.cy,0,0,SizeSelectImage.cx,SizeSelectImage.cy);
    }
  }
