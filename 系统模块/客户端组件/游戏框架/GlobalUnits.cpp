...
bool CGlobalUnits::InitGlobalUnits()
{
  ...
  //创建组件
  if(m_ClientKernelModule.CreateInstance()==false)
  {
    return false;
  }
  if(m_FaceItemControlModule.CreateInstance()==false)
  {
    return false;
  }
  ...
  if(m_UserOrderParserModule.CreateInstance()==false)
  {
    return false;
  }
  ...
  //配置内核
  if(m_ClientKernelModule->InitClientKernel(AfxGetApp()->m_lpCmdLine)==false)
  {
    return false;
  }
...
bool CGlobalUnits::UnInitGlobalUnits()
{
  ...
  //销毁组件
  if(m_ClientKernelModule.GetInterface()!=NULL)
  {
    m_ClientKernelModule.CloseInstance();
  }
  if(m_FaceItemControlModule.GetInterface()!=NULL)
  {
    m_FaceItemControlModule.CloseInstance();
  }
  if(m_GameLevelParserModule.GetInterface()!=NULL)
  {
    m_GameLevelParserModule.CloseInstance();
  }
...
{
  //播放判断
  if(m_bMuteStatuts==true)
  {
    return false;
  }
  if(m_bAllowSound==false)
  {
    return false;
  }
...
bool CGlobalUnits::PlayBackGroundSound(LPCTSTR pszSoundName)
{
  ...
  if(m_bAllowBackGroundSound==false)
  {
    return false;
  }
  if(AfxGetMainWnd() == NULL)
...
bool CGlobalUnits::RegisterGlobalModule(WORD wModuleID, IUnknownEx * pIUnknownEx)
{
  ...
  if((wModuleID==0L)||(pIUnknownEx==NULL))
  {
    return false;
  }
