...
void CGameFrameWnd::InitControlUI()
{
  ...
  if(pControlUI!=NULL)
  {
    pControlUI->SetVisible(pGlobalUnits->m_bMuteStatuts);
  }
...
VOID CGameFrameWnd::RectifyControl(INT nWidth, INT nHeight)
{
  ...
  CRect rcArce;
  SystemParametersInfo(SPI_GETWORKAREA,0,&rcArce,SPIF_SENDCHANGE);
  if(rcArce.Width()<=1280)
  {
    rcSplitter.left=nWidth-238;
    rcSplitter.right=rcSplitter.left+SPLITTER_CX;
  }
  //查询游戏
  CGlobalUnits * pGlobalUnits=CGlobalUnits::GetInstance();
  ...
  if(m_bMaxShow==false)
  {
    DeferWindowPos(hDwp,hWndView,NULL,
                   EncircleInfoFrame.nLBorder,
                   EncircleInfoFrame.nTBorder,
                   m_nScrollXMax-(EncircleInfoFrame.nRBorder+EncircleInfoFrame.nLBorder),
                   m_nScrollYMax-(EncircleInfoFrame.nTBorder+EncircleInfoFrame.nBBorder),
                   uFlags);
  }
  else
  {
    DeferWindowPos(hDwp,hWndView,NULL,
                   ...
                   uFlags);
    //DeferWindowPos(hDwp,hWndView,NULL,
    //  EncircleInfoFrame.nLBorder+EncircleInfoView.nLBorder,
    //  EncircleInfoFrame.nTBorder+EncircleInfoView.nTBorder,
    //  rcSplitter.left-EncircleInfoFrame.nLBorder-EncircleInfoView.nLBorder-EncircleInfoView.nRBorder,
    //  m_nScrollYMax-EncircleInfoFrame.nTBorder-EncircleInfoFrame.nBBorder-EncircleInfoView.nTBorder-EncircleInfoView.nBBorder,
    //  uFlags);
  }
  //移动控件
