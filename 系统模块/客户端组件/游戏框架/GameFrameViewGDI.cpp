...
VOID CGameFrameViewGDI::DrawUserReady(CDC * pDC, INT nXPos, INT nYPos)
{
  ...
  if(RealizeWIN7() && CD2DEngine::GetD2DEngine())
  {
    ...
    {
      if(!m_ImageUserReadyEx.LoadImage(this,GetModuleHandle(GAME_FRAME_DLL_NAME),TEXT("USER_READY")))
      {
        return;
      }
    }
...
VOID CGameFrameViewGDI::DrawUserClock(CDC * pDC, INT nXPos, INT nYPos, WORD wUserClock)
{
  ...
  if((wUserClock>0)&&(wUserClock<100))
  {
    ...
    if(RealizeWIN7() && CD2DEngine::GetD2DEngine())
    {
      ...
      {
        if(!m_ImageClockItemEx.LoadImage(this,GetModuleHandle(GAME_FRAME_DLL_NAME),TEXT("USER_CLOCK_ITEM")))
        {
          return;
        }
      }
      ...
      {
        if(!m_ImageClockBackEx.LoadImage(this,GetModuleHandle(GAME_FRAME_DLL_NAME),TEXT("USER_CLOCK_BACK")))
        {
          return;
        }
      }
...
VOID CGameFrameViewGDI::DrawUserAvatar(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem)
{
  ...
  else
  {
    ...
    if(pGameFrameAvatar!=NULL)
    {
      pGameFrameAvatar->DrawUserAvatar(pDC,nXPos,nYPos,nWidth,nHeight,pIClientUserItem);
    }
  }
...
void CGameFrameViewGDI::AddGameTableMessage(LPCTSTR pszMessage,WORD wLen,WORD wType)
{
  if(wType&SMT_TABLE_ROLL)
  {
    ...
    if(m_strRollText.Find(TEXT("系统配桌"))>=0)
    {
      m_bShowWaitDistribute=TRUE;
    }
    if(m_vecText.empty())
    {
      m_wRollTextCount=160;
