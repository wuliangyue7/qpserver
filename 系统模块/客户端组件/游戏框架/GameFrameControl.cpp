...
VOID CGameFrameControl::TrackUserItemMenu(IClientUserItem * pIClientUserItem)
{
  ...
  if(ImageUserCard.IsNull()==false)
  {
    UserInfoMenu.AppendMenu(IDM_NULL_COMMAND,ImageUserCard,MF_GRAYED);
  }
...
VOID CGameFrameControl::TrackUserItemMenu(IClientUserItem * pIClientUserItem[], WORD wItemCount)
{
  ...
  if((pIClientUserItem==NULL)||(wItemCount<=1))
  {
    return;
  }
...
VOID CGameFrameControl::OnEventUserLeave(IClientUserItem * pIClientUserItem, bool bLookonUser)
{
  ...
  for(INT_PTR i=0; i<m_MenuUserItemArray.GetCount(); i++)
  {
    ...
    if(pIChatUserItem->GetUserID()==dwLeaveUserID)
    {
      m_MenuUserItemArray.RemoveAt(i);
    }
  }
...
BOOL CGameFrameControl::OnInitDialog()
{
  ...
  PropertyResource.hResInstance=hInstance;

  //道具位置
  PropertyResource.PointExcursion.x=4;
  PropertyResource.PointExcursion.y=7;
  PropertyResource.SizeExcursion.cx=35;
  PropertyResource.SizeExcursion.cy=37;
  PropertyResource.LButtonExcursion.x=10;
  PropertyResource.LButtonExcursion.y=30;
  PropertyResource.RButtonExcursion.x=10;
  PropertyResource.RButtonExcursion.y=30;
...
BOOL CGameFrameControl::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch(nCommandID)
  {
    case IDM_SET_CHAT_USER:     //交谈对象
    {
      ...
      if(m_MenuUserItemArray.GetCount()==0)
      {
        return TRUE;
      }
      ...
      if(pIClientUserItem!=NULL)
      {
        m_ChatControl.SetChatTargetUser(pIClientUserItem);
      }
    ...
    case IDM_KICK_USER:             //踢出用户
    {
      ...
      if((bHideUserInfo==true)&&(m_pIMySelfUserItem->GetMemberOrder()<=pIClientUserItem->GetMemberOrder()))
      {
        return FALSE;
      }
  ...
  if(lstrcmp(msg.sType, TEXT("click")) == 0)
  {
    ...
    else if(lstrcmp(pControlUI->GetName(),szButtonChat2ControlName) == 0)
    {
      ...
      for(INT i=0; i<CountArray(ImageColor); i++)
      {
        ...
        if(pGlobalUnits->m_crChatTX==crColor[i])
        {
          ColorMenu.CheckMenuItem(IDM_SELECT_CHAT_COLOR+i,MF_BYCOMMAND|MF_CHECKED);
        }
      }

      //查找对象
      CControlUI * pChatButton2=GetControlByName(szButtonChat2ControlName);
      if(pChatButton2==NULL)
      {
        return;
      }
    ...
    else if(lstrcmp(pControlUI->GetName(),szButtonChat3ControlName) == 0)
    {
      ...
      if(cbMessageMode==MESSAGE_MODE_ALL)
      {
        MenuMessage.CheckMenuItem(IDM_MESSAGE_SHOW_ALL,MF_BYCOMMAND|MF_CHECKED);
      }
      if(cbMessageMode==MESSAGE_MODE_DETEST)
      {
        MenuMessage.CheckMenuItem(IDM_MESSAGE_HIDE_DETEST,MF_BYCOMMAND|MF_CHECKED);
      }
      if(cbMessageMode==MESSAGE_MODE_FRIEND)
      {
        MenuMessage.CheckMenuItem(IDM_MESSAGE_ONLY_FRIEND,MF_BYCOMMAND|MF_CHECKED);
      }

      //查找对象
      CControlUI * pChatButton3=GetControlByName(szButtonChat3ControlName);
      if(pChatButton3==NULL)
      {
        return;
      }
...
VOID CGameFrameControl::RectifyControl(INT nWidth, INT nHeight)
{
  ...
  ////聊天按钮
  //INT nXPosChat=15;
  //INT nYPosChat=nHeight-52;
  //INT nChatButtonSpace=1;
  //LPCTSTR ChatButtonArray[]={szButtonChat1ControlName,szButtonChat2ControlName,szButtonChat3ControlName,szButtonChat4ControlName};
  ...
  ////发送按钮
  //pControlUI=GetControlByName(szButtonSendChatControlName);
  //if(pControlUI) pControlUI->SetPos(rcChatArea.left+22 + rcChatArea.Width()-22-58-20 + 5,rcChatArea.bottom-24);

  ////短语按钮
  //pControlUI=GetControlByName(szButtonChatShortControlName);
  //if(pControlUI) pControlUI->SetPos(rcChatArea.left+22 + rcChatArea.Width()-22-58-20 + 5 + 25 + 5,rcChatArea.bottom-24);
  ...
  //{
  //  //获取对象
  //  pControlUI=GetControlByName(ChatButtonArray[i]);
  //  if(pControlUI==NULL) continue;

  //  //获取位置
  //  CSize SizeChatButton(pControlUI->GetFixedWidth(),pControlUI->GetFixedHeight());

  //  //显示判断
  //  bool bShowButton=true;
  //  nChatButtonSpace=(nWidth-pControlUI->GetFixedWidth()*CountArray(ChatButtonArray)-20)/4;
  //  if ((nXPosChat+SizeChatButton.cx+nChatButtonSpace)>(nWidth-2)) bShowButton=false;

  //  //移动按钮
  //  pControlUI->SetVisible(bShowButton);
  //  pControlUI->SetPos(CRect(nXPosChat,nYPosChat,nXPosChat+SizeChatButton.cx,nYPosChat+SizeChatButton.cy));

  //  //设置间距
  //  nXPosChat+=(SizeChatButton.cx+nChatButtonSpace);
  //}
  ...
  //DeferWindowPos(hDwp,m_GamePropertyCtrl,NULL,0,rcListArea.bottom,nWidth,PROPERTY_CY,uFlags);
...
VOID CGameFrameControl::OnSelectMoreColor()
{
  ...
  {
    pGlobalUnits->m_crChatTX=ColorDialog.GetColor();
  }
...
VOID CGameFrameControl::OnSelectChatColor(UINT nCommandID)
{
  ...
  if(nIndex<CountArray(crColor))
  {
    pGlobalUnits->m_crChatTX=crColor[nIndex];
  }
  ...
  if(pListNotify->iItem!=-1)
  {
    ...
    if(nSelectCount>1L)
    {
      ...
      while((nSelectPos!=NULL)&&(wUserCount<CountArray(pIClientUserItem)))
      {
        ...
        if(lItemData!=NULL)
        {
          pIClientUserItem[wUserCount++]=((IClientUserItem *)(lItemData));
        }
      };
      ...
      if(wUserCount>0)
      {
        TrackUserItemMenu(pIClientUserItem,wUserCount);
      }
    }
    else
    {
      ...
      if(lItemData!=NULL)
      {
        TrackUserItemMenu((IClientUserItem *)(lItemData));
      }
    }
  }
