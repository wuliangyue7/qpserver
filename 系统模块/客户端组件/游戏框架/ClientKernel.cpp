...
bool CClientKernel::InitClientKernel(LPCTSTR lpszComLine)
{
  ...
  if(CommandLine.SearchCommandItem(lpszComLine,TEXT("/Transmittal:"),szTransmittal,CountArray(szTransmittal))==true)
  {
    ...
    if(IsWindow(m_hProcessWnd)==FALSE)
    {
      m_hProcessWnd=NULL;
    }
  }
  ...
  if(m_GameUserManagerModule.CreateInstance()==false)
  {
    return false;
  }
  ...
  //设置组件
  if(m_GameUserManagerModule->SetUserManagerSink(QUERY_ME_INTERFACE(IUnknownEx))==false)
  {
    return false;
  }
  ...
  {
    pVideoServiceManager->SetClientKernel(QUERY_OBJECT_PTR_INTERFACE(this,IUnknownEx));
  }
...
bool CClientKernel::SetStringMessage(IUnknownEx * pIUnknownEx)
{
  ...
  if(pIUnknownEx!=NULL)
  {
    ...
    if(pVideoServiceManager!=NULL)
    {
      pVideoServiceManager->SetMessageProxy(QUERY_OBJECT_PTR_INTERFACE(m_pIStringMessage,IUnknownEx));
    }
  }
  else
  {
...
bool CClientKernel::SetGameAttribute(WORD wKindID, WORD wPlayerCount, DWORD dwClientVersion, HICON hGameIcon, LPCTSTR pszGameName)
{
  ...
  if(VERSION_EFFICACY==TRUE)
  {
    ...
    if(m_GameAttribute.dwClientVersion!=dwProcessVersion)
    {
      throw TEXT("游戏程序版本信息被非法修改");
    }
  }
...
bool CClientKernel::IntermitConnect()
{
  ...
  {
    m_ProcessControlModule->UnInitializeLink();
  }
...
IClientUserItem * CClientKernel::GetTableUserItem(WORD wChariID)
{
  ...
  if(m_GameUserManagerModule.GetInterface()==NULL)
  {
    return NULL;
  }
...
WORD CClientKernel::SwitchViewChairID(WORD wChairID)
{
  ...
  if(m_UserAttribute.dwUserID==0L)
  {
    return INVALID_CHAIR;
  }
...
VOID CClientKernel::KillGameClock(WORD wClockID)
{
  ...
  if((m_wClockID!=0)&&((m_wClockID==wClockID)||(wClockID==0)))
  {
    ...
    if(m_wClockChairID!=INVALID_CHAIR)
    {
      ...
      if(m_pIClientKernelSink!=NULL)
      {
        m_pIClientKernelSink->OnEventGameClockKill(m_wClockChairID);
      }
    }
...
VOID CClientKernel::OnUserItemAcitve(IClientUserItem * pIClientUserItem)
{
  ...
  if((m_UserEventSinkArray.GetCount()==0L)||(pIClientUserItem==NULL))
  {
    return;
  }
  ...
  if(m_pIMySelfUserItem==NULL)
  {
    ...
    if(dwUserID==m_UserAttribute.dwUserID)
    {
    ...
    if(pIClientUserItem->GetUserStatus()!=US_LOOKON)
    {
      ...
      if(pVideoServiceManager!=NULL)
      {
        pVideoServiceManager->LoginVideoSystem();
      }
    }
  }
...
bool CClientKernel::OnProcessMainConfig(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case IPC_SUB_GF_LEVEL_INFO:   //等级信息
    {
      ...
      if((wHeadSize+pLevelInfo->cbItemCount*sizeof(pLevelInfo->LevelItem[0]))!=wDataSize)
      {
        return false;
      }
    ...
    case IPC_SUB_GF_GROWLEVEL_CONFIG: //等级配置
    {
      ...
      if((wHeadSize+pGrowLevlConfig->wLevelCount*sizeof(pGrowLevlConfig->GrowLevelItem[0]))!=wDataSize)
      {
        return false;
      }
    ...
    case IPC_SUB_GF_COLUMN_INFO:  //列表信息
    {
      ...
      if((wHeadSize+pColumnInfo->cbColumnCount*sizeof(pColumnInfo->ColumnItem[0]))!=wDataSize)
      {
        return false;
      }
      ...
      for(BYTE i=0; i<pColumnInfo->cbColumnCount; i++)
      {
        ...
        //排除过虑
        if(pColumnInfo->ColumnItem[i].cbDataDescribe==UD_TABLE)
        {
          continue;
        }
        if(pColumnInfo->ColumnItem[i].cbDataDescribe==UD_CHAIR)
        {
          continue;
        }
        if(pColumnInfo->ColumnItem[i].cbDataDescribe==UD_NOTE_INFO)
        {
          continue;
        }
      ...
      if(m_pIClientKernelSink!=NULL)
      {
        m_pIClientKernelSink->SchemeColumnItem(ColumnItem,cbColumnCount);
      }
    ...
    case IPC_SUB_GF_SERVER_INFO:  //房间配置
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_ServerInfo))
      {
        return false;
      }
      ...
      if(pVideoServiceManager!=NULL)
      {
        pVideoServiceManager->SetVideoServiceInfo(pServerInfo->dwUserID,(pServerInfo->dwAVServerAddr),pServerInfo->wAVServerPort);
      }
    ...
    case IPC_SUB_GF_MATCH_INFO:   //比赛信息
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_MatchInfo))
      {
        return false;
      }
    ...
    case IPC_SUB_GF_PROPERTY_INFO:  //道具信息
    {
      ...
      if((wHeadSize+pPropertyInfo->cbPropertyCount*sizeof(pPropertyInfo->PropertyInfo[0]))!=wDataSize)
      {
        return false;
      }
      ...
      if(m_pIClientKernelSink!=NULL)
      {
        m_pIClientKernelSink->SchemePropertyInfo(PropertyInfo,cbPropertyCount);
      }
    ...
    case IPC_SUB_GF_CONFIG_FINISH:  //配置完成
    {
      ...
      if(m_pIClientKernelSink!=NULL)
      {
        m_pIClientKernelSink->OnSchemeFinish();
      }
    ...
    case IPC_SUB_GF_USER_RIGHT: //玩家权限
    {
      ...
      if(wDataSize<sizeof(IPC_GF_UserRight))
      {
        return false;
      }
...
bool CClientKernel::OnProcessMainUserInfo(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case IPC_SUB_GF_USER_ENTER:   //用户进入
    {
      ...
      if(wDataSize<sizeof(IPC_GF_UserInfo))
      {
        return false;
      }
    ...
    case IPC_SUB_GF_USER_SCORE:   //用户积分
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_UserScore))
      {
        return false;
      }
    ...
    case IPC_SUB_GF_USER_STATUS:  //用户状态
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_UserStatus))
      {
        return false;
      }
    ...
    case IPC_SUB_GF_CUSTOM_FACE:  //自定头像
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_CustomFace))
      {
        return false;
      }
...
bool CClientKernel::OnProcessMainPropertyInfo(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case IPC_SUB_GF_PROPERTY_SUCCESS: //道具成功
    {
      ...
      if(wDataSize!=sizeof(IPC_GR_PropertySuccess))
      {
        return false;
      }
    ...
    case IPC_SUB_GF_PROPERTY_FAILURE: //道具失败
    {
      ...
      if(wDataSize!=sizeof(IPC_GR_PropertyFailure))
      {
        return false;
      }
    ...
    case IPC_SUB_GR_PROPERTY_MESSAGE:   //道具消息
    {
      ...
      if(wDataSize!=sizeof(IPC_GR_PropertyMessage))
      {
        return false;
      }
...
bool CClientKernel::OnSocketSubUserChat(VOID * pData, WORD wDataSize)
{
  ...
  //效验参数
  if(wDataSize<(sizeof(CMD_GF_S_UserChat)-sizeof(pUserChat->szChatString)))
  {
    return false;
  }
  if(wDataSize!=(sizeof(CMD_GF_S_UserChat)-sizeof(pUserChat->szChatString)+pUserChat->wChatLength*sizeof(pUserChat->szChatString[0])))
  {
    return false;
  }
...
bool CClientKernel::OnSocketSubExpression(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GF_S_UserExpression))
  {
    return false;
  }
...
bool CClientKernel::OnSocketSubLookonStatus(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GF_LookonStatus))
  {
    return false;
  }
...
bool CClientKernel::OnSocketSubActionMessage(VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<=wHeadSize)||(wDataSize<(wHeadSize+pActionMessage->wLength*sizeof(TCHAR))))
  {
    return false;
  }
...
VOID CClientKernel::SetGameFrameTitle()
{
  ...
  {
    pMainWnd->SetWindowText(szTitle);
  }
}
