...
CGameFrameView::CGameFrameView()
{
  ...
  if(m_pGameFrameView==NULL)
  {
    m_pGameFrameView=this;
  }
  m_wRollTextCount=0;
...
VOID CGameFrameView::SetClientUserItem(WORD wChairID, IClientUserItem * pIClientUserItem)
{
  ...
  if(wChairID>=CountArray(m_pIClientUserItem))
  {
    return;
  }
...
bool CGameFrameView::PlayGameSound(LPCTSTR pszSoundName)
{
  ...
  {
    pGlobalUnits->PlayGameSound(pszSoundName);
  }
...
bool CGameFrameView::PlayGameSound(HINSTANCE hInstance, LPCTSTR pszSoundName)
{
  ...
  if(pGlobalUnits!=NULL)
  {
    pGlobalUnits->PlayGameSound(hInstance,pszSoundName);
  }
...
LRESULT CGameFrameView::SendEngineMessage(UINT uMessage, WPARAM wParam, LPARAM lParam)
{
  ...
  if(IsWindow(pGameFrameEngine->GetSafeHwnd())==FALSE)
  {
    return 0L;
  }
...
VOID CGameFrameView::OnRButtonUp(UINT nFlags, CPoint Point)
{
  ...
  for(WORD i=0; i<MAX_CHAIR; i++)
  {
    ...
    {
      //位置判断
      if(Point.x<=m_ptAvatar[i].x)
      {
        continue;
      }
      if(Point.y<=m_ptAvatar[i].y)
      {
        continue;
      }
      if(Point.x>=(m_ptAvatar[i].x+FACE_CX))
      {
        continue;
      }
      if(Point.y>=(m_ptAvatar[i].y+FACE_CY))
      {
        continue;
      }
      ...
      if(pIGameFrameControl!=NULL)
      {
        pIGameFrameControl->TrackUserItemMenu(m_pIClientUserItem[i]);
      }
...
bool CGameFrameView::OnEventInsureMessage(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  switch(wSubCmdID)
  {
    ...
    case SUB_GR_USER_INSURE_FAILURE:
    {
      ...
      if(wDataSize<(sizeof(CMD_GR_S_UserInsureFailure)-sizeof(pUserInsureFailure->szDescribeString)))
      {
        return false;
      }
...
void CGameFrameView::ShowInsureSave(IClientKernel * pIClientKernel, CPoint pos)
{
  ...
  if(NULL==m_DlgInsureGame.m_hWnd)
  {
    m_DlgInsureGame.Create(IDD_BANK_STORAGE,this);
  }
