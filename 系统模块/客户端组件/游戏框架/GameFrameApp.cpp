...
BOOL CGameFrameApp::InitInstance()
{
  ...
  try
  {
    ...
    if(m_pGameFrameWnd==NULL)
    {
      throw TEXT("游戏框架窗口创建失败");
    }
    ...
    if((pIClientKernel!=NULL)&&(pIClientKernel->IsSingleMode()==false))
    {
      pIClientKernel->CreateConnect();
    }
...
BOOL CGameFrameApp::ExitInstance()
{
  ...
  {
    pVideoServiceManager->LogoutVideoSystem();
  }
  ...
  if((pIClientKernel!=NULL)&&(pIClientKernel->IsSingleMode()==false))
  {
    pIClientKernel->IntermitConnect();
  }
...
int CGameFrameApp::Run()
{
  ...
  for(;;)
  {
    while(bIdle && !::PeekMessage(&(pState->m_msgCur), NULL, NULL, NULL, PM_NOREMOVE))
    {
      ...
      if(pIGameFrameView)
      {
        pIGameFrameView->RenderGameView();
      }
    }
