...
CGameFrameAvatar::CGameFrameAvatar()
{
  ...
  if(m_pGameFrameAvatar==NULL)
  {
    m_pGameFrameAvatar=this;
  }
...
CGameFrameAvatar::~CGameFrameAvatar()
{
  ...
  if(m_pGameFrameAvatar==this)
  {
    m_pGameFrameAvatar=NULL;
  }

  //销毁资源
  if(m_D3DTextureAvatar.IsNull()==false)
  {
    m_D3DTextureAvatar.Destory();
  }
...
VOID CGameFrameAvatar::DrawUserAvatar(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight, IClientUserItem * pIClientUserItem)
{
  ...
  else
  {
    ...
    if(wFaceID>=pIFaceItemControl->GetFaceCount())
    {
      wFaceID=0;
    }
...
VOID CGameFrameAvatar::DrawUserAvatar(CD3DDevice * pD3DDevice, INT nXPos, INT nYPos, INT nWidth, INT nHeight, WORD wFaceID)
{
  ...
  if(m_D3DTextureAvatar.IsNull()==true)
  {
    return;
  }

  //调整参数
  if(wFaceID>=m_wD3DFullCount)
  {
    wFaceID=0;
  }
