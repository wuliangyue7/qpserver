...
VOID CGameFrameService::ResetGameClient()
{
  ...
  {
    m_pGameFrameView->SwitchToReadyStatus();
  }
  ...
  {
    m_pGameFrameEngine->ResetGameFrameEngine();
  }
...
bool CGameFrameService::OnEventSocket(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  if(wMainCmdID==MDM_GR_INSURE)
  {
    ...
    if(m_pGameFrameView)
    {
      m_pGameFrameView->OnEventInsureMessage(wSubCmdID,pData,wDataSize);
    }
  ...
  if(wMainCmdID==MDM_GF_FRAME)
  {
    switch(wSubCmdID)
    {
      ...
      case SUB_GR_MATCH_INFO:       //比赛信息
      {
        ...
        if(wDataSize!=sizeof(CMD_GR_Match_Info))
        {
          return false;
        }
      ...
      case SUB_GR_MATCH_WAIT_TIP:     //等待提示
      {
        ...
        if(wDataSize!=sizeof(CMD_GR_Match_Wait_Tip) && wDataSize!=0)
        {
          return false;
        }
      ...
      case SUB_GR_MATCH_RESULT:     //比赛结果
      {
        ...
        if(wDataSize!=sizeof(CMD_GR_MatchResult))
        {
          return false;
        }
...
bool CGameFrameService::OnEventGameClockInfo(WORD wChairID, UINT nElapse, WORD wClockID)
{
  ...
  if(m_pGameFrameView!=NULL)
  {
    m_pGameFrameView->SetUserClock(wViewChairID,nElapse);
  }
...
bool CGameFrameService::CreateGameFrameService()
{
  ...
  if((CGameFrameView::GetInstance()==NULL)||(CGameFrameEngine::GetInstance()==NULL))
  {
    return false;
  }
