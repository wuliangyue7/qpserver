...
CMissionAndroid::CMissionAndroid()
{
  ...
  if(m_pMissionAndroid==NULL)
  {
    m_pMissionAndroid=this;
  }
...
CMissionAndroid::~CMissionAndroid()
{
  ...
  if(m_pMissionAndroid==this)
  {
    m_pMissionAndroid=NULL;
  }
  ...
  if(Command.wMainCmdID==MDM_GP_ANDROID_SERVICE)
  {
    switch(Command.wSubCmdID)
    {
      case SUB_GP_ANDROID_PARAMETER:    //机器参数
      {
        ...
        if(wDataSize>sizeof(CMD_GP_AndroidParameter))
        {
          return false;
        }
      ...
      case SUB_GP_OPERATE_FAILURE:  //操作失败
      {
        ...
        if(sizeof(CMD_GP_OperateFailure)<wDataSize)
        {
          return false;
        }
