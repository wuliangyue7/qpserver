...
WORD CDlgSkinControl::GetCurrentItem(CPoint MousePoint)
{
  ...
  for(WORD nXPos=0; nXPos<4; nXPos++)
  {
    ...
    {
      //位置判断
      if(MousePoint.x<nXPos*(m_SizeSkinItem.cx+ITEM_SPACE_CX)+ITEM_ORIGIN_X)
      {
        continue;
      }
      if(MousePoint.y<nYPos*(m_SizeSkinItem.cy+ITEM_SPACE_CY)+ITEM_ORIGIN_Y)
      {
        continue;
      }
      if(MousePoint.x>nXPos*(m_SizeSkinItem.cx+ITEM_SPACE_CX)+ITEM_ORIGIN_X+m_SizeSkinItem.cx)
      {
        continue;
      }
      if(MousePoint.y>nYPos*(m_SizeSkinItem.cy+ITEM_SPACE_CY)+ITEM_ORIGIN_Y+m_SizeSkinItem.cy)
      {
        continue;
      }
...
WORD CDlgSkinControl::GetCurrentColor(CPoint MousePoint)
{
  ...
  {
    //位置判断
    if(MousePoint.x<COLOR_ORIGIN_X)
    {
      continue;
    }
    if(MousePoint.x>(COLOR_ORIGIN_X+m_SizeColorItem.cx))
    {
      continue;
    }
    if(MousePoint.y<(COLOR_ORIGIN_Y+(m_SizeColorItem.cy+COLOR_SPACE_CY)*nIndex))
    {
      continue;
    }
    if(MousePoint.y>(COLOR_ORIGIN_Y+(m_SizeColorItem.cy+COLOR_SPACE_CY)*nIndex+m_SizeColorItem.cy))
    {
      continue;
    }
...
VOID CDlgSkinControl::OnMouseMove(UINT nFlags, CPoint Point)
{
  ...
  if(m_wMouseDownColor!=INVALID_WORD)
  {
    ...
    {
      pParameterGlobal->m_SkinRenderInfo=m_CurrentRenderInfo;
    }
    ...
    {
      pSkinResourceManager->SetSkinResource(m_CurrentRenderInfo);
    }
    ...
    {
      pPlatformEvent->PostPlatformEvent(EVENT_SKIN_CONFIG_UPDATE,0L);
    }
