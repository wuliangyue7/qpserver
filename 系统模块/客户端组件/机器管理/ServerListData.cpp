...
CServerListData::~CServerListData()
{
  ...
  if(m_pServerListData==this)
  {
    m_pServerListData=NULL;
  }
...
bool CServerListData::InsertGamePage(tagGamePage * pGamePage)
{
  ...
  {
    //寻找父项
    if(pGamePage->wNodeID!=0)
    {
      pGamePageItem->m_pParentListItem=SearchGameNode(pGamePage->wNodeID);
    }
    else if(pGamePage->wKindID!=0)
    {
      pGamePageItem->m_pParentListItem=SearchGameKind(pGamePage->wKindID);
    }
...
bool CServerListData::DeleteGameType(WORD wTypeID)
{
  //查找种类
  CGameTypeItem * pGameTypeItem=NULL;
  if(m_GameTypeItemMap.Lookup(wTypeID,pGameTypeItem)==FALSE)
  {
    return false;
  }
...
bool CServerListData::DeleteGameKind(WORD wKindID)
{
  //查找类型
  CGameKindItem * pGameKindItem=NULL;
  if(m_GameKindItemMap.Lookup(wKindID,pGameKindItem)==FALSE)
  {
    return false;
  }
...
bool CServerListData::DeleteGameNode(WORD wNodeID)
{
  //查找节点
  CGameNodeItem * pGameNodeItem=NULL;
  if(m_GameNodeItemMap.Lookup(wNodeID,pGameNodeItem)==FALSE)
  {
    return false;
  }
  ...
  if(m_pIServerListDataSink!=NULL)
  {
    m_pIServerListDataSink->OnGameItemDelete(pGameNodeItem);
  }
...
bool CServerListData::DeleteGameServer(WORD wServerID)
{
  //查找房间
  CGameServerItem * pGameServerItem=NULL;
  if(m_GameServerItemMap.Lookup(wServerID,pGameServerItem)==FALSE)
  {
    return false;
  }
...
bool CServerListData::DeleteGamePage(WORD wPageID)
{
  //查找类型
  CGamePageItem * pGamePageItem=NULL;
  if(m_GamePageItemMap.Lookup(wPageID,pGamePageItem)==FALSE)
  {
    return false;
  }
  ...
  if(m_pIServerListDataSink!=NULL)
  {
    m_pIServerListDataSink->OnGameItemDelete(pGamePageItem);
  }
...
CGameTypeItem * CServerListData::EmunGameTypeItem(POSITION & Position)
{
  ...
  if(Position==NULL)
  {
    Position=m_GameTypeItemMap.GetStartPosition();
  }
  if(Position==NULL)
  {
    return NULL;
