...
BOOL CWndChatControl::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  if((LOWORD(wParam)>=IDM_HISTORY_CHAT_USER)&&(LOWORD(wParam)<(IDM_HISTORY_CHAT_USER+MAX_HISTOY_USER)))
  {
    ...
    if(nIndex<m_ChatUserItemArray.GetCount())
    {
      SetChatTargetUser(m_ChatUserItemArray[nIndex]);
    }
...
VOID CWndChatControl::OnPaint()
{
  ...
  {
    pSkinRenderManager->RenderImage(ImageBackGroud);
  }
...
VOID CWndChatControl::OnBnClickedClose()
{
  //取消聊天
  ASSERT(m_pIChatControlSink!=NULL);
  if(m_pIChatControlSink!=NULL)
  {
    m_pIChatControlSink->OnChangeChatTarget(NULL);
  }
...
VOID CWndChatControl::OnBnClickedControl()
{
  ...
  if((m_pIClientUserItem==NULL)||(m_ChatUserItemArray.GetCount()==0))
  {
    return;
  }
  ...
  if(m_ChatUserItemArray.GetCount()>1)
  {
    for(INT_PTR i=0; i<m_ChatUserItemArray.GetCount(); i++)
    {
      ...
      if(pIClientUserItem->GetUserID()==m_pIClientUserItem->GetUserID())
      {
        continue;
      }
  ...
  {
    ChatUserMenu.AppendMenu(0,TEXT("没有历史聊天对象"),MF_GRAYED);
  }
