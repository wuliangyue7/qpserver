...
OLECONTAINER_DEF::~COleContainerWnd()
{
  if(m_pIControl)
  {
    ...
    if(m_pIConnectionPoint)
    {
      if(m_dwConPointID)
      {
        m_pIConnectionPoint->Unadvise(m_dwConPointID);
      }
...
OLECONTAINER_FUNCTION(BOOL)::Create(GUID clsid, DWORD dwExStyle, DWORD dwStyle, HWND hWndParent, HINSTANCE hInstance)
{
  ...
  if(FAILED(m_pIOleObject->QueryInterface(__uuidof(TControl), (VOID **)&m_pIControl)))
  {
    return FALSE;
  }
  ...
  if(m_bTransparent)
  {
    ...
    {
      if(FAILED(m_pIOleObject->QueryInterface(IID_IOleInPlaceObject, (VOID **)&m_pIOleInPlaceObject)))
      {
        return FALSE;
      }
      m_bTransparent=FALSE;
  ...
  if(FAILED(m_pIOleObject->DoVerb(OLEIVERB_SHOW, NULL, (IOleClientSite *)this, 0, NULL, NULL)))
  {
    return FALSE;
  }
...
BOOL CFlashWnd::OnBeforeShowingContent()
{
  ...
  if(FAILED(m_pIControl->QueryInterface(IID_IConnectionPointContainer, (VOID**)&m_pIConnectionPointContainer)))
  {
    return FALSE;
  }
  if(FAILED(m_pIConnectionPointContainer->FindConnectionPoint(ShockwaveFlashObjects::DIID__IShockwaveFlashEvents, &m_pIConnectionPoint)))
  {
    return FALSE;
  }
  if(FAILED(m_pIConnectionPoint->Advise((ShockwaveFlashObjects::_IShockwaveFlashEvents *)this, &m_dwConPointID)))
  {
    return FALSE;
  }
  ...
  if(m_bTransparent)
  {
    m_pIControl->PutWMode(L"transparent");
  }
...
BOOL CFlashWnd::OnAfterShowingContent()
{
  ...
  if(m_szFile[0]!=0)
  {
    if(FAILED(m_pIControl->LoadMovie(0, m_szFile)))
    {
      return FALSE;
    }
    if(FAILED(m_pIControl->Play()))
    {
      return FALSE;
    }
    try
