...
VOID CPasswordControl::OnEnChangePassword()
{
  ...
  if(pParent!=NULL)
  {
    pParent->SendMessage(WM_COMMAND,MAKELONG(GetWindowLong(m_hWnd,GWL_ID),EN_CHANGE),(LPARAM)m_hWnd);
  }
...
VOID CPasswordKeyboard::SetCurrentStation(CPoint MousePoint)
{
  ...
  if((MousePoint.x>=ITEM_POS_X)&&(MousePoint.y>=ITEM_POS_Y))
  {
    ...
    //参数调整
    if(wHoverLine>=CountArray(m_nItemCount))
    {
      wHoverLine=INVALID_WORD;
    }
    if((wHoverLine==INVALID_WORD)||(wHoverRow>=m_nItemCount[wHoverLine]))
    {
      wHoverRow=INVALID_WORD;
    }
  }
...
WORD CPasswordKeyboard::GetVirualKeyCode(WORD wHoverLine, WORD wHoverRow)
{
  ...
  if((wHoverLine<CountArray(m_nItemCount))&&(wHoverRow<m_nItemCount[wHoverLine]))
  {
    ...
    if(m_bShiftStatus==true)
    {
      bLowerChar=!bLowerChar;
    }
    if((wHoverLine>=2)&&(m_bCapsLockStatus==true))
    {
      bLowerChar=!bLowerChar;
    }
...
BOOL CPasswordKeyboard::OnEraseBkgnd(CDC * pDC)
{
  ...
  for(INT nLine=0; nLine<CountArray(m_nItemCount); nLine++)
  {
    ...
    for(INT nRow=0; nRow<m_nItemCount[nLine]; nRow++)
    {
      ...
      if((m_wHoverLine==nLine)&&(m_wHoverRow==nRow))
      {
        nXImageIndex=(m_bMouseDown==false)?1:2;
      }
      ...
      if(m_bShiftStatus==true)
      {
        nColorIndex=(nColorIndex+1)%CountArray(crTextColor);
      }
      if((m_bCapsLockStatus==true)&&(nLine>=2))
      {
        nColorIndex=(nColorIndex+1)%CountArray(crTextColor);
      }
  ...
  //退格按钮
  {
    //资源位置
    INT nXImageIndex=0;
    if((m_wHoverLine==LINE_FUNCTION)&&(m_wHoverRow==ROW_BACK))
    {
      nXImageIndex=(m_bMouseDown==false)?1:2;
    }
  ...
  //切换按钮
  {
    ...
    if((m_wHoverLine==LINE_FUNCTION)&&(m_wHoverRow==ROW_SHIFT))
    {
      nXImageIndex=(m_bMouseDown==false)?1:2;
    }
  ...
  //大写按钮
  {
    ...
    if((m_wHoverLine==LINE_FUNCTION)&&(m_wHoverRow==ROW_CAPITAL))
    {
      nXImageIndex=(m_bMouseDown==false)?1:2;
    }
    ...
    if((m_wHoverRow==wHoverRow)&&(m_wHoverLine==wHoverLine))
    {
      ...
      switch(wViraulCode)
      {
        ...
        default:      //默认按钮
        {
          ...
          if(pPasswordControl!=NULL)
          {
            pPasswordControl->m_edPassword.SendMessage(WM_CHAR,wViraulCode,0L);
          }
