...
CUserItemElement::CUserItemElement()
{
  ...
  if(m_pUserItemElement==NULL)
  {
    m_pUserItemElement=this;
  }
...
VOID CUserItemElement::SetGrowLevelConfig(WORD wLevelCount,tagGrowLevelConfig * pGrowLevelConfig)
{
  ...
  if(wLevelCount==0 || pGrowLevelConfig==NULL)
  {
    return;
  }
...
WORD CUserItemElement::GetExperienceLevel(DWORD dwExperience)
{
  if(dwExperience>=0L && m_pGrowLevelConfig!=NULL)
  {
    ...
    {
      if(dwExperience<m_pGrowLevelConfig[wLevelIndex].dwExperience)
      {
        break;
      }
...
bool CUserItemElement::EfficacyChineseCharacter(TCHAR chr)
{
  ...
  if((chr&0xff00) == 0x2000 && ::IsDBCSLeadByte(BYTE(chr&0xFF)) == FALSE)
  {
    return false;
  }
  if((chr&0xff00) == 0x3000 && ::IsDBCSLeadByte(BYTE(chr&0xFF)) == FALSE)
  {
    return false;
  }
...
bool CUserItemElement::EfficacyAccounts(LPCTSTR pszAccounts, TCHAR szDescribe[], WORD wMaxCount)
{
  ...
  for(INT i=0; i<nCharLength; i++)
  {
    ...
    {
      //特殊字符
      if(pszAccounts[i]==szSpecialChar)
      {
        continue;
      }

      //数字字符
      if(pszAccounts[i]>=szNumber[0] && pszAccounts[i]<=szNumber[1])
      {
        continue;
      }

      //小写字母
      if(pszAccounts[i]>=szLowerLetter[0] && pszAccounts[i]<=szLowerLetter[1])
      {
        continue;
      }

      //大写字母
      if(pszAccounts[i]>=szUpperLetter[0] && pszAccounts[i]<=szUpperLetter[1])
      {
        continue;
      }
...
bool CUserItemElement::EfficacyPassword(LPCTSTR pszPassword, TCHAR szDescribe[], WORD wMaxCount)
{
  ...
  for(INT i=0; i<nCharLength; i++)
  {
    ...
    //特殊字符
    if(pszPassword[i]==szSpecialChar)
    {
      continue;
    }

    //数字字符
    if(pszPassword[i]>=szNumber[0] && pszPassword[i]<=szNumber[1])
    {
      continue;
    }

    //小写字母
    if(pszPassword[i]>=szLowerLetter[0] && pszPassword[i]<=szLowerLetter[1])
    {
      continue;
    }

    //大写字母
    if(pszPassword[i]>=szUpperLetter[0] && pszPassword[i]<=szUpperLetter[1])
    {
      continue;
    }
