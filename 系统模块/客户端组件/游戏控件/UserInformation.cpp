...
CUserInformation::CUserInformation()
{
  ...
  if(m_pUserInformation==NULL)
  {
    m_pUserInformation=this;
  }
...
bool CUserInformation::LoadInformation()
{
  ...
  try
  {
    ...
    if(pIEnumSTATSTG==NULL)
    {
      throw TEXT("EnumElements Create pIEnumSTATSTG Error");
    }
    ...
    while(pIEnumSTATSTG->Next(1,&Statstg,NULL)==NOERROR)
    {
      ...
      if(Statstg.type==STGTY_STORAGE)
      {
        ...
        if(pIStorageSub==NULL)
        {
          throw TEXT("CreateStorage Create pIStorageSub Error");
        }
        ...
        if(pIStreamSub!=NULL)
        {
          ...
          {
            throw TEXT("Read Data CompanionInfo Error");
          }
          ...
          {
            throw TEXT("CreateCompanionItem Error");
          }
...
tagCompanionInfo * CUserInformation::SearchCompanionInfo(DWORD dwUserID)
{
  ...
  {
    tagCompanionInfo * pCompanionInfo=m_CompanionInfoActive[i];
    if(pCompanionInfo->dwUserID==dwUserID)
    {
      return pCompanionInfo;
    }
  }
...
tagCompanionInfo * CUserInformation::InsertCompanionInfo(IClientUserItem * pIClientUserItem, BYTE cbCompanion)
{
  ...
  if(pCompanionInfo==NULL)
  {
    ...
    if(m_pIUserCompanionSink!=NULL)
    {
      m_pIUserCompanionSink->OnCompanionInsert(pCompanionInfo);
    }
  }
  else
  {
    //修改判断
    bool bModify=false;
    if(pCompanionInfo->cbCompanion!=cbCompanion)
    {
      bModify=true;
    }
    if((bModify==false)&&(lstrcmp(pCompanionInfo->szNickName,pIClientUserItem->GetNickName())!=0))
    {
      bModify=true;
    }
    if((bModify==false)&&(lstrcmp(pCompanionInfo->szUserNote,pIClientUserItem->GetUserNoteInfo())!=0))
    {
      bModify=true;
    }
    ...
    if(m_pIUserCompanionSink!=NULL)
    {
      m_pIUserCompanionSink->OnCompanionUpdate(pCompanionInfo);
    }
  }
  ...
  try
  {
    ...
    if(pIStorageRoot==NULL)
    {
      StgCreateDocfile(szInfomationFile,STGM_CREATE|STGM_READWRITE|STGM_SHARE_EXCLUSIVE,0,&pIStorageRoot);
    }

    //错误判断
    if(pIStorageRoot==NULL)
    {
      throw TEXT("StgCreateDocfile Create pIStorageRoot Error");
    }
    ...
    if(pIStorageSub==NULL)
    {
      pIStorageRoot->CreateStorage(szStorageName,STGM_CREATE|STGM_WRITE|STGM_SHARE_EXCLUSIVE,0,0,&pIStorageSub);
    }
    ...
    if(pIStreamSub==NULL)
    {
      pIStorageSub->CreateStream(STREAM_COMPANION,STGM_CREATE|STGM_WRITE|STGM_SHARE_EXCLUSIVE,0,0,&pIStreamSub);
    }

    //错误处理
    if(pIStreamSub==NULL)
    {
      throw TEXT("CreateStream Create pIStreamSub Error");
    }
