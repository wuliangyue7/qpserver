...
bool CMissionSocket::PerformConnect(bool bContinue)
{
  ...
  if(GetSocketStatus()!=SOCKET_STATUS_IDLE)
  {
    return false;
  }
  ...
  if(m_TCPSocketModule.GetInterface()==NULL)
  {
    ...
    if(m_TCPSocketModule->SetTCPSocketSink(QUERY_ME_INTERFACE(IUnknownEx))==false)
    {
      return false;
    }
  }
...
bool CMissionSocket::PerformSendData(WORD wMainCmdID, WORD wSubCmdID)
{
  ...
  if(GetSocketStatus()!=SOCKET_STATUS_CONNECT)
  {
    return false;
  }
  ...
  if(m_TCPSocketModule.GetInterface()!=NULL)
  {
    m_TCPSocketModule->SendData(wMainCmdID,wSubCmdID);
  }
...
bool CMissionSocket::PerformSendData(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
  ...
  if(m_TCPSocketModule.GetInterface()!=NULL)
  {
    m_TCPSocketModule->SendData(wMainCmdID,wSubCmdID,pData,wDataSize);
  }
...
bool CMissionSocket::SwitchServerItem()
{
  do
  {
    ...
    //结果判断
    if((m_szCurrentServer[0]==0)&&(m_bSwitchDns==true))
    {
      break;
    }

    //相同判断
    if((m_bSwitchDns==false)&&(lstrcmp(m_szCurrentServer,m_szCustomServer)==0))
    {
      continue;
    }
