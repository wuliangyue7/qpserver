...
INT CUserListControl::GetInsertIndex(VOID * pItemData)
{
  ...
  for(INT i=0; i<nItemCount; i++)
  {
    ...
    //自己判断
    if(pIUserItemTemp->GetUserID()==m_dwMySelfUserID)
    {
      continue;
    }

    //对比等级
    if(nImportOrder>pIUserItemTemp->GetImportOrder())
    {
      return i;
    }
  }
...
INT CUserListControl::SortItemData(LPARAM lParam1, LPARAM lParam2, WORD wColumnIndex, bool bAscendSort)
{
  ...
  //自己置顶
  if(pIClientUserItem1->GetUserID()==m_dwMySelfUserID)
  {
    return SORT_FRONT;
  }
  if(pIClientUserItem2->GetUserID()==m_dwMySelfUserID)
  {
    return SORT_AFTER;
  }
...
VOID CUserListControl::ConstructString(VOID * pItemData, WORD wColumnIndex, LPTSTR pszString, WORD wMaxCount)
{
  ...
  if(wColumnIndex>=m_wColumnCount)
  {
    return;
  }
  ...
  switch(m_cbDataDescribe[wColumnIndex])
  {
    ...
    case UD_TABLE:      //桌子号码
    {
      ...
      {
        _sntprintf(pszString,wMaxCount,TEXT("%ld"),wTableID+1);
      }
      break;
    }
    case UD_CHAIR:      //椅子号码
    {
      ...
      if(wChairID!=INVALID_TABLE)
      {
        _sntprintf(pszString,wMaxCount,TEXT("%ld"),wChairID+1);
      }
      break;
    }
    case UD_SCORE:      //用户积分
...
VOID CUserListControl::DrawCustomItem(CDC * pDC, LPDRAWITEMSTRUCT lpDrawItemStruct, CRect&rcSubItem, INT nColumnIndex)
{
  ...
  if(nColumnIndex>=m_wColumnCount)
  {
    return;
  }
  ...
  switch(m_cbDataDescribe[nColumnIndex])
  {
    case UD_IMAGE_FLAG:   //用户标志
    {
      ...
      //索引计算
      if((cbCompanion==CP_FRIEND)&&(pIClientUserItem->GetUserID()!=m_dwMySelfUserID))
      {
        uImageIndex=INDEX_FRIEND;
      }
      if((cbCompanion==CP_DETEST)&&(pIClientUserItem->GetUserID()!=m_dwMySelfUserID))
      {
        uImageIndex=INDEX_FRIEND+1;
      }
      if((uImageIndex==INDEX_NORMAL)&&(pIClientUserItem->GetMasterOrder()>0))
      {
        uImageIndex=INDEX_MASTER+(pUserInfo->cbMasterOrder-1);
      }
      if((uImageIndex==INDEX_NORMAL)&&(pIClientUserItem->GetMemberOrder()>0))
      {
