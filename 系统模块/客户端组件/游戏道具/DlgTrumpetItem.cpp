...
{
  //销毁窗口
  if(m_pExpressionControl && IsWindow(m_pExpressionControl->m_hWnd))
  {
...
void  CDlgTrumpetItem::UpdateControlSurface()
{
  ...
  if(pButtonTrumpet->GetCheck()==true)
  {
    ...
    if(m_pIClientUserItem->GetMemberOrder()>0)
    {
      lPrice=lPrice*wDiscount/100;
    }
...
VOID CDlgTrumpetItem::OnOK()
{
  ...
  if(m_pGameTrumpetSink != NULL)
  {
    m_pGameTrumpetSink->OnEventUseProperty(wPropertyIndex);
  }

  if(m_bYinX)
  {
    int nRunCount = GetDlgItemInt(IDC_EDIT_RUN_COUNT);
    if(nRunCount<5)
    {
      nRunCount=5;
    }
    //  CString str;
    //str.Format(TEXT("%d,%d"),nRunCount,nRunCount);AfxMessageBox(str);
    SetTimer(100,nRunCount*1000*60+2000,NULL);
    ShowWindow(SW_HIDE);
    return ;
  }
  //关闭窗口
...
VOID CDlgTrumpetItem::OnTimer(UINT nIDEvent)
{
  if(nIDEvent==100)
  {
    //KillTimer(100);
...
{
  OnCancel();
  if(m_pGameTrumpetSink)
  {
    m_pGameTrumpetSink->PerformShowBag();
  }
}
...
void CDlgTrumpetItem::OnBnClickedShop()
{
  ...
  if(m_pGameTrumpetSink)
  {
    m_pGameTrumpetSink->PerformShowShop();
  }
}
...
void CDlgTrumpetItem::ShowTrumpetWindow(CWnd * pParentWnd,IClientUserItem  * pIClientUserItem)
{
  ...
  DWORD dwGameID=pIClientUserItem->GetGameID();
  if(((dwGameID%111000)%111==0)||((dwGameID%100000)%11111==0)||((dwGameID%110000)%1111==0)||((dwGameID%111100)%11==0)/*||((dwGameID%111110)%1==0)*/)
  {
    m_bYinX=true;
    ((CEdit *)GetDlgItem(IDC_EDIT_RUN_COUNT))->ShowWindow(SW_SHOW);
    GetDlgItem(IDC_STATIC_RUN_TIP1)->ShowWindow(SW_SHOW);
    GetDlgItem(IDC_STATIC_RUN_TIP2)->ShowWindow(SW_SHOW);
  }
  //设置变量
  m_strInuptChat=TEXT("");
  UpdateData(FALSE);
