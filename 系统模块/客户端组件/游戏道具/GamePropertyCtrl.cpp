...
bool CGamePropertyCtrl::SetGamePropertyInfo(tagPropertyInfo PropertyInfo[], BYTE cbPropertyCount, WORD wIssueArea)
{
  ...
  for(BYTE i=0; i<cbPropertyCount; i++)
  {
    if((PropertyInfo[i].wUseArea&PT_ISSUE_AREA_SERVER)!=0)
    {
      ...
      if(m_cbPropertyCount>=CountArray(m_pGamePropertyItem))
      {
        break;
      }
      ...
      if(m_cbPresentCount>=CountArray(m_pGamePreSentItem))
      {
        break;
      }
...
WORD CGamePropertyCtrl::SwitchItemFromPoint(CPoint MousePoint)
{
  ...
  //范围判断
  if((MousePoint.x<nItemStart)||(MousePoint.x>=(nItemStart+nItemCount*SizeExcursion.cx)))
  {
    return INVALID_WORD;
  }
  if((MousePoint.y<PointExcursion.y)||(MousePoint.y>(PointExcursion.y+2L*SizeExcursion.cy)))
  {
    return INVALID_WORD;
  }

  //溢出判断
  if(((MousePoint.x-nItemStart-PointExcursion.x)%SizeExcursion.cx)>=PROPERTY_IMAGE_CX)
  {
    return INVALID_WORD;
  }
  if(((MousePoint.y-PointExcursion.y)%SizeExcursion.cy)>=PROPERTY_IMAGE_CY)
  {
    return INVALID_WORD;
  }
  ...
  if(m_cbCurrType==PT_TYPE_PROPERTY && wSwitchIndex>=m_cbPropertyCount)
  {
    return INVALID_WORD;
  }
  if(m_cbCurrType==PT_TYPE_PRESENT && wSwitchIndex>=m_cbPresentCount)
  {
    return INVALID_WORD;
  }
...
void CGamePropertyCtrl::OnBeginPaintWindow(HDC hDC)
{
  ...
  for(BYTE i=0; i<m_cbPropertyCount; i++)
  {
    ...
    if(ImageProperty.IsNull()==false)
    {
      ...
      //计算位置
      INT nImagePos=0;
      if(m_wDownItem==cbIndex && m_cbCurrType==PT_TYPE_PROPERTY)
      {
        nImagePos=SizeProperty.cx;
      }
      if((m_wDownItem!=cbIndex)&&(m_wHoverItem==cbIndex && m_cbCurrType==PT_TYPE_PROPERTY))
      {
        nImagePos=SizeProperty.cx*2;
      }
  ...
  for(BYTE i=0; i<m_cbPresentCount; i++)
  {
    ...
    if(ImageProperty.IsNull()==false)
    {
      ...
      //计算位置
      INT nImagePos=0;
      if(m_wDownItem==cbIndex && m_cbCurrType==PT_TYPE_PRESENT)
      {
        nImagePos=SizeProperty.cx;
      }
      if((m_wDownItem!=cbIndex)&&(m_wHoverItem==cbIndex && m_cbCurrType==PT_TYPE_PRESENT))
      {
        nImagePos=SizeProperty.cx*2;
      }
  ...
  if(m_wHoverItem != INVALID_WORD /*&& nXHoverPos != -1 && nYHoverPos != -1*/)
  {
    ...
    //位置修正
    if(m_wHoverItem <= nItemCount/2)
    {
      nXHoverPos += (PROPERTY_IMAGE_CX+20);
    }
    else
    {
      nXHoverPos -= (PROPERTY_IMAGE_CX+20);
  ...
  if(m_wDownItem!=INVALID_WORD)
  {
    ...
    if(wLastDown==m_wHoverItem && cbLastType==m_cbCurrType)
    {
      ...
      if(m_cbCurrType==PT_TYPE_PROPERTY)
      {
        ...
        if(m_cbPropertyStartIndex+wLastDown>=m_cbPropertyCount)
        {
          return;
        }
      ...
      if(m_cbCurrType==PT_TYPE_PRESENT)
      {
        ...
        if(m_cbPresentStartIndex+wLastDown>=m_cbPresentCount)
        {
          return;
        }
