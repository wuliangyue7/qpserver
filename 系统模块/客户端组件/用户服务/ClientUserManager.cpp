...
LONG CClientUserItem::GetImportOrder()
{
  //构造等级
  INT nOrder=0;
  if(m_cbCompanion==CP_FRIEND)
  {
    nOrder+=1000;
  }
  if(m_UserInfo.cbMemberOrder!=0)
  {
    nOrder+=m_UserInfo.cbMemberOrder*100L;
  }
  if(m_UserInfo.cbMasterOrder!=0)
  {
    nOrder+=m_UserInfo.cbMasterOrder*10000L;
  }
...
float CClientUserItem::GetUserWinRate()
{
  ...
  if(lPlayCount!=0L)
  {
    return (float)(m_UserInfo.dwWinCount*100.0f/(float)lPlayCount);
  }
...
float CClientUserItem::GetUserLostRate()
{
  ...
  if(lPlayCount!=0L)
  {
    return (float)(m_UserInfo.dwLostCount*100.0f/(float)lPlayCount);
  }
...
float CClientUserItem::GetUserDrawRate()
{
  ...
  if(lPlayCount!=0L)
  {
    return (float)(m_UserInfo.dwDrawCount*100.0f/(float)lPlayCount);
  }
...
float CClientUserItem::GetUserFleeRate()
{
  ...
  if(lPlayCount!=0L)
  {
    return (float)(m_UserInfo.dwFleeCount*100.0f/(float)lPlayCount);
  }
...
IClientUserItem * CPlazaUserManager::ActiveUserItem(tagUserInfo & UserInfo,tagCustomFaceInfo & CustomFaceInfo,bool bIsAndroid)
{
  ...
  if(pCompanionInfo!=NULL)
  {
    ...
    if((bSameData==true)&&(lstrcmp(UserInfo.szNickName,pCompanionInfo->szNickName)!=0))
    {
      bSameData=false;
    }
    ...
    if(bSameData==false)
    {
      m_pIUserInformation->InsertCompanionInfo(pClientUserItem,pCompanionInfo->cbCompanion);
    }
  }
...
IClientUserItem * CPlazaUserManager::SearchUserByNickName(LPCTSTR pszNickName)
{
  ...
  for(INT_PTR i=0; i<m_UserItemActive.GetCount(); i++)
  {
    ...
    if(lstrcmp(pszNickName,pszTempNickName)==0)
    {
      return pClientUserItem;
    }
  }
...
CGameUserManager::~CGameUserManager()
{
  ...
  {
    if(m_pTableUserItem[i]!=NULL)
    {
      m_UserItemStorage.Add(m_pTableUserItem[i]);
    }
  }
...
bool CGameUserManager::DeleteUserItem(IClientUserItem * pIClientUserItem)
{
  ...
  if((pIClientUserItem==NULL)||(pIClientUserItem->GetChairID()>=MAX_CHAIR))
  {
    return false;
  }
  ...
  if(pIClientUserItem->GetUserStatus()!=US_LOOKON)
  {
    ...
    if(m_pTableUserItem[pIClientUserItem->GetChairID()]!=pIClientUserItem)
    {
      return false;
    }
...
IClientUserItem * CGameUserManager::ActiveUserItem(tagUserInfo & UserInfo, tagCustomFaceInfo & CustomFaceInfo, BYTE cbCompanion, LPCTSTR pszUserNote,bool bIsAndroid)
{
  ...
  if(UserInfo.wChairID>=MAX_CHAIR)
  {
    return NULL;
  }
  ...
  {
    m_UserItemLookon.Add(pClientUserItem);
  }
  else
  {
...
IClientUserItem * CGameUserManager::GetTableUserItem(WORD wChariID)
{
  ...
  if(wChariID>=CountArray(m_pTableUserItem))
  {
    return NULL;
  }
...
{
  if(wEnumIndex>=m_UserItemLookon.GetCount())
  {
    return NULL;
  }
...
IClientUserItem * CGameUserManager::SearchUserByUserID(DWORD dwUserID)
{
  ...
  for(WORD i=0; i<MAX_CHAIR; i++)
  {
    ...
    if((pClientUserItem!=NULL)&&(pClientUserItem->GetUserID()==dwUserID))
    {
      return pClientUserItem;
    }
  }
  ...
  for(INT_PTR i=0; i<m_UserItemLookon.GetCount(); i++)
  {
    ...
    if(pClientUserItem->GetUserID()==dwUserID)
    {
      return pClientUserItem;
    }
  }
...
IClientUserItem * CGameUserManager::SearchUserByGameID(DWORD dwGameID)
{
  ...
  {
    CClientUserItem * pClientUserItem=m_pTableUserItem[i];
    if((pClientUserItem!=NULL)&&(pClientUserItem->GetGameID()==dwGameID))
    {
      return pClientUserItem;
    }
  }
  ...
  {
    CClientUserItem * pClientUserItem=m_UserItemLookon[i];
    if(pClientUserItem->GetGameID()==dwGameID)
    {
      return pClientUserItem;
    }
  }
