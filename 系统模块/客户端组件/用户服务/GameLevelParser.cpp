...
bool CGameLevelParser::LoadGameLevelItem(LPCTSTR pszGameLevelFileName)
{
  ...
  do
  {
    ...
    if(szReadData[0]!=0)
    {
      ...
      {
        //过虑处理
        if((szReadData[i]==TEXT(','))||(szReadData[i]==0))
        {
          break;
        }
        if((wStringIndex==0)&&(szReadData[i]==TEXT(' ')))
        {
          continue;
        }
      ...
      while(((pszScore[0]>0)&&(pszScore[0]<TEXT('0')))||(pszScore[0]>TEXT('9')))
      {
        pszScore++;
      }
...
WORD CGameLevelParser::GetGameLevelItem(tagLevelItem LevelItem[], WORD wMaxCount)
{
  ...
  if(wMaxCount<m_LevelDescribeActive.GetCount())
  {
    return 0;
  }
...
LPCTSTR CGameLevelParser::GetLevelDescribe(SCORE lScore)
{
  ...
  {
    tagLevelDescribe * pLevelDescribe=m_LevelDescribeActive[i];
    if((lScore>=pLevelDescribe->lLevelScore)||(i==(wItemCount-1)))
    {
      return pLevelDescribe->szLevelName;
    }
  }
...
LPCTSTR CGameLevelParser::GetLevelDescribe(IClientUserItem * pIClientUserItem)
{
  ...
  if(m_LevelDescribeActive.GetCount()==0)
  {
    return GetDefaultLevel(pIClientUserItem->GetUserScore());
  }
  ...
  for(INT_PTR i=0; i<nItemCount; i++)
  {
    ...
    if((lUserScore>=pLevelDescribe->lLevelScore)||(i==(nItemCount-1)))
    {
      return pLevelDescribe->szLevelName;
    }
  }
...
LPCTSTR CGameLevelParser::GetDefaultLevel(SCORE lScore)
{
  ...
  {
    if(lScore>=lLevelScore[CountArray(lLevelScore)-i-1])
    {
