...
CCustomFaceManager::CCustomFaceManager()
{
  ...
  if(m_pCustomFaceManager==NULL)
  {
    m_pCustomFaceManager=this;
  }
...
CCustomFaceManager::~CCustomFaceManager()
{
  ...
  if(m_pCustomFaceManager==this)
  {
    m_pCustomFaceManager=NULL;
  }
...
bool CCustomFaceManager::LoadUserCustomFace(DWORD dwUserID, DWORD dwCustomID, tagCustomFaceInfo & CustomFaceInfo)
{
  ...
  //打开文件
  CFile DataFile;
  if(DataFile.Open(szCustomFacePath,CFile::modeRead)==FALSE)
  {
    return false;
  }
  ...
  if(DataFile.GetLength()!=sizeof(tagCustomFaceFile))
  {
    return false;
  }
  ...
  //读取效验
  if(uReadCount!=sizeof(CustomFaceFile))
  {
    return false;
  }
  if(CustomFaceFile.dwCustomID!=dwCustomID)
  {
    return false;
  }
  if(CustomFaceFile.CustomFaceInfo.dwDataSize!=sizeof(CustomFaceFile.CustomFaceInfo))
  {
    return false;
  }
...
bool CCustomFaceManager::LoadUserCustomFace(DWORD dwUserID, DWORD dwCustomID, tagCustomFaceInfo96 & CustomFaceInfo)
{
  ...
  if(DataFile.GetLength()!=sizeof(tagCustomFaceFile96))
  {
    return false;
  }
...
bool CCustomFaceManager::OnDownLoadError(enDownLoadError DownLoadError)
{
  ...
  if(m_CustomFaceIndexArray.GetCount()==0L)
  {
    return false;
  }
...
bool CCustomFaceManager::OnDataStream(const VOID * pcbMailData, WORD wStreamSize)
{
  ...
  if((wStreamSize+m_CustomFaceInfo.dwDataSize)>sizeof(m_CustomFaceInfo.dwCustomFace))
  {
    return false;
  }
