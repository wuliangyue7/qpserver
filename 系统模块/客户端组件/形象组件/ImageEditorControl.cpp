...
bool CImageEditorControl::DestoryImage()
{
  ...
  if(m_pIImageEditorEvent!=NULL)
  {
    m_pIImageEditorEvent->OnEventImageSelect(false,false);
  }
...
bool CImageEditorControl::LoadEditImage(CImage & Image)
{
  ...
  if(m_pIImageEditorEvent!=NULL)
  {
    m_pIImageEditorEvent->OnEventImageSelect(true,false);
  }
...
VOID CImageEditorControl::OnMouseMove(UINT nFlags, CPoint MousePoint)
{
  ...
  if(m_cbOperateType==OPERATE_SIZE)
  {
    ...
    switch(m_cbConsultType)
    {
      ...
      case LOCK_TYPE_R:         //锁定边框
      {
        ...
        //宽度判断
        if((m_ptLockPoint.x+nSelectDrag)<rcImageArea.left)
        {
          nSelectDrag=rcImageArea.left-m_ptLockPoint.x;
        }
        if((m_ptLockPoint.x+nSelectDrag)>rcImageArea.right)
        {
          nSelectDrag=rcImageArea.right-m_ptLockPoint.x;
        }

        //高度判断
        INT nYQuotiety=(nSelectDrag>0)?1L:-1L;
        if((m_ptLockPoint.y-nSelectDrag*nYQuotiety/2L)<rcImageArea.top)
        {
          nSelectDrag=(m_ptLockPoint.y-rcImageArea.top)*nYQuotiety*2L;
        }
        if((m_ptLockPoint.y+nSelectDrag*nYQuotiety/2L)>rcImageArea.bottom)
        {
          nSelectDrag=(rcImageArea.bottom-m_ptLockPoint.y)*nYQuotiety*2L;
        }
      ...
      case LOCK_TYPE_B:         //锁定边框
      {
        ...
        //高度判断
        if((m_ptLockPoint.y+nSelectDrag)<rcImageArea.top)
        {
          nSelectDrag=rcImageArea.top-m_ptLockPoint.y;
        }
        if((m_ptLockPoint.y+nSelectDrag)>rcImageArea.bottom)
        {
          nSelectDrag=rcImageArea.bottom-m_ptLockPoint.y;
        }

        //宽度判断
        INT nXQuotiety=(nSelectDrag>0)?1L:-1L;
        if((m_ptLockPoint.x-nSelectDrag*nXQuotiety/2L)<rcImageArea.left)
        {
          nSelectDrag=(m_ptLockPoint.x-rcImageArea.left)*nXQuotiety*2L;
        }
        if((m_ptLockPoint.x+nSelectDrag*nXQuotiety/2L)>rcImageArea.right)
        {
          nSelectDrag=(rcImageArea.right-m_ptLockPoint.x)*nXQuotiety*2L;
        }
      ...
      case LOCK_TYPE_R|LOCK_TYPE_B:   //锁定转角
      {
        ...
        //目标系数
        INT nYQuotiety=1L;
        if(m_cbConsultType==(LOCK_TYPE_R|LOCK_TYPE_T))
        {
          nYQuotiety=-1L;
        }
        if(m_cbConsultType==(LOCK_TYPE_L|LOCK_TYPE_B))
        {
          nYQuotiety=-1L;
        }
        ...
        //高度判断
        if((m_ptLockPoint.y+nSelectDrag*nYQuotiety)<rcImageArea.top)
        {
          nSelectDrag=(rcImageArea.top-m_ptLockPoint.y)*nYQuotiety;
        }
        if((m_ptLockPoint.y+nSelectDrag*nYQuotiety)>rcImageArea.bottom)
        {
          nSelectDrag=(rcImageArea.bottom-m_ptLockPoint.y)*nYQuotiety;
        }
    ...
    if(rcImageSelect!=m_rcImageSelect)
    {
      ...
      if(m_pIImageEditorEvent!=NULL)
      {
        m_pIImageEditorEvent->OnEventImageSelect(true,true);
      }
    }
  }
  ...
  if(m_cbOperateType==OPERATE_DRAG)
  {
    ...
    //宽度判断
    if((m_rcImageSelect.left+nSelectDragCX)<rcImageArea.left)
    {
      nSelectDragCX=rcImageArea.left-m_rcImageSelect.left;
    }
    if((m_rcImageSelect.right+nSelectDragCX)>rcImageArea.right)
    {
      nSelectDragCX=rcImageArea.right-m_rcImageSelect.right;
    }

    //高度判断
    if((m_rcImageSelect.top+nSelectDragCY)<rcImageArea.top)
    {
      nSelectDragCY=rcImageArea.top-m_rcImageSelect.top;
    }
    if((m_rcImageSelect.bottom+nSelectDragCY)>rcImageArea.bottom)
    {
      nSelectDragCY=rcImageArea.bottom-m_rcImageSelect.bottom;
    }
