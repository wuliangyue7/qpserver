...
void CALLBACK VideoDataCallBack(DWORD dwUserID, LPVOID lpBuf, DWORD dwLen, BITMAPINFOHEADER bmiHeader, LPVOID lpUserValue)
{
  ...
  if(pVideoServiceManager&&pVideoServiceManager->m_hWnd!=NULL)
  {
    ...
    if(pIClientKernel->IsServiceStatus()==false)
    {
      return;
    }
...
void CALLBACK AudioDataCallBack(DWORD dwUserID, LPVOID lpBuf, DWORD dwLen, WAVEFORMATEX waveFormatEx, LPVOID lpUserValue)
{
  ...
  if(pVideoServiceManager)
  {
    ...
    if(pVideoServiceControl==NULL)
    {
      return;
    }
    ASSERT(pVideoServiceControl->GetSafeHwnd()!=NULL);
    if(pVideoServiceControl->GetSafeHwnd()==NULL)
    {
      return;
    }
...
void CVideoServiceControl::SetVolume(DWORD dwCurrentVolume)
{
  ...
  if(m_fVolumePercent>1.0)
  {
    m_fVolumePercent=1.0f;
  }
...
void CVideoServiceControl::GetVideoSize(CRect &rcVideo)
{
  ...
  {
    rcVideo.SetRect(X_SMALL_POS,Y_SMALL_POS,X_SMALL_POS+W_SMALL_POS,Y_SMALL_POS+H_SMALL_POS);
  }
  else
  {
    rcVideo.SetRect(X_LARGE_POS,Y_LARGE_POS,X_LARGE_POS+W_LARGE_POS,Y_LARGE_POS+H_LARGE_POS);
  }
}
...
bool CVideoServiceControl::InitVideoService(bool bLocalMode, bool bSmallMode)
{
  ...
  if((m_hWnd==NULL)||(m_bInitVideo==true))
  {
    return false;
  }
...
VOID CVideoServiceControl::RectifyControl()
{
  ...
  if(m_bInitVideo==true)
  {
    ...
    if(m_bSmallMode==true)
    {
      pImageFrame=&m_ImageSmallFrame;
    }
    if(m_bSmallMode==false)
    {
      pImageFrame=&m_ImageLargeFrame;
    }
...
VOID CVideoServiceControl::OnBnClickedClone()
{
  ...
  {
    if(m_DlgVideoClone.m_hWnd==NULL)
    {
      m_DlgVideoClone.Create(IDD_VIDEO_CLONE,this);
    }
...
//  }
  ...
  switch(LOWORD(wParam))
  {
    case IDM_UM_COPY_NAME:    //拷贝名字
    {
      ...
      //寻找玩家
      if((m_pUserData==NULL)||(m_pUserData->dwUserID==0))
      {
        return TRUE;
      }
...
VOID CVideoServiceControl::OnBnClickedVideo()
{
  ...
  if(dwReturnValue!=GV_ERR_SUCCESS || szDeviceName[0]==0)
  {
    return;
  }
  ...
  if(m_bSmallMode==false)
  {
    m_btVideo.SetButtonImage((m_bEnableVideo==true)?IDB_BT_BIG_SEE:IDB_BT_BIG_NOSEE,hInstance,false,false);
  }
  if(m_bSmallMode==true)
  {
    m_btVideo.SetButtonImage((m_bEnableVideo==true)?IDB_BT_SMALL_SEE:IDB_BT_SMALL_NOSEE,hInstance,false,false);
  }
  ...
  {
    m_ToolTipCtrl.AddTool(&m_btVideo, TEXT("启用视频"));
  }
...
VOID CVideoServiceControl::OnBnClickedAudio()
{
  ...
  if(m_bSmallMode==false)
  {
    m_btAudio.SetButtonImage((m_bEnableAudio==true)?IDB_BT_BIG_TALK:IDB_BT_BIG_NOTALK,hInstance,false,false);
  }
  if(m_bSmallMode==true)
  {
    m_btAudio.SetButtonImage((m_bEnableAudio==true)?IDB_BT_SMALL_TALK:IDB_BT_SMALL_NOTALK,hInstance,false,false);
  }
  ...
  {
    m_ToolTipCtrl.AddTool(&m_btAudio, TEXT("启用语音"));
  }
...
void CVideoServiceControl::DrawUserVideo(DWORD dwUserid, LPVOID lpBuf, DWORD dwLen, BITMAPINFOHEADER bmiHeader)
{
  ...
  if(m_DlgVideoClone.m_hWnd!=NULL)
  {
    m_DlgVideoClone.DrawUserVideo(dwUserid, lpBuf, dwLen, bmiHeader);
  }
...
{
  //设置标识
  if(cbState==GV_CAMERA_STATE_OPEN)
  {
    m_bShowVideo=true;
  }
  else
...
CVideoServiceManager::CVideoServiceManager()
{
  ...
  if(m_pVideoServiceManager==NULL)
  {
    m_pVideoServiceManager=this;
  }
...
void CVideoServiceManager::LoginVideoSystem()
{
  ...
  if(pUserData==NULL || pUserData->cbUserStatus==US_LOOKON)
  {
    return;
  }
  ...
  {
    strLog.Format(TEXT("连接服务器失败，错误代码未：%ld"), dwReturnValue);
  }
...
bool CVideoServiceManager::OnEventGameReset()
{
  ...
  {
    strLog.Format(TEXT("登陆视频房间失败，错误代码未：%ld"), dwReturnValue);
  }
...
{
#ifdef _DEBUG
  if(m_pIMessageProxy!=NULL)
  {
    m_pIMessageProxy->InsertSystemString(strLog);
  }
#endif
}
...
LRESULT CVideoServiceManager::OnGVClientConnect(WPARAM wParam, LPARAM lParam)
{
  ...
  if(wParam)
  {
    ...
    {
      strLog.Format(TEXT("登陆视频服务器失败，错误代码未：%ld"), dwReturnValue);
    }
...
LRESULT CVideoServiceManager::OnGVClientLogin(WPARAM wParam, LPARAM lParam)
{
  ...
  if(lParam==0)
  {
    ...
    if(m_dwLocalUserID==wParam)
    {
      strLog=TEXT("登陆视频服务器成功");
    }
    else
    {
      strLog.Format(TEXT("视频服务器返回用户标识和本地标识不一致，返回:%ld，本地:%ld"),wParam,m_dwLocalUserID);
    }
