...
VOID CSkinComboBox::DrawControlView(CDC * pDC)
{
  ...
  else if(bDropped==true)
  {
    nImageIndex=1;
  }
  else if(m_bHovering==true)
  {
    nImageIndex=3;
  }
  ...
  //渲染资源
  CSkinRenderManager * pSkinRenderManager=CSkinRenderManager::GetInstance();
  if((pSkinRenderManager!=NULL)&&(m_bRenderImage==true))
  {
...
bool CSkinComboBox::VerdictOverButton(CPoint MousePoint)
{
  ...
  //下拉列表
  if((rcClient.PtInRect(MousePoint)==TRUE)&&((GetStyle()&CBS_SIMPLE)!=0))
  {
    return true;
  }

  //坐标计算
  if((MousePoint.y>(rcClient.Height()-1))||(MousePoint.y<1))
  {
    return false;
  }
  if((MousePoint.x<(rcClient.Width()-(INT)m_uButtonWidth-1))||(MousePoint.x>(rcClient.Width()-1)))
  {
    return false;
  }
