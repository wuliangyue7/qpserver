...
bool CParameterGame::InitParameter(LPCTSTR pszProcessName)
{
  ...
  {
    //终止判断
    if(pszProcessName[wNameIndex]==0)
    {
      break;
    }
    if(pszProcessName[wNameIndex]==TEXT('.'))
    {
      break;
    }
...
CParameterGlobal::CParameterGlobal()
{
  ...
  if(m_pParameterGlobal==NULL)
  {
    m_pParameterGlobal=this;
  }
...
CParameterGlobal::~CParameterGlobal()
{
  ...
  if(m_pParameterGlobal==this)
  {
    m_pParameterGlobal=NULL;
  }
...
VOID CParameterGlobal::SaveParameter()
{
  ...
  while(PositionGame!=NULL)
  {
    ...
    if(pParameterGame!=NULL)
    {
      pParameterGame->SaveParameter();
    }
  }
...
CParameterGame * CParameterGlobal::GetParameterGame(tagGameKind * pGameKind)
{
  //寻找现存
  CParameterGame * pParameterGame=NULL;
  if(m_ParameterGameMap.Lookup(pGameKind->wKindID,pParameterGame)==TRUE)
  {
...
CParameterServer * CParameterGlobal::GetParameterServer(tagGameServer * pGameServer)
{
  //寻找现存
  CParameterServer * pParameterServer=NULL;
  if(m_ParameterServerMap.Lookup(pGameServer->wServerID,pParameterServer)==TRUE)
  {
