...
CPlazaViewServerCtrl::~CPlazaViewServerCtrl()
{
  ...
  if(m_pTrumpetItem)
  {
    m_pTrumpetItem->DestroyWindow();
  }
  if(m_pExpressionControl)
  {
    m_pExpressionControl->DestroyWindow();
  }
  ...
  if(lstrcmp(msg.sType, TEXT("click")) == 0)
  {
    ...
    else if(lstrcmp(pControlUI->GetName(),szButtonChat2ControlName) == 0)
    {
      ...
      for(INT i=0; i<CountArray(ImageColor); i++)
      {
        ...
        if(pParameterGlobal->m_crChatTX==crColor[i])
        {
...
//道具配置
VOID CPlazaViewServerCtrl::SetPropertyDescribe(tagPropertyInfo PropertyInfo[], BYTE cbPropertyCount)
{
  //设置道具
  m_GamePropertyCtrl.SetGamePropertyInfo(PropertyInfo,cbPropertyCount,PT_ISSUE_AREA_SERVER);
...
VOID CPlazaViewServerCtrl::SetTrumpetCount(const DWORD dwCount)
{
  ...
  if(m_pTrumpetItem)
  {
    m_pTrumpetItem->SetTrumpet(dwCount);
  }
}
...
VOID CPlazaViewServerCtrl::SetTyphonCount(const DWORD dwCount)
{
  ...
  if(m_pTrumpetItem)
  {
    m_pTrumpetItem->SetTyphon(dwCount);
  }
}
...
VOID CPlazaViewServerCtrl::TrackUserItemMenu(IClientUserItem * pIClientUserItem[], WORD wItemCount)
{
  ...
  if((pIClientUserItem==NULL)||(wItemCount==0))
  {
    return;
  }
...
BOOL CPlazaViewServerCtrl::OnInitDialog()
{
  ...
  ////创建广告
  //m_BillPublicize.SetBoradColor(RGB(255,255,255));
  //m_BillPublicize.Create(NULL,NULL,WS_CHILD|WS_VISIBLE|WS_CLIPCHILDREN,rcCreate,this,IDC_WEB_BILL_PUBLICIZE);

  //获取对象
  //CGlobalWebLink * pGlobalWebLink=CGlobalWebLink::GetInstance();

  //构造网页
  //TCHAR szUrl[128]=TEXT("");
  //_sntprintf(szUrl,CountArray(szUrl),TEXT("%s/ads/GameRoom.aspx"),pGlobalWebLink->GetPlatformLink());
  //m_BillPublicize.Navigate(szUrl);
  ...
  PropertyResource.PointExcursion.y=6;
  PropertyResource.SizeExcursion.cx=36;
  PropertyResource.SizeExcursion.cy=36;
  PropertyResource.LButtonExcursion.x=12;
  PropertyResource.LButtonExcursion.y=30;
  PropertyResource.RButtonExcursion.x=12;
...
BOOL CPlazaViewServerCtrl::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch(nCommandID)
  {
    ...
    case IDM_KILL_USER:             //踢用户下线
    {
      ...
      bool bHideUserInfo=CServerRule::IsAllowAvertCheatMode(m_dwServerRule);
      if((bHideUserInfo==true)&&(m_pIMySelfUserItem->GetMasterOrder()==0)&&
         (m_pIMySelfUserItem->GetMemberOrder()<=pIClientUserItem->GetMemberOrder()))
      {
        return FALSE;
      }
    ...
    case IDM_LOOKON_USER:           //旁观游戏
    {
      ...
      if(pPlazaViewServer->GetServiceStatus()!=ServiceStatus_ServiceIng)
      {
        return TRUE;
      }
    ...
    case IDM_INVITE_USER:           //邀请游戏
    {
      ...
      if(m_pIMySelfUserItem->GetTableID()==INVALID_TABLE)
      {
        return TRUE;
      }
...
VOID CPlazaViewServerCtrl::RectifyControl(INT nWidth, INT nHeight)
{
  ...
  //广告控件
  //DeferWindowPos(hDwp,m_BillPublicize,NULL,EncircleInfoList.nLBorder+4,10,nWidth-EncircleInfoList.nLBorder-EncircleInfoList.nRBorder-8,BILL_VIEW_CY,uFlags);
...
VOID CPlazaViewServerCtrl::OnSelectMoreColor()
{
  ...
  {
    pParameterGlobal->m_crChatTX=ColorDialog.GetColor();
  }
...
VOID CPlazaViewServerCtrl::OnSelectChatColor(UINT nCommandID)
{
  ...
  {
    pParameterGlobal->m_crChatTX=crColor[nIndex];
  }
