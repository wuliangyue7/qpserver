...
BOOL CDlgWhisper::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch(LOWORD(wParam))
  {
    case IDM_SEND_BY_ENTER:
    {
      ...
      {
        pParameterGlobal->m_bSendWhisperByEnter=true;
      }
    ...
    case IDM_SEND_BY_CTRL_ENTER:
    {
      ...
      if(pParameterGlobal!=NULL)
      {
        pParameterGlobal->m_bSendWhisperByEnter=false;
      }
...
bool CDlgWhisper::DisuseEstimate()
{
  ...
  if(m_ClientUserItemArray.GetCount()<=1L)
  {
    return true;
  }
...
VOID CDlgWhisper::OnEventExpression(IClientUserItem * pIClientUserItem, WORD wItemIndex)
{
  ...
  if(IsWindowVisible()==FALSE)
  {
    ...
    else
    {
      ...
      {
        pPlatformFrame->InsertWhisperItem(this);
      }
    }
  }
  else
...
bool CDlgWhisper::SendColloquyChatPacket(IClientUserItem * pIClientUserItem[], WORD wUserCount, LPCTSTR pszChatString)
{
  ...
  for(WORD i=0; i<wUserCount; i++)
  {
    ...
    if(i>=CountArray(ColloquyChat.dwTargetUserID))
    {
      break;
    }
...
VOID CDlgWhisper::OnBnClickedSendControl()
{
  ...
  //设置菜单
  CParameterGlobal * pParameterGlobal=CParameterGlobal::GetInstance();
  if(pParameterGlobal->m_bSendWhisperByEnter==true)
  {
    Menu.CheckMenuItem(IDM_SEND_BY_ENTER,MF_BYCOMMAND|MF_CHECKED);
  }
  if(pParameterGlobal->m_bSendWhisperByEnter==false)
  {
    Menu.CheckMenuItem(IDM_SEND_BY_CTRL_ENTER,MF_BYCOMMAND|MF_CHECKED);
  }
...
VOID CDlgWhisper::OnBnClickedChatButton2()
{
  ...
  for(INT i=0; i<CountArray(ImageColor); i++)
  {
    ...
    if(pParameterGlobal->m_crWhisperTX==crColor[i])
    {
...
VOID CDlgWhisper::OnSelectMoreColor()
{
  ...
  {
    pParameterGlobal->m_crWhisperTX=ColorDialog.GetColor();
  }
...
VOID CDlgWhisper::OnSelectChatColor(UINT nCommandID)
{
  ...
  {
    pParameterGlobal->m_crWhisperTX=crColor[nIndex];
  }
...
{
  //选择判断
  if(m_ComboAndroidList.GetCurSel()==-1)
  {
    return;
  }
