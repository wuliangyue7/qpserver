...
CTableResource::CTableResource()
{
  ...
  if(m_pTableResource==NULL)
  {
    m_pTableResource=this;
  }
...
CTableResource::~CTableResource()
{
  ...
  //设置变量
  if(m_pTableResource==this)
  {
    m_pTableResource=NULL;
  }
...
bool CTableResource::LoadResource(LPCTSTR pszResDirectory, WORD wChairCount, bool bAvertCheatMode)
{
  ...
  if((pszResDirectory==NULL)||(wChairCount>MAX_CHAIR))
  {
    return false;
  }
...
bool CTableResource::UpdateResource(CSkinRenderManager * pSkinRenderManager)
{
  ...
  if((m_bShowEnter==true)&&(m_ImageEnter.IsNull()==true))
  {
    ...
    //加载资源
    if(m_ImageEnter.LoadImage(szImagePath)==false)
    {
      return false;
    }
  }
  ...
  if((m_bCustomUser==true)&&(m_ImageCustomUser.IsNull()==true))
  {
    ...
    //加载资源
    if(m_ImageCustomUser.LoadImage(szImagePath)==false)
    {
      return false;
    }
  }
  ...
  if((m_bShowChair==true)&&(m_bShowTable==true))
  {
    ...
    if(ImageChair.LoadImage(szImagePath)==false)
    {
      return false;
    }
...
IClientUserItem * CTableView::GetClientUserItem(WORD wChairID)
{
  ...
  if(wChairID>=m_TableAttribute.wChairCount)
  {
    return NULL;
  }
...
VOID CTableView::DrawTableView(CDC * pDC, DWORD dwServerRule, CTableResource * pTableResource)
{
  ...
  if((pTableResource->m_bShowEnter==true)&&(pTableResource->m_ImageEnter.IsNull()==false))
  {
    //计算位置
    INT nImageIndex=0;
    if(m_TableAttribute.bPlaying==true)
    {
      nImageIndex=1;
    }
    if((m_wHoverChairID==INDEX_ENTER_CHAIR)&&(m_TableAttribute.bPlaying==false)&&(m_bMouseDown==true))
    {
      nImageIndex=1;
    }
    if((m_wHoverChairID==INDEX_ENTER_CHAIR)&&(m_TableAttribute.bPlaying==false)&&(m_bMouseDown==false))
    {
      nImageIndex=2;
    }
      ...
      if(pTableResource->m_bShowUser==true)
      {
        ...
        if((bAvertCheatMode==false)||(m_TableAttribute.wTableID==0))
        {
          ...
          if((pTableResource->m_bCustomUser==false)&&((m_wHoverChairID==i)||(m_TableAttribute.pIClientUserItem[i]!=NULL)))
          {
            ...
            if((m_wHoverChairID==i)&&(m_bMouseDown==false))
            {
              cbFrameKind=ITEM_FRAME_HOVER;
            }
            if((m_wHoverChairID==i)&&(m_bMouseDown==true))
            {
              cbFrameKind=ITEM_FRAME_HITDOWN;
            }
...
VOID CTableView::DrawTableName(CDC * pDC, WORD wChairID, LPCTSTR pszNickName, CTableResource * pTableResource)
{
  ...
  if(nNameHeight<=12)
  {
    ...
    else if(nDrawStyte&NT_RIGHT)
    {
      nDrawFormat|=DT_RIGHT;
    }
    else
    {
      nDrawFormat|=(DT_CENTER|DT_SINGLELINE);
    }
  }
  else
  {
  ...
  //竖向位置
  INT nYPos=0;
  if(nDrawStyte&NT_BOTTOM)
  {
    nYPos=rcName.Height()-nNameHeight;
  }
  else if(nDrawStyte&NT_TOP)
  {
    nYPos=0;
  }
  else
  {
    nYPos=(rcName.Height()-nNameHeight)/2;
  }
...
VOID CTableViewFrame::SetTableStatus(WORD wTableID, bool bPlaying, bool bLocker,LONG lCellScore)
{
  ...
  if(pITableView!=NULL)
  {
    pITableView->SetTableStatus(bPlaying,bLocker,lCellScore);
  }
...
bool CTableViewFrame::VisibleTable(WORD wTableID)
{
  ...
  if(rcTable.top<0L)
  {
    m_nScrollPos+=rcTable.top;
  }
  if((rcTable.top>0)&&(rcTable.bottom>rcClient.bottom))
  {
    m_nScrollPos+=rcTable.bottom-rcClient.bottom;
  }
...
bool CTableViewFrame::UpdateTableView(WORD wTableID)
{
  //获取桌子
  ITableView * pITableView=GetTableViewItem(wTableID);
  if((pITableView==NULL)||(m_nXTableCount==0))
  {
    return false;
  }
...
ITableView * CTableViewFrame::GetTableViewItem(WORD wTableID)
{
  ...
  if(wTableID!=INVALID_TABLE)
  {
    ...
    if(wTableID>=m_TableViewArray.GetCount())
    {
      return NULL;
    }
...
VOID CTableViewFrame::OnLButtonDown(UINT nFlags, CPoint MousePoint)
{
  ...
  if((m_bRMouseDown==false)&&(m_bLMouseDown==false))
  {
    ...
    if(m_wDownTableID!=INVALID_TABLE)
    {
      pITableView=GetTableViewItem(m_wDownTableID);
    }
...
{
  //状态效验
  if(m_TableResource.m_SizeTable.cx==0)
  {
    return INVALID_TABLE;
  }
  if(m_TableResource.m_SizeTable.cy==0)
  {
    return INVALID_TABLE;
  }
  ...
  if((nYItem>=0)&&(nXItem>=0)&&(nYItem<m_nYTableCount)&&(nXItem<m_nXTableCount))
  {
    ...
    {
      bool bAvertCheatMode=CServerRule::IsAllowAvertCheatMode(m_dwServerRule);
      if((wResultItem==0)||(bAvertCheatMode==false))
      {
        return wResultItem;
      }
    }
...
WORD CTableViewFrame::SwitchChairID(POINT MousePoint)
{
  ...
  if((m_TableResource.m_bShowUser==true)||(m_TableResource.m_bShowChair==true))
  {
    ...
    {
      //横向判断
      if(MousePoint.x<m_TableResource.m_rcChairArray[i].left)
      {
        continue;
      }
      if(MousePoint.x>m_TableResource.m_rcChairArray[i].right)
      {
        continue;
      }

      //竖向判断
      if(MousePoint.y<m_TableResource.m_rcChairArray[i].top)
      {
        continue;
      }
      if(MousePoint.y>m_TableResource.m_rcChairArray[i].bottom)
      {
        continue;
      }
  ...
  if(m_TableResource.m_bShowEnter==true)
  {
    ...
    if(SizeTable.cx > rcClient.Width())
    {
      SizeTable.cx=rcClient.Width();
    }

    //横向判断
    if(MousePoint.x<SizeTable.cx/2+m_TableResource.m_ptEnter.x)
    {
      bOutSide=true;
    }
    if(MousePoint.x>SizeTable.cx/2+m_TableResource.m_ptEnter.x+m_TableResource.m_SizeEnter.cx)
    {
      bOutSide=true;
    }

    //竖向判断
    if(MousePoint.y<SizeTable.cy/2+m_TableResource.m_ptEnter.y)
    {
      bOutSide=true;
    }
    if(MousePoint.y>SizeTable.cy/2+m_TableResource.m_ptEnter.y+m_TableResource.m_SizeEnter.cy)
    {
      bOutSide=true;
    }
...
LRESULT CTableViewFrame::OnMessagePlatformEvent(WPARAM wParam, LPARAM lParam)
{
  ...
  if(wParam==EVENT_SKIN_CONFIG_UPDATE)
  {
    ...
    if(m_TableResource.m_bRenderImage==true)
    {
      ...
      {
        m_TableResource.UpdateResource(pSkinRenderManager);
      }
...
CMatchViewFrame::~CMatchViewFrame()
{
  ...
  if(m_ImageStatus.IsNull()==false)
  {
    m_ImageStatus.DestroyImage();
  }
  if(m_ImageNumber1.IsNull()==false)
  {
    m_ImageNumber1.DestroyImage();
  }
  if(m_ImageNumber2.IsNull()==false)
  {
    m_ImageNumber2.DestroyImage();
  }

  //释放资源
  if(m_ImageMatchBack.IsNull()==false)
  {
    m_ImageMatchBack.DestroyImage();
  }
  if(m_ImageMatchText.IsNull()==false)
  {
    m_ImageMatchText.DestroyImage();
  }
  if(m_ImageMatchShow.IsNull()==false)
  {
    m_ImageMatchShow.DestroyImage();
  }
...
bool CMatchViewFrame::OnEventViewData(WORD wCmdID, VOID * pData, WORD wDataSize)
{
  switch(wCmdID)
  {
    case SUB_GR_MATCH_RULE:     //比赛规则
    {
      ...
      if(wDataSize>sizeof(CMD_GR_Match_Rule))
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_NUM:      //参赛人数
    {
      ...
      if(wDataSize!=sizeof(CMD_GR_Match_Num))
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_DESC:     //比赛描述
    {
      ...
      if(wDataSize!=sizeof(CMD_GR_MatchDesc))
      {
        return false;
      }
...
bool CMatchViewFrame::LoadResource(LPCTSTR pszResDirectory)
{
  ...
  //判断目录
  if(IsDirectoryExist(szMatchDirectory)==false)
  {
    return false;
  }
...
{
  //判断指针
  if(m_GameMatch.dwMatchID==0 || nWidth==0 || nHeight==0)
  {
    return;
  }
...
VOID CMatchViewFrame::DrawWords(CDC * pDC,INT nXPos,INT nYPos,CPngImage * pWordsImage,LPCTSTR pszWords,LPCTSTR pszText,UINT nFormat)
{
  ...
  //调整横坐标
  if((nFormat&DT_LEFT)!=0)
  {
    nXPos -= 0;
  }
  if((nFormat&DT_CENTER)!=0)
  {
    nXPos -= (lstrlen(szText)*SizeWord.cx)/2;
  }
  if((nFormat&DT_RIGHT)!=0)
  {
    nXPos -= lstrlen(szText)*SizeWord.cx;
  }
...
VOID CMatchViewFrame::DrawNumber(CDC * pDC,INT nXPos,INT nYPos,CPngImage * pNumberImage,LPCTSTR pszNumber,INT nNumber,LPCTSTR pszFormat,UINT nFormat)
{
  ...
  if((nFormat&DT_CENTER)!=0)
  {
    nXPos -= (lstrlen(szValue)*SizeNumber.cx)/2;
  }
  if((nFormat&DT_RIGHT)!=0)
  {
    nXPos -= lstrlen(szValue)*SizeNumber.cx;
  }
