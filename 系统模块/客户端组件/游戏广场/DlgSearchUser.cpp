...
CDlgSearchUser::CDlgSearchUser() : CSkinDialog(IDD_DLG_POPUP)
{
  ...
  if(m_pDlgSearchUser==NULL)
  {
    m_pDlgSearchUser=this;
  }
...
CDlgSearchUser::~CDlgSearchUser()
{
  ...
  if(m_pDlgSearchUser==this)
  {
    m_pDlgSearchUser=NULL;
  }
...
VOID CDlgSearchUser::OnOK()
{
  ...
  if(pNickNameControl==NULL || pGameIDControl==NULL)
  {
    return;
  }
  ...
  if(bNickNameMode==false)
  {
    strSearchKey = pGameIDControl->GetText();
  }
  ...
  if(bNickNameMode==true)
  {
    ...
    //查找自己则退出
    if(lstrcmp(pGlobalUserData->szNickName,m_szNickName)==0)
    {
      return;
    }
  }
  else
  {
    ...
    //查找自己则退出
    if(pGlobalUserData->dwGameID==m_dwGameID)
    {
      return;
    }
  }
...
void CDlgSearchUser::InitControlUI()
{
  ...
  {
    pEditControl->SetMaxChar(LEN_NICKNAME-1);
  }
  ...
  {
    pButtonControl->SetVisible(false);
  }
}
...
void CDlgSearchUser::Notify(TNotifyUI &  msg)
{
  ...
  if(lstrcmp(msg.sType, TEXT("click")) == 0)
  {
    ...
    if(lstrcmp(pControlUI->GetName(), szButtonEnterControlName) == 0)
    {
      ...
      {
        pPlatformEvent->SendPlatformEvent(EVENT_USER_TRACK_ACCOUNTS,(LPARAM)&TrackAccountsInfo);
      }
  ...
  else if(lstrcmp(msg.sType,TEXT("itemselect")) ==0)
  {
    if(lstrcmp(pControlUI->GetName(), szComboGameIDControlName)==0)
    {
      ...
      if(pEditGameID!=NULL)
      {
        pEditGameID->SetText(pControlUI->GetText());
      }
    ...
    else if(lstrcmp(pControlUI->GetName(), szComboNickNameControlName)==0)
    {
      ...
      if(pEditNickName!=NULL)
      {
        pEditNickName->SetText(pControlUI->GetText());
      }
  ...
  if(Command.wMainCmdID==MDM_GP_REMOTE_SERVICE)
  {
    switch(Command.wSubCmdID)
    {
      case SUB_GP_S_SEARCH_CORRESPOND:  //查找结果
      {
        ...
        //效验参数
        if(wDataSize>sizeof(CMD_GP_S_SearchCorrespond))
        {
          return false;
        }
        if(wDataSize<(sizeof(CMD_GP_S_SearchCorrespond)-sizeof(pSearchCorrespond->UserRemoteInfo)))
        {
          return false;
        }
        if(wDataSize!=(sizeof(CMD_GP_S_SearchCorrespond)-sizeof(pSearchCorrespond->UserRemoteInfo)+pSearchCorrespond->wUserCount*sizeof(pSearchCorrespond->UserRemoteInfo[0])))
        {
          return false;
        }
...
VOID CDlgSearchUser::LoadFriendUserInfo()
{
  ...
  {
    //判断关系
    if(pCompanionInfo->cbCompanion!=CP_FRIEND)
    {
      continue;
    }
