...
CPlatformEvent::CPlatformEvent()
{
  ...
  if(m_pPlatformEvent==NULL)
  {
    m_pPlatformEvent=this;
  }
...
CPlatformEvent::~CPlatformEvent()
{
  ...
  if(m_pPlatformEvent==this)
  {
    m_pPlatformEvent=NULL;
  }
...
VOID CPlatformEvent::SendPlatformEvent(WORD wEventID, LPARAM lParam)
{
  ...
  for(INT_PTR i=0; i<m_EventWndArray.GetCount(); i++)
  {
    ...
    if(IsWindow(hWnd))
    {
      SendMessage(hWnd,WM_PLATFORM_EVENT,wEventID,lParam);
    }
  }
...
VOID CPlatformEvent::PostPlatformEvent(WORD wEventID, LPARAM lParam)
{
  ...
  for(INT_PTR i=0; i<m_EventWndArray.GetCount(); i++)
  {
    ...
    if(IsWindow(hWnd))
    {
      PostMessage(hWnd,WM_PLATFORM_EVENT,wEventID,lParam);
    }
  }
