...
void CPlazaHallList::LoadDataFromMemory()
{
  ...
  while(true)
  {
    ...
    if(itGameKind==m_vGameKind.end())
    {
      m_vGameKind.push_back(pGameKindItem);
    }
    if(Position==NULL)
    {
      break;
  ...
  while(true)
  {
    ...
    if(itGameServer==m_vGameServer.end())
    {
      m_vGameServer.push_back(pGameServerItem);
    }
    if(Position==NULL)
    {
      break;
...
void CPlazaHallList::CreateUI()
{
  ...
  if(pGameList != NULL)
  {
    ...
    for(vector<CNodeElementUI *>::iterator iter=vItme0.begin(); iter!=vItme0.end(); iter++)
    {
      ...
      if(pControl != NULL)
      {
        pControl->SetText(pGameTypeItem->m_GameType.szTypeName);
      }
      ...
      if(pControl != NULL)
      {
        pControl->SetText(szTypeGameNum);
      }
    ...
    for(vector<CNodeElementUI *>::iterator iter=vItme1.begin(); iter!=vItme1.end(); iter++)
    {
      ...
      if(pControl != NULL)
      {
        pControl->SetText(pGameKind->m_GameKind.szKindName);
      }
      ...
      if(pControl != NULL)
      {
        pControl->SetVisible((pGameKind->m_GameKind.wGameFlag & 0x1) == 0x1);
      }
      pControl =  itme1->FindControl(_TEXT("GameKindFlagRecommed"));
      if(pControl != NULL)
      {
        pControl->SetVisible((pGameKind->m_GameKind.wGameFlag & 0x2) == 0x2);
      }
      pControl =  itme1->FindControl(_TEXT("GameKindFlagHot"));
      if(pControl != NULL)
      {
        pControl->SetVisible((pGameKind->m_GameKind.wGameFlag & 0x4) == 0x4);
      }
      pControl =  itme1->FindControl(_TEXT("GameKindFlagMatch"));
      if(pControl != NULL)
      {
        pControl->SetVisible((pGameKind->m_GameKind.wGameFlag & 0x8) == 0x8);
      }
    ...
    for(vector<CNodeElementUI *>::iterator iter=vItme2.begin(); iter!=vItme2.end(); iter++)
    {
      ...
      if(pControl != NULL)
      {
        pControl->SetText(pGameServer->m_GameServer.szServerName);
      }
      ...
      if(pControl != NULL)
      {
        pControl->SetText(szServerGameLimit);
      }
...
bool CPlazaHallList::SendMessage(UINT message, WPARAM wParam , LPARAM lParam)
{
  ...
  {
    pPlatformEvent->SendPlatformEvent(message,(LPARAM)wParam);
  }
  return true;
}
//////////////////////////////////////////////////////////////////////////////////