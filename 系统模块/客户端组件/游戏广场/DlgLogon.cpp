      ...
      if(pCnt)
      {
        ...
        if(pAccountsInfo != NULL)
        {
          ...
          else
          {
            if(pCheckButtonPasswd!=NULL)
            {
              pCheckButtonPasswd->SetCheck(true);
            }
            if(pEditPassword)
            {
...
WORD CDlgLogon::ConstructLogonPacket(BYTE cbBuffer[], WORD wBufferSize, BYTE cbValidateFlags)
{
  ...
  switch(m_cbLogonMode)
  {
    case LOGON_BY_VISITOR:      //游客登录
    {
      ...
      {
        pGlobalWebLink->GetSessionKey(pLogonVisitor->szSessionKey);
      }
    ...
    case LOGON_BY_GAME_ID:      //I D 登录
    {
      ...
      if(pGlobalWebLink != NULL)
      {
        pGlobalWebLink->GetSessionKey(pLogonGameID->szSessionKey);
      }
...
void CDlgLogon::LoadAccountsInfo()
{
  ...
  if(RegUserInfo.OpenRegKey(REG_USER_INFO, false)==false)
  {
    return;
  }
    ...
    if(pAccountsInfo->szAccounts[0]!=0)
    {
      try
      {
        ...
        if(pListLabelElement)
        {
          ...
          if(pComboAccounts->Add(pListLabelElement)==true)
          {
            pListLabelElement->SetUserData(pAccountsInfo);
          }

          //默认帐户
          if(dwLastUserID==pAccountsInfo->dwUserID)
          {
            ((IListItemUI*)pListLabelElement)->Select(true);
          }
        }
...
bool CDlgLogon::GetInformation()
{
 ...
 /* CEditComboUI * pEditServer = (CEditComboUI *)GetControlByName(szComboServerControlName);
  if(pEditServer!=NULL)
  {*/
    lstrcpyn(m_szLogonServer, szLogonServer,CountArray(m_szLogonServer));
  //}
  ...
  {
    lstrcpyn(m_szAccounts,pEditAccounts->GetText(),CountArray(m_szAccounts));
  }
...
void CDlgLogon::OnBnClickedRegister()
{
  ...
  if(pControl != NULL)
  {
    strLogonServer=pControl->GetText();
  }
  ...
  {
    CMissionLogon::GetInstance()->ShowRegister(false);
  }
