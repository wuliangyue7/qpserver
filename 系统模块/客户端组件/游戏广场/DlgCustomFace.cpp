...
bool CDlgCustomFace::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
  ...
  if((Command.wMainCmdID==MDM_GP_USER_SERVICE)&&(Command.wSubCmdID==SUB_GP_USER_FACE_INFO))
  {
    ...
    if(wDataSize!=sizeof(CMD_GP_UserFaceInfo))
    {
      return false;
    }
    ...
    if(pUserFaceInfo->dwCustomID==0)
    {
      ...
      {
        m_pICustomFaceEvent->OnEventSystemFace(pGlobalUserData->dwUserID,pUserFaceInfo->wFaceID);
      }
    }
    else
    {
      ...
      if(m_pICustomFaceEvent!=NULL)
      {
        m_pICustomFaceEvent->OnEventCustomFace(pGlobalUserData->dwUserID,pUserFaceInfo->dwCustomID,m_CustomFaceInfo);
      }
    }
...
{
  if(m_pTileLayoutSysAvatar == NULL)
  {
    return false;
  }
