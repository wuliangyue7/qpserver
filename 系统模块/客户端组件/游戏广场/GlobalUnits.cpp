...
bool CGlobalUnits::PlayGameSound(LPCTSTR pszSoundName)
{
  ...
  //播放判断
  if(pParameterGlobal->m_bAllowSound==false)
  {
    return false;
  }
...
VOID CGlobalUnits::ClearPropertyConfigInfo()
{
  m_ConfigProperty.cbPropertyCount = 0;
  ZeroMemory(m_ConfigProperty.PropertyInfo,sizeof(m_ConfigProperty.PropertyInfo));
}


VOID CGlobalUnits::AddPropertyConfigItem(const tagPropertyInfo* pInfo, const BYTE cbCount /*= 1*/)
{
  memcpy(m_ConfigProperty.PropertyInfo+m_ConfigProperty.cbPropertyCount, pInfo, sizeof(tagPropertyInfo)*cbCount);
  m_ConfigProperty.cbPropertyCount += cbCount;
}


VOID CGlobalUnits::SetPropertyConfigInfo(const tagPropertyInfo* pInfo, const BYTE cbCount)
{
  m_ConfigProperty.cbPropertyCount = cbCount;
  memcpy(m_ConfigProperty.PropertyInfo, pInfo, sizeof(tagPropertyInfo)*cbCount);
}

const ConfigProperty* CGlobalUnits::GetPropertyConfigInfo()
{
