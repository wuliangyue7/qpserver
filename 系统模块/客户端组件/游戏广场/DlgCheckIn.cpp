...
VOID CDlgCheckIn::UpdateCheckIn()
{
  ...
  if(pButtonUI != NULL)
  {
    pButtonUI->SetEnabled(!m_bTodayChecked);
  }
...
bool CDlgCheckIn::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
  ...
  if(Command.wSubCmdID==SUB_GP_CHECKIN_INFO)
  {
    ...
    if(sizeof(CMD_GP_CheckInInfo)!=wDataSize)
    {
      return false;
    }
  ...
  if(Command.wSubCmdID==SUB_GP_CHECKIN_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GP_CheckInResult))
    {
      return false;
    }
  ...
  if(Command.wSubCmdID==SUB_GP_MEMBER_QUERY_INFO_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GP_MemberQueryInfoResult))
    {
      return false;
    }
  ...
  if(Command.wSubCmdID==SUB_GP_MEMBER_DAY_PRESENT_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GP_MemberDayPresentResult))
    {
      return false;
    }
  ...
  if(Command.wSubCmdID==SUB_GP_MEMBER_DAY_GIFT_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GP_MemberDayGiftResult))
    {
      return false;
    }
...
{
  CDlgMemberGift dlg;
  dlg.SetMember(m_bButtonEnableMemberDayGift);
  dlg.SetData(m_GiftSub,m_wGiftSubCount);
  if(IDOK == dlg.DoModal())
  {
  ...
  if(wMainCmdID==MDM_GR_CHECKIN)
  {
    switch(wSubCmdID)
    {
      case SUB_GR_CHECKIN_INFO:
      {
        ...
        if(sizeof(CMD_GR_CheckInInfo)!=wDataSize)
        {
          return false;
        }
      ...
      case SUB_GR_CHECKIN_RESULT:
      {
        ...
        if(wDataSize>sizeof(CMD_GR_CheckInResult))
        {
          return false;
        }
  ...
  if(wMainCmdID==MDM_GR_MEMBER)
  {
    switch(wSubCmdID)
    {
      case SUB_GP_MEMBER_QUERY_INFO_RESULT:
      {
        ...
        if(wDataSize>sizeof(CMD_GR_MemberQueryInfoResult))
        {
          return false;
        }
      ...
      case SUB_GP_MEMBER_DAY_PRESENT_RESULT:
      {
        ...
        if(wDataSize>sizeof(CMD_GR_MemberDayPresentResult))
        {
          return false;
        }
      ...
      case SUB_GP_MEMBER_DAY_GIFT_RESULT:
      {
        ...
        if(wDataSize>sizeof(CMD_GR_MemberDayGiftResult))
        {
          return false;
        }
