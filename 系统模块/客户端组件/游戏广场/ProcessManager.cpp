...
bool CProcessManager::OnProcessControlData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize, HWND hWndSend)
{
  ...
  if(m_pIProcessManagerSink==NULL)
  {
    return false;
  }
  ...
  if((m_hProcessControl!=NULL)&&(m_hProcessControl!=hWndSend))
  {
    return false;
  }
...
bool CProcessManager::CloseGameProcess(bool bPerformSink)
{
  ...
  if(m_ProcessInfo.hProcess!=NULL)
  {
    ...
    //事件通知
    ASSERT(m_pIProcessManagerSink!=NULL);
    if(bPerformSink==true)
    {
      m_pIProcessManagerSink->OnGameProcessClose(dwExitCode);
    }
  }
...
bool CProcessManager::DetachGameProcess(bool bPerformSink)
{
  ...
  if(m_ProcessInfo.hProcess!=NULL)
  {
    ...
    if(bPerformSink==true)
    {
      m_pIProcessManagerSink->OnGameProcessClose(0L);
    }
  }
...
bool CProcessManager::SendPropertyInfo(tagPropertyInfo PropertyInfo[], BYTE cbPropertyCount)
{
  ...
  const WORD wHeadSize=sizeof(PropertyInfoData)-sizeof(PropertyInfoData.PropertyInfo);
  WORD wDataSize = 0;
  //构造信息
  const int cbMaxPropertyCount = __min(cbPropertyCount,CountArray(PropertyInfoData.PropertyInfo));
  int index = 0;
  for(int i = 0; i < cbMaxPropertyCount; i++)
  {
    PropertyInfoData.PropertyInfo[index++] = PropertyInfo[i];
    PropertyInfoData.cbPropertyCount++;
    wDataSize += sizeof(tagPropertyInfo);
    if(wDataSize >= (IPC_PACKET-sizeof(tagPropertyInfo)))
    {
      //发送数据
...
VOID CProcessManager::OnTimer(UINT_PTR nIDEvent)
{
  ...
  if(nIDEvent==IDI_PROCESS_DETECT)
  {
    ...
    if((m_hProcessControl==NULL)||(IsWindow(m_hProcessControl)==FALSE))
    {
      ...
      if(m_pIProcessManagerSink!=NULL)
      {
        m_pIProcessManagerSink->OnGameProcessError();
      }
    }
