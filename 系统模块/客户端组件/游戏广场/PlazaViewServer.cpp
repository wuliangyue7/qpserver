...
CPlazaViewServer::~CPlazaViewServer()
{
  if(m_ImageGameName.IsNull()==false)
  {
    m_ImageGameName.DestroyImage();
  }
...
BOOL CPlazaViewServer::OnInitDialog()
{
  ...
  //创建组件
  if(m_GameLevelParserModule.CreateInstance()==false)
  {
    throw TEXT("等级解释组件创建失败！");
  }
  if(m_PlazaUserManagerModule.CreateInstance()==false)
  {
    throw TEXT("用户管理组件创建失败！");
  }
  ...
  if(m_PlazaUserManagerModule->SetUserManagerSink(pIUnknownEx)==false)
  {
    throw TEXT("用户管理组件配置失败！");
  }
  if(m_PlazaUserManagerModule->SetUserInformation(pUserInformation)==false)
  {
    throw TEXT("用户管理组件配置失败！");
  }
  ...
  {
    pPlatformEvent->RegisterEventWnd(m_hWnd);
  }
...
BOOL CPlazaViewServer::OnCommand(WPARAM wParam, LPARAM lParam)
{
  ...
  switch(nCommandID)
  {
    ...
    case IDM_DELETE_SERVER_ITEM:  //关闭房间
    {
      ...
      {
        pPlatformFrame->DeleteServerViewItem();
      }
...
VOID CPlazaViewServer::OnLButtonHitTable(WORD wTableID, WORD wChairID)
{
  ...
  {
    pTableUserItem=m_pITableViewFrame->GetClientUserItem(wTableID,wChairID);
  }
  ...
  if(pTableUserItem!=NULL)
  {
    ...
    else
    {
      ...
      if((wMeTableID==wTableID)&&(wMeChairID==wChairID))
      {
        return;
      }
      if((m_wReqTableID!=INVALID_TABLE)||(m_wReqChairID!=INVALID_CHAIR))
...
bool CPlazaViewServer::OnEventBuyProperty(LPCTSTR pszNickName, WORD wItemCount, WORD wPropertyIndex)
{
  ...
  if((pszNickName==NULL)||(wItemCount==0))
  {
    return false;
  }
...
bool CPlazaViewServer::OnGameProcessCreate(bool bAlreadyExist)
{
  ...
  {
    m_ProcessManager.SendMatchInfo(pGameServerItem->m_GameMatch);
  }
  ...
  if((wTableID!=INVALID_TABLE)&&(wChairID!=INVALID_CHAIR))
  {
    ...
    while(true)
    {
      ...
      //发送判断
      if(pIClientUserItem->GetTableID()!=wTableID)
      {
        continue;
      }
...
bool CPlazaViewServer::OnProcessManagerData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  if((wMainCmdID==IPC_CMD_GF_USER_INFO)&&(wSubCmdID==IPC_SUB_GF_USER_ATTRIB))
  {
    ...
    if(wDataSize!=sizeof(IPC_GF_UserAttrib))
    {
      return false;
    }
  ...
  if((wMainCmdID==IPC_CMD_GF_USER_INFO)&&(wSubCmdID==IPC_SUB_GF_KICK_USER))
  {
    ...
    if(wDataSize!=sizeof(IPC_GF_KickUser))
    {
      return false;
    }
  ...
  if((wMainCmdID==IPC_CMD_GF_PROPERTY_INFO))
  {
    ...
    if(wSubCmdID==IPC_SUB_GF_BUY_PROPERTY)
    {
      ...
      if(wDataSize!=sizeof(IPC_GF_BuyProperty))
      {
        return false;
      }
    ...
    if(wSubCmdID==IPC_SUB_GR_PROPERTY_TRUMPET)
    {
      ...
      if(wDataSize!=sizeof(IPC_GR_PropertyTrumpet))
      {
        return false;
      }
...
VOID CPlazaViewServer::OnUserItemDelete(IClientUserItem * pIClientUserItem)
{
  ...
  if((wLastTableID!=INVALID_TABLE)&&(wLastChairID!=INVALID_CHAIR))
  {
    ...
    {
      m_pITableViewFrame->SetClientUserItem(wLastTableID,wLastChairID,NULL);
    }
  ...
  for(INT_PTR i=0; i<m_DlgWhisperItemArray.GetCount(); i++)
  {
    ...
    if(pDlgWhisper->m_hWnd!=NULL)
    {
      pDlgWhisper->OnEventUserLeave(pIClientUserItem);
    }
  }
...
VOID CPlazaViewServer::OnUserFaceUpdate(IClientUserItem * pIClientUserItem)
{
  ...
  for(INT_PTR i=0; i<m_DlgWhisperItemArray.GetCount(); i++)
  {
    ...
    if(pDlgWhisper->m_hWnd!=NULL)
    {
      pDlgWhisper->OnEventUserStatus(pIClientUserItem);
    }
  }
...
VOID CPlazaViewServer::OnUserItemUpdate(IClientUserItem * pIClientUserItem, tagUserScore & LastScore)
{
  ...
  if(pIClientUserItem==m_pIMySelfUserItem)
  {
    ...
    if(pPlatformEvent!=NULL)
    {
      pPlatformEvent->PostPlatformEvent(EVENT_USER_INFO_UPDATE,0L);
    }
  }
...
VOID CPlazaViewServer::SearchGameTable(tagSearchTable & SearchTable)
{
  ...
  for(WORD i=0; i<m_pITableViewFrame->GetTableCount(); i++)
  {
    ...
    if(wMeTableID!=SearchTable.wResultTableID)
    {
      ...
      if((SearchTable.bFilterPass==true)&&(pITableView->GetLockerFlag()==true))
      {
        continue;
      }
      ...
      if(wNullCount>0)
      {
        ...
        //数目判断
        if((SearchTable.bOneNull==true)&&(wNullCount==1))
        {
          return;
        }
        if((SearchTable.bTwoNull==true)&&(wNullCount==2))
        {
          return;
        }
        if((SearchTable.bNotFull==true)&&(wNullCount<wChairUser))
        {
          return;
        }
        if((SearchTable.bAllNull==true)&&(wNullCount==wChairUser))
        {
          return;
        }
      }
...
bool CPlazaViewServer::OnSocketMainConfig(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case SUB_GR_CONFIG_COLUMN:    //列表配置
    {
      ...
      if((wHeadSize+pConfigColumn->cbColumnCount*sizeof(pConfigColumn->ColumnItem[0]))!=wDataSize)
      {
        return false;
      }
    ...
    case SUB_GR_CONFIG_PROPERTY:  //道具配置
    {
      ...
      //保存道具
      memcpy(m_PropertyInfo+m_cbPropertyCount, pGamePropertyItem, sizeof(tagPropertyInfo)*wItemCount);
      m_cbPropertyCount += wItemCount;
    ...
    case SUB_GR_CONFIG_USER_RIGHT:  //配置玩家权限
    {
      ...
      if(wDataSize<sizeof(CMD_GR_ConfigUserRight))
      {
        return false;
      }
      ...
      for(INT_PTR i = 0; i < m_DlgWhisperItemArray.GetCount(); i++)
      {
        ...
        if(pdlg)
        {
          pdlg->SetUserRight(m_dwUserRight);
        }
      }
...
bool CPlazaViewServer::OnSocketMainTask(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  if(m_pDlgTaskServerCenter==NULL)
  {
    return true;
  }
...
bool CPlazaViewServer::OnSocketMainProperty(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    ...
    case SUB_GR_QUERY_PROPERTY_RESULT_FINISH: //查询道具
    {
      if(m_pDlgShopServer)
      {
        return m_pDlgShopServer->OnServerShopMessage(MDM_GR_PROPERTY, wSubCmdID, pData, wDataSize);
      }
    }
...
bool CPlazaViewServer::OnSocketMainManager(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case SUB_GR_OPTION_CURRENT: //当前配置
    {
      ...
      if(wDataSize!=sizeof(CMD_GR_OptionCurrent))
      {
        return false;
      }
...
bool CPlazaViewServer::OnSocketMainMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case SUB_GR_MATCH_FEE:    //费用查询
    {
      ...
      if(wDataSize>sizeof(CMD_GR_Match_Fee))
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_SAFECARD:   //保险卡
    {
      ...
      if(wDataSize>sizeof(CMD_GR_Match_SafeCard))
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_REVIVE:   //复活提醒
    {
      ...
      if(wDataSize>sizeof(CMD_GR_Match_Revive))
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_GOLDUPDATE: //金币更新
    {
      ...
      if(sizeof(CMD_GR_MatchGoldUpdate)!=wDataSize)
      {
        return false;
      }
...
bool CPlazaViewServer::OnSocketSubUserEnter(VOID * pData, WORD wDataSize)
{
  ...
  if(pIClientUserItem==NULL)
  {
    pIClientUserItem=m_PlazaUserManagerModule->ActiveUserItem(UserInfo,CustomFaceInfo,pUserInfoHead->bIsAndroid);
  }
...
bool CPlazaViewServer::OnSocketSubUserItemGameData(VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<=wHeadSize)||(wDataSize!=(wHeadSize+CountStringBuffer(pUserGameData->szUserGameData))))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubLogonFinish(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_LogonFinish))
  {
    return false;
  }
  ...
  if(pPlatformFrame!=NULL)
  {
    pPlatformFrame->ActiveServerViewItem();
  }
...
bool CPlazaViewServer::OnSocketSubUpdateNotify(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_UpdateNotify))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubRequestFailure(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize<=(sizeof(CMD_GR_RequestFailure)-sizeof(pRequestFailure->szDescribeString)))
  {
    return false;
  }
  ...
  if((m_wReqTableID!=INVALID_TABLE)&&(m_wReqChairID!=INVALID_TABLE)&&(m_wReqChairID!=INDEX_ENTER_CHAIR))
  {
    ...
    if(pITableUserItem==m_pIMySelfUserItem)
    {
      m_pITableViewFrame->SetClientUserItem(m_wReqTableID,m_wReqChairID,NULL);
    }
  }
...
bool CPlazaViewServer::OnSocketSubUserChat(VOID * pData, WORD wDataSize)
{
  ...
  //效验参数
  if(wDataSize<(sizeof(CMD_GR_S_UserChat)-sizeof(pUserChat->szChatString)))
  {
    return false;
  }
  if(wDataSize!=(sizeof(CMD_GR_S_UserChat)-sizeof(pUserChat->szChatString)+pUserChat->wChatLength*sizeof(pUserChat->szChatString[0])))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubWisperUserChat(VOID * pData, WORD wDataSize)
{
  ...
  //效验参数
  if(wDataSize<(sizeof(CMD_GR_S_WisperChat)-sizeof(pWisperChat->szChatString)))
  {
    return false;
  }
  if(wDataSize!=(sizeof(CMD_GR_S_WisperChat)-sizeof(pWisperChat->szChatString)+pWisperChat->wChatLength*sizeof(pWisperChat->szChatString[0])))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubWisperExpression(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_S_WisperExpression))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubPropertySuccess(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_S_PropertySuccess))
  {
    return false;
  }
  ...
  //道具提示
  if(pPropertySuccess->wKind != PROP_KIND_GIFT) //礼物类型的道具不提示
  {
    //变量定义
...
bool CPlazaViewServer::OnSocketSubPropertyEffect(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_S_PropertyEffect))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketSubUserInvite(VOID * pData, WORD wDataSize)
{
  ...
  if(m_pITableViewFrame->GetPlayFlag(pUserInvite->wTableID)==true)
  {
    return true;
  }
  ...
  if((pParameterGlobal->m_cbInviteMode==INVITE_MODE_FRIEND)&&(pIUserItem->GetUserCompanion()!=CP_FRIEND))
  {
    return true;
  }
...
bool CPlazaViewServer::OnSocketPropertyMessage(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_S_PropertyMessage))
  {
    return false;
  }
...
bool CPlazaViewServer::OnSocketPropertyTrumpet(VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_S_SendTrumpet))
  {
    return false;
  }
  ...
  if(pSendTrumpet->dwSendUserID==m_pIMySelfUserItem->GetUserID())
  {
    ...
    if(pSendTrumpet->wPropertyIndex==PROPERTY_ID_TRUMPET)
    {
      --pPropertyPackage->wTrumpetCount;
    }
    if(pSendTrumpet->wPropertyIndex==PROPERTY_ID_TYPHON)
    {
      --pPropertyPackage->wTyphonCount;
    }
    //if(m_pTrumpetItem!=NULL) m_pTrumpetItem->UpdateControlSurface();
  }
...
bool CPlazaViewServer::OnSocketSubUserPropBuff(VOID *pData, WORD wDataSize)
{
  ...
  for(int i = 0; i < pPropertyBuff->cbBuffCount; i++)
  {
    ...
    if(pPlatformEvent!=NULL)
    {
      pPlatformEvent->SendPlatformEvent(EVENT_USER_SHOW_UPDATE_BUFF, (LPARAM)&pPropertyBuff->PropertyBuff[i]);
    }
    ...
    if(pIStringMessage)
    {
      pIStringMessage->InsertSystemString(szString);
    }
  }
...
bool CPlazaViewServer::OnSocketSubActionMessage(VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<=wHeadSize)||(wDataSize<(wHeadSize+pActionMessage->wLength*sizeof(WCHAR))))
  {
    return false;
  }
  ...
  while(wExcursion<wDataSize)
  {
    ...
    //效验参数
    if((wExcursion+sizeof(tagActionHead))>wDataSize)
    {
      return false;
    }
    if((wExcursion+sizeof(tagActionHead)+pActionHead->wAppendSize)>wDataSize)
    {
      return false;
    }
  ...
  //关闭房间
  if(pActionMessage->wType&SMT_CLOSE_ROOM)
  {
    PostMessage(WM_COMMAND,IDM_DELETE_SERVER_ITEM,0);
  }
...
VOID CPlazaViewServer::InitServerViewItem(CGameServerItem * pGameServerItem,WORD wAVServerPort,DWORD dwAVServerAddr)
{
  ...
  if(m_GameServer.wServerID==0)
  {
    throw TEXT("很抱歉，此游戏房间已经关闭了，不允许继续进入！");
  }
  ...
  //创建框架
  if(CreateViewFrame()==false)
  {
    throw TEXT("桌子视图框架创建失败！");
  }
...
VOID CPlazaViewServer::WhisperConversation(IClientUserItem * pIClientUserItem[], WORD wUserCount)
{
  ...
  {
    //用户过虑
    if(pIClientUserItem[i]->GetUserID()==0L)
    {
      continue;
    }
  ...
  if(pDlgWhisper!=NULL)
  {
    ...
    if(pPlatformFrame!=NULL)
    {
      pPlatformFrame->RemoveWhisperItem(pDlgWhisper);
    }
  }
...
CDlgWhisper * CPlazaViewServer::CreateWhisperWnd(DWORD dwConversationID,DWORD dwSendUserID,DWORD dwUserID[], WORD wUserCount, bool bCreate)
{
  ...
  for(INT_PTR i=0; i<m_DlgWhisperItemArray.GetCount(); i++)
  {
    //获取窗口
    CDlgWhisper * pDlgWhisper=m_DlgWhisperItemArray[i];
    if((pDlgWhisper->m_hWnd==NULL)||(pDlgWhisper->CompareUserItem(dwUserID,wUserCount)==false))
    {
      continue;
    }
  ...
  for(INT_PTR i=0; i<m_DlgWhisperItemArray.GetCount(); i++)
  {
    //获取窗口
    CDlgWhisper * pDlgWhisper=m_DlgWhisperItemArray[i];
    if((pDlgWhisper->m_hWnd==NULL)||(pDlgWhisper->DisuseEstimate()==false))
    {
      continue;
    }
    ...
    for(WORD i=0; i<wUserCount; i++)
    {
      ...
      if((pIClientUserItem!=NULL)&&(pIClientUserItem!=m_pIMySelfUserItem))
      {
        ClientUserItemArray.Add(pIClientUserItem);
      }
    }
...
bool CPlazaViewServer::SendLogonPacket(LPCTSTR pszServerPasswd)
{
  ...
  LogonUserID.lCode=8623+m_GameKind.wKindID+m_GameServer.wServerID*2;
  LogonUserID.dwUserID=pGlobalUserData->dwUserID+LogonUserID.lCode-1258;
...
bool CPlazaViewServer::PerformLookonAction(WORD wTableID, WORD wChairID)
{
  ...
  if((m_wReqTableID==wTableID)&&(m_wReqChairID==wChairID))
  {
    return false;
  }
...
bool CPlazaViewServer::PerformSitDownAction(WORD wTableID, WORD wChairID, bool bEfficacyPass)
{
  ...
  if((m_wReqTableID!=INVALID_TABLE)&&(m_wReqTableID==wTableID))
  {
    return false;
  }
  if((m_wReqChairID!=INVALID_CHAIR)&&(m_wReqChairID==wChairID))
  {
    return false;
  }
  ...
  switch(pParameterGlobal->m_cbActionHitAutoJoin)
  {
    case ACTION_SEARCH_TABLE:   //查找位置
    {
      ...
      if(m_wFindTableID!=INVALID_CHAIR)
      {
        ...
        {
          PerformSitDownAction(m_wFindTableID,wChairID,true);
        }
      }
      else
      {
...
bool CPlazaViewServer::EfficacyTableRule(WORD wTableID, WORD wChairID, bool bReqLookon, CString & strDescribe)
{
  ...
  if(wTableID>=m_pITableViewFrame->GetTableCount())
  {
    return false;
  }
  if(wChairID>=m_pITableViewFrame->GetChairCount())
  {
    return false;
  }
  ...
  if((bReqLookon==false)&&(m_pIMySelfUserItem->GetMasterOrder()==0))
  {
    ...
    for(WORD i=0; i<m_pITableViewFrame->GetChairCount(); i++)
    {
      ...
      if((pITableUserItem==NULL)||(pITableUserItem==m_pIMySelfUserItem))
      {
        continue;
      }
...
{
  //权限判断
  if(CMasterRight::CanManagerServer(m_dwMasterRight)==false)
  {
    return;
  }
...
{
  //权限判断
  if(CMasterRight::CanKillUser(m_dwMasterRight)==false)
  {
    return;
  }
...
{
  //权限判断
  if(CMasterRight::CanIssueMessage(m_dwMasterRight)==false)
  {
    return;
  }
...
VOID CPlazaViewServer::OnNcDestroy()
{
  ...
  if(m_pDlgTaskServerCenter!=NULL)
  {
    m_pDlgTaskServerCenter->EndDialog(IDOK);
  }
  if(m_pDlgCheckIn!=NULL)
  {
    m_pDlgCheckIn->EndDialog(IDOK);
  }
  if(m_pDlgBaseEnsure!=NULL)
  {
    m_pDlgBaseEnsure->EndDialog(IDOK);
  }
  if(m_pDlgBagServer!=NULL)
  {
    m_pDlgBagServer->EndDialog();
  }
  if(m_pDlgShopServer!=NULL)
  {
    m_pDlgShopServer->EndDialog();
  }

  //销毁窗口
  if(m_pDlgSearchTable && IsWindow(m_pDlgSearchTable->m_hWnd))
  {
    m_pDlgSearchTable->DestroyWindow();
  }
  if(m_pDlgManageUser && IsWindow(m_pDlgManageUser->m_hWnd))
  {
    m_pDlgManageUser->DestroyWindow();
  }
...
//
////右键列表
//VOID CPlazaViewServer::OnNMRclickUserList(NMHDR * pNMHDR, LRESULT * pResult)
//{
//  //变量定义
...
//
////双击列表
//VOID CPlazaViewServer::OnNMDblclkUserList(NMHDR * pNMHDR, LRESULT * pResult)
//{
//  //变量定义
//  NMITEMACTIVATE * pListNotify=(NMITEMACTIVATE *)pNMHDR;
//
...
//            {
//              m_pITableViewFrame->FlashGameTable(wTableID);
//            }
//            else
//            {
//              m_pITableViewFrame->FlashGameChair(wTableID,wChairID);
//            }
//          }
//
//          break;
...
//
////双击列表
//VOID CPlazaViewServer::OnNMClickUserList(NMHDR * pNMHDR, LRESULT * pResult)
//{
//  //变量定义
//  NMITEMACTIVATE * pListNotify=(NMITEMACTIVATE *)pNMHDR;
//
