  ...
  if(Command.wMainCmdID==MDM_GP_USER_SERVICE)
  {
    switch(Command.wSubCmdID)
    {
      case SUB_GP_GROWLEVEL_PARAMETER:  //等级参数
      {
        ...
        if(wDataSize!=sizeof(CMD_GP_GrowLevelParameter))
        {
          return false;
        }
      ...
      case SUB_GP_GROWLEVEL_UPGRADE:    //等级升级
      {
        ...
        if(wDataSize<CountStringBuffer(pGrowLevelUpgrade->szNotifyContent))
        {
          return false;
        }
      ...
      case SUB_GP_USER_INDIVIDUAL:  //个人信息
      {
        ...
        if(pIndividualUserData->szCompellation[0] ==0 || pIndividualUserData->szPassPortID[0] == 0)
        {
          ...
          if(pPlatformEvent!=NULL)
          {
            pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_REAL_TIP,0);
          }
...
bool CPlazaUserInfo::Notify(TNotifyUI &  msg)
{
  ...
  if(lstrcmp(msg.sType, TEXT("click")) == 0)
  {
    ...
    else if(lstrcmp(pControlUI->GetName(), szRealTipButtonEnterControlName)==0)
    {
      ...
      if(pPlatformEvent!=NULL)
      {
        pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_PERSONER,3L);
      }
...
{
  if(IDI_CHECK_USER_BUFF == nIDEvent)
  {
    UpdateBuffUI();
  }
  else if(IDI_CHECK_SEND_PRESENT == nIDEvent)
  {
    AvtiveQuerySendPresentMission();
  }
}
