...
BOOL CDlgInsure::OnInitDialog()
{
  ...
  //查询银行信息
  if(CGlobalUserInfo::GetInstance()->GetGlobalUserData()->cbInsureEnabled==TRUE)
  {
    QueryUserInsureInfo();
  }
...
void CDlgInsure::Notify(TNotifyUI &  msg)
{
  ...
  if(msg.sType == TEXT("click"))
  {
    ...
    else if(pControlUI->GetName()== szButtonGetScoreControlName)
    {
      ...
      if(pTabLayoutUI == NULL || pOptionUI == NULL)
      {
        return;
      }
  ...
  else if(lstrcmp(msg.sType, TEXT("selectchanged")) == 0)
  {
    ...
    if(bShow)
    {
      ...
      if(IDOK ==Information.ShowMessageBox(TEXT("初次使用，请先开通银行！"),MB_OKCANCEL|MB_ICONINFORMATION))
      {
        ...
        {
          pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_PERSONER,2L);
        }
        return OnCancel();
      }
    }
  ...
  if(m_pItemWealth)
  {
    m_pItemWealth->Notify(msg);
  }
  if(m_pItemAccess)
  {
    m_pItemAccess->Notify(msg);
  }
  if(m_pItemPresent)
  {
    m_pItemPresent->Notify(msg);
  }
  return;
}
  ...
  if(Command.wMainCmdID==MDM_GP_USER_SERVICE)
  {
    switch(Command.wSubCmdID)
    {
      ...
      case SUB_GP_USER_INSURE_ENABLE_RESULT:  //开通结果
      {
        ...
        if(wDataSize>sizeof(CMD_GP_UserInsureEnableResult))
        {
          return false;
        }
      ...
      case SUB_GP_QUERY_USER_INFO_RESULT: //用户信息
      {
        ...
        if(wDataSize<sizeof(CMD_GP_UserTransferUserInfo))
        {
          return false;
        }
...
VOID CDlgInsure::InitUI()
{
  ...
  if(m_pItemWealth==NULL)
  {
    m_pItemWealth = new CInsureItemWealth(this);
  }
  if(m_pItemAccess==NULL)
  {
    m_pItemAccess = new CInsureItemAccess(this);
  }
  if(m_pItemPresent==NULL)
  {
    m_pItemPresent = new CInsureItemPresent(this);
  }

  if(m_pItemWealth)
  {
    m_pItemWealth->InitUI();
  }
  if(m_pItemAccess)
  {
    m_pItemAccess->InitUI();
  }
  if(m_pItemPresent)
  {
    m_pItemPresent->InitUI();
  }
...
VOID CDlgInsure::UpdateUI()
{
  if(m_pItemWealth)
  {
    m_pItemWealth->UpdateUI();
  }
  if(m_pItemAccess)
  {
    m_pItemAccess->UpdateUI();
  }
  if(m_pItemPresent)
  {
    m_pItemPresent->UpdateUI();
  }
...
VOID CInsureItemWealth::UpdateUI()
{
  ...
  //魅力值
  SetNumber(m_pContainerUI, szNumberGlamourValueControlName, (double)pGlobalUserData->dwLoveLiness);
...
void CInsureItemWealth::Notify(TNotifyUI &  msg)
{
  ...
  if(msg.sType == TEXT("click"))
  {
            ...
            || pControlUI->GetName()== szButtonGetBaseEnsureControlName)
    {
      ...
      if(pPlatformEvent!=NULL)
      {
        pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_BASE_ENSURE,0L);
      }
            ...
            || pControlUI->GetName()== szButtonExchargeControlName2)
    {
      ...
      if(pPlatformEvent!=NULL)
      {
        pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_SHOP,0L);
      }
...
VOID CInsureItemAccess::ShowCapital()
{
  ...
  if(m_LabelInOut != NULL)
  {
    m_LabelInOut->SetText(szBuffer);
  }
}
...
void CInsureItemPresent::Notify(TNotifyUI &  msg)
{
  ...
  else if(msg.sType == TEXT("textchanged"))
  {
    if(pControlUI->GetName() == szEditPresentAmountControlName)
    {
      ...
      if(m_pLabelChineseNumeral != NULL)
      {
        m_pLabelChineseNumeral->SetText(szBuffer);
      }
    }
  }
}
  ...
  if(wMainCmdID==MDM_GR_INSURE)
  {
    switch(wSubCmdID)
    {
      ...
      case SUB_GR_USER_INSURE_ENABLE_RESULT:  //开通结果
      {
        ...
        if(wDataSize>sizeof(CMD_GR_S_UserInsureEnableResult))
        {
          return false;
        }
      ...
      case SUB_GR_USER_TRANSFER_USER_INFO:  //用户信息
      {
        ...
        if(wDataSize<sizeof(CMD_GR_S_UserTransferUserInfo))
        {
          return false;
        }
