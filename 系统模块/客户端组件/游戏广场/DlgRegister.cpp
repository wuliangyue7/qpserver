...
WORD CDlgRegister::ConstructRegisterPacket(BYTE cbBuffer[], WORD wBufferSize, BYTE cbValidateFlags)
{
  ...
  if(m_bVerifyIndividual==false)
  {
    ...
    ZeroMemory(pRegisterAccounts->szDataEncrypt,sizeof(pRegisterAccounts->szDataEncrypt));
    _sntprintf(szResPassword,sizeof(szResPassword),TEXT("%s68y56p99@QTc.DV*F"),pRegisterAccounts->szAccounts);
    CWHEncrypt::MD5Encrypt(szResPassword,pRegisterAccounts->szDataEncrypt);
    ...
    {
      pGlobalWebLink->GetSessionKey(pRegisterAccounts->szSessionKey);
    }
    //发送数据
...
void CDlgRegister::OnVerifypIndividualResult(WORD wVerifyMask,bool bVerifyPassage,LPCTSTR pszErrorMsg)
{
  if(bVerifyPassage==true)
  {
    ...
    if(wVerifyMask==VERIFY_ACCOUNTS)
    {
      ...
      if(pLabelAccountValidate!=NULL)
      {
        pLabelAccountValidate->SetVisible(true);
      }
    ...
    if(wVerifyMask==VERIFY_NICKNAME)
    {
      ...
      if(pLabelNickNameValidate!=NULL)
      {
        pLabelNickNameValidate->SetVisible(true);
      }
...
bool CDlgRegister::GetInformation()
{
  ...
  if(pEditAccountsUI!=NULL)
  {
    GetControlString(pEditAccountsUI,m_szAccounts,CountArray(m_szAccounts));
  }
  ...
  //读取推荐人
  ZeroMemory(m_szSpreader,CountArray(m_szSpreader));
  GetPrivateProfileString(TEXT("SpreaderInfo"),TEXT("SpreaderName"),TEXT(""),m_szSpreader,CountArray(m_szSpreader),szFileName);

  //登录密码
  CControlUI *  pEditLogonPassUI = GetControlByName(szEditLogonPassControlName);
  if(pEditLogonPassUI!=NULL)
  {
    GetControlString(pEditLogonPassUI,m_szLogonPass,CountArray(m_szLogonPass));
  }
  ...
  if(pEditCompellationUI!=NULL)
  {
    GetControlString(pEditCompellationUI,m_szCompellation,CountArray(m_szCompellation));
  }

  //验证码
  TCHAR szValidateCode[5]=TEXT("");
  CControlUI * pEditValidateCodeUI = GetControlByName(szEditValidateCodeControlName);
  if(pEditValidateCodeUI!=NULL)
  {
    GetControlString(pEditValidateCodeUI,szValidateCode,CountArray(szValidateCode));
  }
...
void CDlgRegister::SelectedTipInfo(WORD wPreVerifyMask)
{
  ...
  if(wPreVerifyMask==PERFORM_VERIFY_ACCOUNTS)
  {
    ...
    if(pLabelAccountValidate!=NULL)
    {
      pLabelAccountValidate->SetVisible(false);
    }

    CControlUI * pLabelAccountInvalid = GetControlByName(szLabelAccountsInvalidControlName);
    if(pLabelAccountInvalid!=NULL)
    {
      pLabelAccountInvalid->SetVisible(false);
    }
  ...
  else if(wPreVerifyMask==PERFORM_VERIFY_NICKNAME)
  {
    ...
    if(pLabelNickNameValidate!=NULL)
    {
      pLabelNickNameValidate->SetVisible(false);
    }

    CControlUI * pLabelNickNameInvalid = GetControlByName(szLabelNickNameInvalidControlName);
    if(pLabelNickNameInvalid!=NULL)
    {
      pLabelNickNameInvalid->SetVisible(false);
    }
...
void CDlgRegister::PerformVerifyIndividual(WORD wPreVerifyMask)
{
  ...
  if(wPreVerifyMask==PERFORM_VERIFY_ACCOUNTS)
  {
    ...
    if(pLabelAccountTip!=NULL)
    {
      pLabelAccountTip->SetVisible(false);
    }
    ...
    if(pEditAccountsUI!=NULL)
    {
      GetControlString(pEditAccountsUI,szAccounts,CountArray(szAccounts));
    }
    if(lstrlen(szAccounts) ==0)
    {
      return;
    }
  ...
  if(wPreVerifyMask==PERFORM_VERIFY_NICKNAME)
  {
    ...
    if(LabelNickNameTip!=NULL)
    {
      LabelNickNameTip->SetVisible(false);
    }
    ...
    //用户帐号
    CControlUI * pEditNickNameUI = GetControlByName(szEditNickNameControlName);
    if(pEditNickNameUI!=NULL)
    {
      GetControlString(pEditNickNameUI,szNickName,CountArray(szNickName));
    }
    if(lstrlen(szNickName) ==0)
    {
      return;
    }
  ...
  if(wPreVerifyMask==PERFORM_VERIFY_PASSWORD)
  {
    ...
    if(LabelLogonPassTip!=NULL)
    {
      LabelLogonPassTip->SetVisible(false);
    }
    ...
    if(pEditLogonPassUI!=NULL)
    {
      GetControlString(pEditLogonPassUI,szLogonPass,CountArray(szLogonPass));
    }

    if(lstrlen(szLogonPass) ==0)
    {
      return;
    }
    ...
    if(pLabelLogonPassValidate!=NULL)
    {
      pLabelLogonPassValidate->SetVisible(true);
    }
  ...
  if(wPreVerifyMask==PERFORM_VERIFY_TWICE)
  {
    ...
    if(LabelLogonPassTip1!=NULL)
    {
      LabelLogonPassTip1->SetVisible(false);
    }
    ...
    CControlUI * pEditLogonPass1UI = GetControlByName(szEditLogonPass1ControlName);
    if(pEditLogonPass1UI!=NULL)
    {
      GetControlString(pEditLogonPass1UI,szLogonPass1,CountArray(szLogonPass1));
    }

    if(lstrlen(szLogonPass) ==0 && lstrlen(szLogonPass1) ==0)
    {
      return;
    }
    //密码判断
    ...
    if(pLabelLogonPassValidate1!=NULL)
    {
      pLabelLogonPassValidate1->SetVisible(true);
    }
  }
}
...
void CDlgRegister::OnEnChangeLogonPass()
{
  ...
  if(pEditLogonPass!=NULL)
  {
    lstrcpyn(szPassword,pEditLogonPass->GetText(),CountArray(szPassword));
  }
