...
Node* CTaskUI::AddNode(const tagTaskListItemInfo& item, Node* parent)
{
  ...
  if(!parent->Data().bChildVisible)
  {
    pListElement->SetVisible(false);
  }
  if(parent != m_pRootNode && !parent->Data().plistElment->IsVisible())
  {
    pListElement->SetVisible(false);
  }
  ...
  {
    if(parent == m_pRootNode)
    {
      index = 0;
    }
    else
    {
      index = parent->Data().plistElment->GetIndex() + 1;
    }
  }
...
{
  if(Parent==NULL || Parent == m_pRootNode)
  {
    return NULL;
  }
