...
BOOL CDlgServerManager::OnInitDialog()
{
  ...
  {
    m_pITCPSocket->SendData(MDM_GR_MANAGE,SUB_GR_QUERY_OPTION);
  }
...
VOID CDlgServerManager::OnOK()
{
  ...
  //库银衰减
  ServerOption.ServerOptionInfo.lStorageStart = GetDlgItemInt(IDC_STORAGE_START);
  ServerOption.ServerOptionInfo.lStorageCurrent = GetDlgItemInt(IDC_STORAGE_CURRENT);
  ServerOption.ServerOptionInfo.lStorageMax = GetDlgItemInt(IDC_STORAGE_MAX);
  ServerOption.ServerOptionInfo.nStorageDeduct = GetDlgItemInt(IDC_STORAGE_DEDUCT);
  //发送数据
  ASSERT(m_pITCPSocket!=NULL);
...
{
  //挂接配置
  if(ServerOptionInfo.wNodeID!=0)
  {
    SetDlgItemInt(IDC_NODE_ID,ServerOptionInfo.wNodeID);
  }
  if(ServerOptionInfo.wSortID!=0)
  {
    SetDlgItemInt(IDC_SORT_ID,ServerOptionInfo.wSortID);
  }
  if(ServerOptionInfo.wKindID!=0)
  {
    SetDlgItemInt(IDC_KIND_ID,ServerOptionInfo.wKindID);
  }

  //房间属性
  if(ServerOptionInfo.szServerName[0]!=0)
  {
    SetDlgItemText(IDC_SERVER_NAME,ServerOptionInfo.szServerName);
  }
  if(ServerOptionInfo.lRestrictScore!=0)
  {
    SetDlgItemInt(IDC_RESTRICT_SCORE,(LONG)ServerOptionInfo.lRestrictScore);
  }
  ...
  if(ServerOptionInfo.lMinTableScore!=0)
  {
    SetDlgItemInt(IDC_MIN_TABLE_SCORE,(LONG)ServerOptionInfo.lMinTableScore);
  }
  ...
  //库银衰减
  SetDlgItemInt(IDC_STORAGE_START,ServerOptionInfo.lStorageStart);
  SetDlgItemInt(IDC_STORAGE_CURRENT,ServerOptionInfo.lStorageCurrent);
  SetDlgItemInt(IDC_STORAGE_MAX,ServerOptionInfo.lStorageMax);
  SetDlgItemInt(IDC_STORAGE_DEDUCT,ServerOptionInfo.nStorageDeduct);
