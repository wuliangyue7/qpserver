...
bool CDownLoadTaskSink::OnDownLoadStatus(enDownLoadStatus DownLoadStatus)
{
  ...
  switch(DownLoadStatus)
  {
    ...
    case DownLoadStatus_Conclude:   //完成状态
    {
      ...
      if(m_pUpdateMessage!=NULL)
      {
        m_pUpdateMessage->SendMessage(WM_DOWNLOAD_TASK_FINISH,TRUE);
      }
...
bool CDownLoadTaskSink::OnDataStream(const VOID * pcbMailData, WORD wStreamSize)
{
  ...
  if(m_pUpdateMessage!=NULL)
  {
    m_pUpdateMessage->SendMessage(WM_DOWNLOAD_TASK_UPDATE,m_dwDownLoadSize==m_dwTotalFileSize);
  }
  ...
  if(m_pUpdateMessage!=NULL)
  {
    m_pUpdateMessage->SendMessage(WM_UI_UPDATEDOWNLOAD,(WPARAM)&UIInfo);
  }
...
bool CDownLoadTaskSink::OnDataInformation(bool bPartialConent,DWORD dwTotalFileSize, LPCTSTR pszEntityTag, LPCTSTR pszLocation)
{
  ...
  if(m_pUpdateMessage!=NULL)
  {
    m_pUpdateMessage->SendMessage(WM_DOWNLOAD_TASK_UPDATE);
  }
...
bool CDownLoadTaskSink::CreateDownLoadFile(LPCTSTR pszLocation)
{
  ...
  while((dwIndex>0)&&(szLocationDirectory[dwIndex]!=TEXT('\\')))
  {
    dwIndex--;
  }
  ...
  //创建文件
  if(m_DownFile.Open(szLocationPath,CFile::modeCreate|CFile::modeWrite)==FALSE)
  {
    return false;
  }
...
bool CUpdateCheckSink::OnDownLoadError(enDownLoadError DownLoadError)
{
  ...
  if(m_pUpdateMessage!=NULL)
  {
    m_pUpdateMessage->SendMessage(WM_UPDATECHECK_ERROR);
  }
...
VOID CUpdateCheckSink::PerformCheckSum(LPVOID pThreadData)
{
  ...
  //判断状态
  if(pUpdateCheckSink->m_bCheckData == false)
  {
    return;
  }
  ...
  try
  {
    ...
    if(dwUnCompressSize==0)
    {
      ...
      {
        pUpdateMessage->PostMessage(WM_UPDATECHECK_FINISH);
      }
