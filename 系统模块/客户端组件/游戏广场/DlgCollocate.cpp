...
bool CCollocateBar::SaveParameter()
{
  ...
  if(m_wItemActive!=INVALID_WORD)
  {
    ...
    if(pCollocateItem->SaveParameter()==false)
    {
      return false;
    }
  }
...
bool CCollocateBar::SetItemCreateRect(CRect rcItemCreate)
{
  ...
  for(INT_PTR i=0; i<m_CollocateItemArray.GetCount(); i++)
  {
    ...
    if(pWnd->m_hWnd!=NULL)
    {
      pWnd->SetWindowPos(NULL,rcItemCreate.left,rcItemCreate.top,rcItemCreate.Width(),rcItemCreate.Height(),SWP_NOZORDER);
    }
  }
...
bool CCollocateBar::SetActiveCollocateItem(CCollocateItem * pCollocateItem)
{
  ...
  if(m_wItemActive!=INVALID_WORD)
  {
    ...
    //切换判断
    if(pCollocateActive==pCollocateItem)
    {
      return true;
    }
    if(pCollocateActive->SaveParameter()==false)
    {
      return false;
    }
  }
...
WORD CCollocateBar::SwitchToButtonIndex(CPoint MousePoint)
{
  ...
  //边界判断
  if(MousePoint.x<((rcClient.Width()-m_SizeButton.cx)/2))
  {
    return INVALID_WORD;
  }
  if(MousePoint.x>((rcClient.Width()+m_SizeButton.cx)/2))
  ...
  for(WORD i=0; i<m_CollocateItemArray.GetCount(); i++)
  {
    ...
    //区域判断
    if(rcButton.PtInRect(MousePoint))
    {
      return i;
    }
  }
...
VOID CCollocateBar::OnPaint()
{
  ...
  for(INT i=0; i<m_CollocateItemArray.GetCount(); i++)
  {
    ...
    //横行计算
    if((m_wItemHover==i)&&(m_wItemDown!=i))
    {
      nXButtomPos=m_SizeButton.cx*2;
    }
    if((m_wItemActive==i)||(m_wItemDown==i))
    {
      nXButtomPos=m_SizeButton.cx;
    }
...
BOOL CCollocateProxy::OnInitDialog()
{
  ...
  for(INT i=0; i<CountArray(szProxyType); i++)
  {
    ...
    if(m_cbProxyType==cbProxyType[i])
    {
      m_cmProxyType.SetCurSel(nItem);
    }
  }
  ...
  //设置选择
  if(m_cmProxyType.GetCurSel()==CB_ERR)
  {
...
{
  //聊天设置
  if(IsDlgButtonChecked(IDC_MESSAGE_SHOW_ALL)==BST_CHECKED)
  {
    m_cbMessageMode=INVITE_MODE_ALL;
  }
  if(IsDlgButtonChecked(IDC_MESSAGE_HIDE_DETEST)==BST_CHECKED)
  {
    m_cbMessageMode=MESSAGE_MODE_DETEST;
  }
  if(IsDlgButtonChecked(IDC_MESSAGE_ONLY_FRIEND)==BST_CHECKED)
  {
    m_cbMessageMode=MESSAGE_MODE_FRIEND;
  }

  //邀请设置
  if(IsDlgButtonChecked(IDC_INVITE_ALL)==BST_CHECKED)
  {
    m_cbInviteMode=INVITE_MODE_ALL;
  }
  if(IsDlgButtonChecked(IDC_INVITE_NONE)==BST_CHECKED)
  {
    m_cbInviteMode=INVITE_MODE_NONE;
  }
  if(IsDlgButtonChecked(IDC_INVITE_ONLY_FRIEND)==BST_CHECKED)
  {
    m_cbInviteMode=INVITE_MODE_FRIEND;
  }
...
{
  //双击列表
  if(IsDlgButtonChecked(IDC_ACTION_ORIENTATION)==BST_CHECKED)
  {
    m_cbActionLeftDoubleList=ACTION_ORIENTATION;
  }
  if(IsDlgButtonChecked(IDC_ACTION_SEND_WHISPER)==BST_CHECKED)
  {
    m_cbActionLeftDoubleList=ACTION_SEND_WHISPER;
  }
  if(IsDlgButtonChecked(IDC_ACTION_SHOW_USER_INFO)==BST_CHECKED)
  {
    m_cbActionLeftDoubleList=ACTION_SHOW_USER_INFO;
  }

  //自动加入
  if(IsDlgButtonChecked(IDC_ACTION_SEARCH_TABLE)==BST_CHECKED)
  {
    m_cbActionHitAutoJoin=ACTION_SEARCH_TABLE;
  }
  if(IsDlgButtonChecked(IDC_SHOW_SEARCH_TABLE_DLG)==BST_CHECKED)
  {
    m_cbActionHitAutoJoin=ACTION_SHOW_SEARCH_DLG;
  }

  //发送私聊
  if(IsDlgButtonChecked(IDC_SNED_WHISPER_BY_ENTER)==BST_CHECKED)
  {
    m_bSendWhisperByEnter=true;
  }
  if(IsDlgButtonChecked(IDC_SNED_WHISPER_BY_CTRL_ENTER)==BST_CHECKED)
  {
    m_bSendWhisperByEnter=false;
  }
...
{
  //保存参数
  if(m_CollocateBar.SaveParameter()==false)
  {
    return;
  }
