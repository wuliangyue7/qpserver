...
bool CDlgCollocateRoom::SaveParameter()
{
  ...
  {
    lstrcpyn(m_szTablePassword,pEditPass1Control->GetText(),CountArray(m_szTablePassword));
  }

  //设置密码
  if(m_szTablePassword[0]!=0)
  {
    m_bTakePassword = true;
  }
...
bool CDlgCollocateRoom::UpdateControlStatus()
{
  ...
  if(pCheckOption)
  {
    pCheckOption->SetCheck(m_bLimitWinRate);
  }
  ...
  if(pCheckOption)
  {
    pCheckOption->SetCheck(m_bLimitFleeRate);
  }
  ...
  if(pCheckOption)
  {
    pCheckOption->SetCheck(m_bLimitGameScore);
  }
  ...
  if(pCheckOption)
  {
    pCheckOption->SetCheck(m_bLimitDetest);
  }
  ...
  if(pCheckOption)
  {
    pCheckOption->SetCheck(m_bLimitSameIP);
  }
