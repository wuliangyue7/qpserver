...
{
  if(m_pPlazaViewContainer==NULL)
  {
    m_pPlazaViewContainer=this;
  }
  return;
}
...
CPlazaViewContainer::~CPlazaViewContainer()
{
  if(m_pPlazaViewContainer==this)
  {
    m_pPlazaViewContainer=NULL;
  }
...
BOOL CPlazaViewContainer::InsertViewItem(CWnd * pViewWnd,emViewItemArea ItemViewArea)
{
  ...
  if(wViewItemIndex!=INVALID_WORD)
  {
    ...
    {
      if(pViewItemInfo->pItemView==pViewWnd)
      {
        return TRUE;
      }
      pViewItemInfo->pItemView->ShowWindow(SW_HIDE);
...
{
  if(pViewItemInfo==NULL || pViewItemInfo->pItemView==NULL)
  {
    return FALSE;
  }
