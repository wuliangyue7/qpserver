...
void CDlgFriend::InitControlUI()
{
  ...
  if(m_pButtonJionGame)
  {
    m_pButtonJionGame->SetEnabled(bEnable);
  }
  if(m_pButtonWatchGame)
  {
    m_pButtonWatchGame->SetEnabled(bEnable);
  }
  if(m_pButtonChat)
  {
    m_pButtonChat->SetEnabled(bEnable);
  }
...
void CDlgFriend::UpdateFriendInfo(const tagUserRemoteInfo& UserInfo)
{
  ...
  if(m_pLabelFriendGameID)
  {
    m_pLabelFriendGameID->SetText(szGameID);
  }
  ...
  if(m_pLabelFriendInGame)
  {
    m_pLabelFriendInGame->SetText(tKindName.c_str());
  }
  ...
  if(m_pLabelFriendInRoom)
  {
    m_pLabelFriendInRoom->SetText(szGameServer);
  }
  ...
  if(m_pButtonJionGame)
  {
    m_pButtonJionGame->SetEnabled(bCanJoinGame);
  }
  if(m_pButtonWatchGame)
  {
    m_pButtonWatchGame->SetEnabled(bCanLookOnGame);
  }
  if(m_pButtonChat)
  {
    m_pButtonChat->SetEnabled(bCanChat);
  }
...
bool CDlgFriend::JoinGame(const WORD dwTableID, const WORD dwChairID)
{
  ...
  if(pPlazaViewServer)
  {
    pPlazaViewServer->PerformSitDownAction(dwTableID,dwChairID,false);
  }
  return true;
}
...
bool CDlgFriend::LookOnGame(const WORD dwTableID, const WORD dwChairID)
{
  ...
  if(pPlazaViewServer)
  {
    pPlazaViewServer->PerformLookonAction(dwTableID, dwChairID);
  }
  return true;
}
