...
BOOL CDlgProxyConfig::OnInitDialog()
{
  ...
  for(INT i=0; i<CountArray(szProxyType); i++)
  {
    ...
    if(pParameterGlobal->m_cbProxyType==cbProxyType[i])
    {
  ...
  if(pParameterGlobal->m_ProxyServer.wProxyPort!=0)
  {
    SetDlgItemInt(IDC_PROXY_PORT,pParameterGlobal->m_ProxyServer.wProxyPort);
  }
