  ...
  switch(uMessage)
  {
    ...
    case WM_LBUTTONDBLCLK:    //鼠标消息
    {
      ...
      if((hTreeItem!=NULL)&&(GetChildItem(hTreeItem)!=NULL))
      {
        ...
        CRect rcTreeItem;
        GetItemRect(hTreeItem,&rcTreeItem,TRUE);
        if(rcTreeItem.PtInRect(MousePoint)==FALSE)
        {
          break;
        }
...
VOID CServerListView::OnGameKindFinish(WORD wKindID)
{
  ...
  if(pGameKindItem!=NULL)
  {
    ...
    //更新标题
    if(hItemAttachUpdate!=NULL)
    {
      SetItemText(hItemAttachUpdate,pszTitle);
    }
    if(hItemNormalUpdate!=NULL)
    {
      SetItemText(hItemNormalUpdate,pszTitle);
    }
  }
  ...
  switch(pGameListItem->GetItemGenre())
  {
    case ItemGenre_Type:  //游戏种类
    {
      ...
      if(pParentListItem!=NULL)
      {
        ...
        {
          pGameTypeItem->m_hTreeItemAttach=InsertGameListItem(pszTypeName,IND_TYPE,pGameTypeItem,hTreeItemAttach);
        }
      }
      else
      {
      ...
      {
        SetItemState(pGameTypeItem->m_hTreeItemNormal,TVIS_BOLD,TVIS_BOLD);
      }
      if(pGameTypeItem->m_hTreeItemAttach!=NULL)
      {
        SetItemState(pGameTypeItem->m_hTreeItemAttach,TVIS_BOLD,TVIS_BOLD);
      }
    ...
    case ItemGenre_Node:  //游戏节点
    {
      ...
      if(pParentListItem!=NULL)
      {
        ...
        if(hTreeItemNormal!=NULL)
        {
          pGameNodeItem->m_hTreeItemNormal=InsertGameListItem(pszNodeName,IND_NODE,pGameNodeItem,hTreeItemNormal);
        }
        ...
        if(hTreeItemAttach!=NULL)
        {
          pGameNodeItem->m_hTreeItemAttach=InsertGameListItem(pszNodeName,IND_NODE,pGameNodeItem,hTreeItemAttach);
        }
      }
    ...
    case ItemGenre_Page:  //定制子项
    {
      ...
      if(pParentListItem!=NULL)
      {
        ...
        if(hTreeItemAttach!=NULL)
        {
          pGameListItem->m_hTreeItemAttach=InsertGameListItem(pszDisplayName,IND_BROWSE,pGameListItem,hTreeItemAttach);
        }
      }
      else
      {
    ...
    case ItemGenre_Server:  //游戏房间
    {
      ...
      if(pParentListItem!=NULL)
      {
        ...
        if(hTreeItemNormal!=NULL)
        {
          pGameServerItem->m_hTreeItemNormal=InsertGameListItem(szTitle,uImageIndex,pGameServerItem,hTreeItemNormal);
        }
        ...
        if(hTreeItemAttach!=NULL)
        {
          pGameServerItem->m_hTreeItemAttach=InsertGameListItem(szTitle,uImageIndex,pGameServerItem,hTreeItemAttach);
        }
...
VOID CServerListView::OnGameItemDelete(CGameListItem * pGameListItem)
{
  ...
  if(pGameListItem->m_hTreeItemAttach!=NULL)
  {
    DeleteItem(pGameListItem->m_hTreeItemAttach);
  }
  if(pGameListItem->m_hTreeItemNormal!=NULL)
  {
    DeleteItem(pGameListItem->m_hTreeItemNormal);
  }
...
VOID CServerListView::DeleteUpdateItem(CGameListItem * pGameListItem)
{
  ...
  while(pGameListItem!=NULL)
  {
    ...
    if(pGameListItem->GetItemGenre()==ItemGenre_Kind)
    {
      //删除子项
      CGameKindItem * pGameKindItem=(CGameKindItem *)pGameListItem;
      if(pGameKindItem->m_hItemAttachUpdate!=NULL)
      {
        DeleteItem(pGameKindItem->m_hItemAttachUpdate);
      }
      if(pGameKindItem->m_hItemNormalUpdate!=NULL)
      {
        DeleteItem(pGameKindItem->m_hItemNormalUpdate);
      }
...
HTREEITEM CServerListView::InsertInsideItem(LPCTSTR pszTitle, UINT uImage, DWORD dwInsideID, HTREEITEM hParentItem)
{
  ...
  if(pGameInsideItem!=NULL)
  {
    pGameInsideItem->m_dwInsideID=dwInsideID;
  }
...
VOID CServerListView::OnNMLClick(NMHDR * pNMHDR, LRESULT * pResult)
{
  ...
  if((pGameListItem!=NULL)&&(pGameListItem->GetItemGenre()==ItemGenre_Inside))
  {
    ...
    if(szAssistantLink[0]!=0)
    {
      ...
      {
        pPlatformFrame->WebBrowse(szAssistantLink,false);
      }
    }
  ...
  if(pGameListItem!=NULL)
  {
    switch(pGameListItem->GetItemGenre())
    {
      case ItemGenre_Kind:  //游戏种类
      {
        ...
        if(pPlatformFrame!=NULL)
        {
          pPlatformFrame->WebBrowse(szRuleLink,false);
        }
      ...
      case ItemGenre_Page:  //定制类型
      {
        ...
        if(pPlatformFrame!=NULL)
        {
          pPlatformFrame->WebBrowse(szPageLink,false);
        }
...
{
  if(pGameServer == NULL || pszBuffer == NULL)
  {
    return NULL;
  }
...
{
  if(pGameKind == NULL || pszBuffer == NULL)
  {
    return NULL;
  }
