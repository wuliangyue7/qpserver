  ...
  if(Command.wMainCmdID==MDM_GP_USER_SERVICE)
  {
    switch(Command.wSubCmdID)
    {
      ...
      case SUB_GP_INDIVIDUAL_RESULT:
      {
        ...
        if(wDataSize<(sizeof(CMD_GP_IndividuaResult)-sizeof(pIndividualResult->szNotifyContent)))
        {
          return false;
        }
      ...
      case SUB_GP_USER_INDIVIDUAL:  //个人信息
      {
        ...
        if(wDataSize<sizeof(CMD_GP_UserIndividual))
        {
          return false;
        }
...
void CDlgIndividual::OnMissionLinkIndividualModify()
{
  ...
  if(m_cbSelectedIndex ==0)
  {
    ...
    if(pManButtonUI && pManButtonUI->GetCheck())
    {
      pModifyIndividual->cbGender=GENDER_MANKIND;
    }
    if(pFemaleButtonUI && pFemaleButtonUI->GetCheck())
    {
      pModifyIndividual->cbGender=GENDER_FEMALE;
    }
...
void CDlgIndividual::UpdateData()
{
  ...
  if(m_cbSelectedIndex == 0)
  {
    ...
    if(pManButtonUI && pManButtonUI->GetCheck())
    {
      pGlobalUserData->cbGender=GENDER_MANKIND;
    }
    if(pFemaleButtonUI && pFemaleButtonUI->GetCheck())
    {
      pGlobalUserData->cbGender=GENDER_FEMALE;
    }
