...
{
  if(m_pDlgBagUse!=NULL)
  {
    m_pDlgBagUse->EndDialog(IDOK);
  }
...
CVerticalLayoutUI* CDlgBagMain::CreateItem(tagBackpackProperty& BackpackProperty)
{
  ...
  if(pControlUI != NULL)
  {
    pControlUI->SetTag((UINT_PTR)&BackpackProperty);
  }
...
VOID CDlgBagMain::UpdateItemUI(CContainerUI* pVerticalLayoutUI, const tagBackpackProperty& BackpackProperty)
{
  ...
  if(pPropertyImage != NULL)
  {
    ...
    csImageKind.Format(TEXT("DLG_BOX_SHOP\\GMAE_PROPERTY_%d.png"),BackpackProperty.Property.wIndex);
...
bool CDlgBagMain::AddByType(CContainerUI* pVerticalLayoutUI, const int nType)
{
  ...
  if(nType == 0) //礼物
  {
    if(m_pPageLayoutUI[4])
    {
      m_pPageLayoutUI[4]->Add(pVerticalLayoutUI);
    }
  }
  else if(nType == 1) //宝石
  {
    if(m_pPageLayoutUI[1])
    {
      m_pPageLayoutUI[1]->Add(pVerticalLayoutUI);
    }
  }
  else if(nType == 2) //卡片
  {
    if(m_pPageLayoutUI[2])
    {
      m_pPageLayoutUI[2]->Add(pVerticalLayoutUI);
    }
  }
  else if(nType == 3) //道具
  {
    if(m_pPageLayoutUI[3])
    {
      m_pPageLayoutUI[3]->Add(pVerticalLayoutUI);
    }
  }
  else
  {
    if(m_pPageLayoutUI[0])
    {
      m_pPageLayoutUI[0]->Add(pVerticalLayoutUI);
    }
  }
  return true;
}
...
CContainerUI* CDlgBagMain::FindUIItem(CPageLayoutUI* pPageLayoutUI, const DWORD dwPropID)
{
  ...
  for(int i = 0; i < pPageLayoutUI->GetCount(); i++)
  {
    ...
    if(pContainerUI != NULL)
    {
      ...
      {
        tagBackpackProperty* pBackpackProperty = (tagBackpackProperty*)pControlUI->GetTag();
        if(pBackpackProperty->Property.wIndex == dwPropID)
        {
          return pContainerUI;
        }
      }
...
bool CDlgBagMain::UpdateProperty(const tagBackpackProperty& BackpackProperty)
{
  ...
  {
    if(BackpackProperty.Property.wIndex == 0)
    {
      continue;
    }
    ...
    if(BackpackProperty.nCount <= 0)
    {
      m_pPageLayoutUI[i]->Remove(pContainerUI, false);
    }
    else
    {
      UpdateItemUI(pContainerUI, BackpackProperty);
    }
    bUpdate = true;
  }
  return bUpdate;
