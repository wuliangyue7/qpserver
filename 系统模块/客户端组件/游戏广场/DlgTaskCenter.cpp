...
void CDlgTaskGuide::Notify(TNotifyUI &  msg)
{
  ...
  if(lstrcmp(msg.sType, TEXT("click")) == 0)
  {
    ...
    if(m_btLook==pControlUI)
    {
      ...
      {
        pPlatformEvent->PostPlatformEvent(EVENT_USER_SHOW_TASKVIEW,0L);
      }
      ...
      if(lstrcmp(pControlUI->GetClass(), TEXT("ListUI")) == 0)
      {
        ...
        if(m_pTaskList->CanExpand(node)==false)
        {
          ...
          if(pTaskParameter!=NULL)
          {
            ...
            //调整区域
            if(m_rcItemSelected.top<rcTaskList.top)
            {
              m_rcItemSelected.OffsetRect(0,rcTaskList.top-m_rcItemSelected.top+6);
            }
            if(m_rcItemSelected.bottom>rcTaskList.bottom)
            {
              m_rcItemSelected.OffsetRect(0,rcTaskList.bottom-m_rcItemSelected.bottom-6);
            }
...
VOID CDlgTaskCenter::OnDrawClientAreaBegin(CDC * pDC, INT nWidth, INT nHeight)
{
  ...
  if(m_pTaskParameter!=NULL)
  {
    ...
    if(nRewardYPos<rcTaskDescribe1.bottom+8)
    {
      nRewardYPos=rcTaskDescribe1.bottom+8;
    }
...
VOID CDlgTaskCenter::OnMissionStart()
{
  ...
  if(m_pTaskList!=NULL)
  {
    m_pTaskList->SetEnabled(false);
  }
  if(m_btTaskTake!=NULL)
  {
    m_btTaskTake->SetEnabled(false);
  }
  if(m_btTakeReward!=NULL)
  {
    m_btTakeReward->SetEnabled(false);
  }
}
...
VOID CDlgTaskCenter::OnMissionConclude()
{
  ...
  if(m_pTaskList!=NULL)
  {
    m_pTaskList->SetEnabled(true);
  }
  if(m_btTaskTake!=NULL)
  {
    m_btTaskTake->SetEnabled(true);
  }
  if(m_btTakeReward!=NULL)
  {
    m_btTakeReward->SetEnabled(true);
  }
}
...
VOID CDlgTaskCenter::RectifyControlUI()
{
  ...
  if(m_pTaskParameter==NULL || m_pTaskStatus!=NULL)
  {
    m_btTaskTake->SetEnabled(false);
  }
...
BOOL CDlgTaskCenter::AddMineTaskItem(tagTaskStatus & TaskStatus)
{
  ...
  if(m_TaskParameterMap.Lookup(pTaskStatus->wTaskID,pTaskParameter)==TRUE)
  {
    ...
    if(pTaskStatus->cbTaskStatus != TASK_STATUS_REWARD)
    {
      ...
      if(pNodeTask)
      {
        pNodeTask->Data().plistElment->SetUserData(pTaskParameter);
      }
...
BOOL CDlgTaskCenter::AddAllTaskItem(VOID * pTaskParameterData,WORD wParameterSize)
{
  ...
  if(pTaskParameterData==NULL || wParameterSize==0)
  {
    return FALSE;
  }
...
bool CDlgTaskPlazaCenter::OnEventMissionRead(TCP_Command Command, VOID * pData, WORD wDataSize)
{
  ...
  if(Command.wSubCmdID==SUB_GP_TASK_INFO)
  {
    ...
    if(wDataSize!=sizeof(pTaskInfo->TaskStatus[0])*pTaskInfo->wTaskCount+sizeof(pTaskInfo->wTaskCount))
    {
      return false;
    }
    ...
    {
      if(AddMineTaskItem(pTaskInfo->TaskStatus[wTaskIndex])==FALSE)
      {
        break;
      }
    }
  ...
  if(Command.wSubCmdID==SUB_GP_TASK_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GP_TaskResult))
    {
      return false;
    }
...
bool CDlgTaskServerCenter::OnTaskMessage(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  if(wSubCmdID==SUB_GR_TASK_RESULT)
  {
    ...
    if(wDataSize>sizeof(CMD_GR_S_TaskResult))
    {
      return false;
    }
