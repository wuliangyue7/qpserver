...
bool CUDownLoadSink::CloseDownLoad()
{
  ...
  //关闭文件
  if(m_DownFile.m_hFile!=INVALID_HANDLE_VALUE)
  {
    m_DownFile.Close();
  }
...
bool CUDownLoadSink::PerformDownLoad(LPCTSTR pszNetAddress,LPCTSTR pszFileName,bool bBreakpointDownLoad)
{
  ...
  if((m_cbDownLoadStatus!=DOWN_LOAD_IDLE)&&(m_cbDownLoadStatus!=DOWN_LOAD_ERROR))
  {
    return false;
  }
...
VOID CUDownLoadSink::PerformInstall()
{
  ...
  while(--nIndex>=0) if(szTargetName[nIndex]==TEXT('/'))
    {
      szTargetName[nIndex]=TEXT('\\');
    }
...
bool CUDownLoadSink::LoadDownFile(LPCTSTR pszLocation)
{
  ...
  while((dwIndex>0)&&(pszLocation[dwIndex]!=TEXT('/')))
  {
    dwIndex--;
  }
  ...
  //创建文件
  if(m_DownFile.Open(szFilePath,CFile::modeRead|CFile::modeWrite|CFile::shareDenyNone)==FALSE)
  {
    return false;
  }
...
bool CUDownLoadSink::CreateDownFile(LPCTSTR pszLocation)
{
  ...
  while((dwIndex>0)&&(szDownLoadPath[dwIndex]!=TEXT('\\')))
  {
    dwIndex--;
  }
  ...
  //创建文件
  if(m_DownFile.Open(szFilePath,CFile::modeCreate|CFile::modeWrite)==FALSE)
  {
    return false;
  }
...
bool CDownLoadManager::StartDownLoadTask()
{
  ...
  if(m_pDownLoadSink!=NULL)
  {
    ...
    if(m_DownLoadFileArray.GetCount()>0)
    {
      ...
      {
        m_pGameUpdateFrame->OnEventSwitchDownLoadTask(m_DownLoadFileArray[0]->szDownloadFile);
      }
    }
  }
...
LRESULT CDownLoadManager::OnItemDownLoadFinishMessage(WPARAM wParam, LPARAM lParam)
{
  ...
  if(m_pDownLoadSink!=NULL)
  {
    ...
    else
    {
      ...
      if(m_pGameUpdateFrame!=NULL)
      {
        m_pGameUpdateFrame->OnEventDownLoadEnd();
      }
    }
  }
...
VOID CGameUpdateFrame::OnDrawClientArea(CDC * pDC, INT nWidth, INT nHeight)
{
  ...
  if(m_pDownLoadManager!=NULL && m_pDownLoadManager->m_pDownLoadSink!=NULL)
  {
    ...
    //设置变量
    if(dwTotalTaskCount>0)
    {
      dwTotalProgress = (DWORD)(((double)dwTaskFinished/(double)dwTotalTaskCount)*(double)(ProgressLineSize.cx));
    }
    if(dwTotalFileSize>0)
    {
      dwCurrProgress = (DWORD)(((double)dwDownLoadFileSize/(double)dwTotalFileSize)*(double)ProgressLineSize.cx);
    }
    if(dwTotalTaskCount>0)
    {
      dwTotalProgress += dwCurrProgress/dwTotalTaskCount;
    }
