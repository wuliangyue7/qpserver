...
void CBitImageEx::LoadFromResource(CWnd* pWnd, HINSTANCE hInstance, UINT nIDResource)
{
  if(CD2DEngine::GetD2DEngine())
  {
    m_pWnd = pWnd;
    CD2DEngine::GetD2DEngine()->LaodImage(pWnd, hInstance, nIDResource, m_strImageName);
  }
...
//创建区域
bool CBitImageEx::CreateImageRegion(CRgn & ImageRgn, COLORREF crTransColor)
{
  //创建区域
...
//创建区域
bool CBitImageEx::CreateImageRegion(CRgn & ImageRgn, INT nXSrcPos, INT nYSrcPos, INT nSrcWidth, INT nSrcHeight, COLORREF crTransColor)
{
  //判断状态
  ASSERT(IsNull()==false);
  if(IsNull()==true)
  ...
  if((nXSrcPos+nSrcWidth)>nImageWidht)
  {
    nSrcWidth=nImageWidht-nXSrcPos;
  }
  if((nYSrcPos+nSrcHeight)>nImageHeight)
  {
    nSrcHeight=nImageHeight-nYSrcPos;
  }
  ...
  while((ImageInfo.bmWidthBytes%4)!=0)
  {
    ImageInfo.bmWidthBytes++;
  }
  ...
  for(INT nYPos=0; nYPos<ImageInfo.bmHeight; nYPos++)
  {
    for(INT nXPos=0; nXPos<ImageInfo.bmWidth; nXPos++)
    {
      ...
      do
      {
        ...
        if((cbRTranColor==GetRValue(crImageColor))&&(cbGTranColor==GetGValue(crImageColor))&&(cbBTranColor==GetBValue(crImageColor)))
        {
          break;
        }
      }
...
//平铺绘画
bool CBitImageEx::DrawImageTile(CDC * pDC, CRect & rcDestRect)
{
  //平铺绘画
...
//平铺绘画
bool CBitImageEx::DrawImageTile(CDC * pDC, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
  //效验状态
  ASSERT(IsNull()==false);
  if(IsNull()==true)
  ...
  for(INT nXDrawPos=0; nXDrawPos<nWidth; nXDrawPos+=nImageWidth)
  {
    for(INT nYDrawPos=0; nYDrawPos<nHeight; nYDrawPos+=nImageHeight)
    {
      ...
      if(bIntersect==true)
      {
        if(CD2DEngine::GetD2DEngine())
        {
          CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,nXDrawPos+nXPos,nYDrawPos+nYPos,nDrawWidth,nDrawHeight,0,0,255);
        }
        else
        {
          BitBlt(pDC->m_hDC,nXDrawPos+nXPos,nYDrawPos+nYPos,nDrawWidth,nDrawHeight,0,0);
        }
      }
    }
  }
...
//一般绘画
BOOL CBitImageEx::BitBlt(HDC hDestDC, int xDest, int yDest, DWORD dwROP /*= SRCCOPY*/)
{
  return BitBlt(hDestDC, xDest, yDest, GetWidth(), GetHeight(), 0, 0);
}

//一般绘画
BOOL CBitImageEx::BitBlt(HDC hDestDC, const POINT& pointDest, DWORD dwROP /*= SRCCOPY*/)
{
  return BitBlt(hDestDC, pointDest.x, pointDest.y, GetWidth(), GetHeight(), 0, 0);
}

//一般绘画
BOOL CBitImageEx::BitBlt(HDC hDestDC, int xDest, int yDest, int nDestWidth, int nDestHeight, int xSrc, int ySrc, DWORD dwROP /*= SRCCOPY*/)
{
  if(CD2DEngine::GetD2DEngine())
  {
    CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,xDest,yDest,nDestWidth,nDestHeight,xSrc,ySrc,255);
    return true;
  }
...
//一般绘画
BOOL CBitImageEx::BitBlt(HDC hDestDC, const RECT& rectDest, const POINT& pointSrc, DWORD dwROP /*= SRCCOPY*/)
{
...
//透明绘画
bool CBitImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, COLORREF crTransColor)
{
  //绘画图片
...
//透明绘画
bool CBitImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, COLORREF crTransColor)
{
  if(CD2DEngine::GetD2DEngine())
  {
    CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,xDest,yDest,cxDest,cyDest,xSrc,ySrc,cxDest,cyDest,crTransColor, 255);
    return true;
  }
  ...
  if(IsRectIntersect(rcDirty,xDest,yDest,cxDest,cyDest)==false)
  {
    return true;
  }
...
//混合绘画
bool CBitImageEx::TransDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, COLORREF crTransColor)
{
  if(CD2DEngine::GetD2DEngine())
  {
    CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,xDest,yDest,cxDest,cyDest,xSrc,ySrc,cxSrc,cySrc,crTransColor, 255);
    return true;
  }
...
//混合绘画
bool CBitImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, BYTE cbAlphaDepth)
{
  //透明绘画
...
//混合绘画
bool CBitImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, BYTE cbAlphaDepth)
{
  if(CD2DEngine::GetD2DEngine())
  {
...
//混合绘画
bool CBitImageEx::AlphaDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, BYTE cbAlphaDepth)
{
  if(CD2DEngine::GetD2DEngine())
  {
...
//混合绘画
bool CBitImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, COLORREF crTransColor, BYTE cbAlphaDepth)
{
  //透明绘画
...
//混合绘画
bool CBitImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, COLORREF crTransColor, BYTE cbAlphaDepth)
{
  if(CD2DEngine::GetD2DEngine())
  {
    CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,xDest,yDest,cxDest,cyDest,xSrc,ySrc,cxDest,cyDest,crTransColor, cbAlphaDepth);
    return true;
  }
...
//混合绘画
bool CBitImageEx::BlendDrawImage(CDC * pDestDC, INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, INT cxSrc, INT cySrc, COLORREF crTransColor, BYTE cbAlphaDepth)
{
  if(CD2DEngine::GetD2DEngine())
  {
    CD2DEngine::GetD2DEngine()->DrawImage(m_pWnd,m_strImageName,xDest,yDest,cxDest,cyDest,xSrc,ySrc,cxSrc,cySrc, crTransColor, cbAlphaDepth);
    return true;
  }
...
//矩形检查
bool CBitImageEx::IsRectIntersect(const CRect & rcRect, INT nXPos, INT nYPos, INT nWidth, INT nHeight)
{
  //范围检测
  if((nXPos>rcRect.right)||(nYPos>rcRect.bottom))
  {
    return false;
  }
  if(((nXPos+nWidth)<rcRect.left)||((nYPos+nHeight)<rcRect.top))
  {
    return false;
  }
...
//区域计算
bool CBitImageEx::GetDrawImageArea(INT xDest, INT yDest, INT cxDest, INT cyDest, INT xSrc, INT ySrc, CRect & rcDirty, tagImageRender & ImageRender)
{
  //相交矩形
  CRect rcIntersect;
...
//D2D自定义函数
bool CBitImageEx::D2DDrawCustomImage(CWnd* pWnd, INT nXDest, INT nYDest, INT nDestWidth, INT nDestHeight, DWORD* pImageData, INT nImageWidth, INT nImageHeight, BYTE cbAlpha /*= 255*/)
{
  if(CD2DEngine::GetD2DEngine())
  {
