...
bool CGIFImage::LoadFromMemroy(BYTE * pcbData, LONG lDataSize)
{
  ...
  if((pcbData==NULL)||(lDataSize<sizeof(tagGIFHeader)))
  {
    return false;
  }
  ...
  if(memcmp(&pGIFHeader->cbSignature,"GIF",3)!= 0)
  {
    return false;
  }
  if((memcmp(&pGIFHeader->cbVersion,"87a",3)!= 0)&&(memcmp(&pGIFHeader->cbVersion,"89a",3)!= 0))
  {
    return false;
  }
  ...
  do
  {
    ...
    if((GIFBlockTypes==BLOCK_UNKNOWN)||(GIFBlockTypes==BLOCK_TRAILER))
    {
      break;
    }
    ...
    if(GIFBlockTypes==BLOCK_CONTROLEXT)
    {
      ...
      {
        //切换区块
        if(SwitchNextBlock(ImageDataInfo)==false)
        {
          return false;
        }
        ...
        if((GIFBlockTypes==BLOCK_IMAGE)||(GIFBlockTypes==BLOCK_PLAINTEXT))
        {
          break;
        }
...
bool CGIFImage::LoadFromResource(LPCTSTR pszResourceName, LPCTSTR pszResourceType)
{
  ...
  if((pszResourceName==NULL)||(pszResourceType==NULL))
  {
    return false;
  }
...
{
  LONG lBlockSize=GetCurrentBlockSize(ImageDataInfo);
  if((lBlockSize<=0)||((ImageDataInfo.lCurentPos+lBlockSize)>ImageDataInfo.lDataSize))
  {
    return false;
  }
