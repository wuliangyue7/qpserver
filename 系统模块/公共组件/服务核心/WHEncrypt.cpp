...
VOID CMD5Aide::MD5Update(BYTE * pcbInput, UINT inputLen)
{
  ...
  if((m_lCount[0]+=((ULONG)inputLen<<3))<((ULONG)inputLen<<3))
  {
    m_lCount[1]++;
  }
  ...
  if(inputLen>=nPartLen)
  {
    ...
    {
      MD5Transform(m_lState,&pcbInput[i]);
    }
    nIndex=0;
  }
  else
...
bool CWHEncrypt::XorEncrypt(LPCTSTR pszSourceData, LPTSTR pszEncrypData, WORD wMaxCount)
{
  ...
  for(WORD i=1; i<CountArray(wRandKey); i++)
  {
    wRandKey[i]=rand()%0xFFFF;
  }
...
bool CWHEncrypt::XorCrevasse(LPCTSTR pszEncrypData, LPTSTR pszSourceData, WORD wMaxCount)
{
  ...
  if(wEncrypPassLen<ENCRYPT_KEY_LEN*8)
  {
    return false;
  }
  ...
  if(wEncrypPassLen!=(((wSoureLength+ENCRYPT_KEY_LEN-1)/ENCRYPT_KEY_LEN)*ENCRYPT_KEY_LEN*8))
  {
    return false;
  }
