...
bool CWHService::GetMachineIDEx(TCHAR szMachineID[LEN_MACHINE_ID])
{
  ...
  GetVolumeInformation(TEXT("C:\\"),NULL,0,&dwVolumeSerialNumber,NULL,NULL,0,NULL);
  if(dwVolumeSerialNumber==0)
  {
    GetVolumeInformation(TEXT("D:\\"),NULL,0,&dwVolumeSerialNumber,NULL,NULL,0,NULL);
  }

  TCHAR szMACAddressEx[LEN_MACHINE_ID];
  TCHAR szCPU[17]= {0};
  memset(szMACAddressEx, 0, sizeof(szMACAddressEx));
  //lstrcpyn(szMACAddressEx,szMACAddress,CountArray(szMACAddress));

  if(s1 != 0 && s2 != 0)
  {
    _sntprintf(szCPU,sizeof(szCPU),TEXT("%08X%08X"),s1,s2);
    //_sntscanf(szMACAddressEx, CountArray(szMACAddressEx), TEXT("%08X%08X"),s1,s2);
  }
  else  if(s3 != 0 && dwVolumeSerialNumber != 0)
  {
    _sntprintf(szCPU,sizeof(szCPU),TEXT("%08X%08X"),s3,dwVolumeSerialNumber);
    // _sntscanf(szMACAddressEx, CountArray(szMACAddressEx), TEXT("%08X"),s3);
  }
  else  if(s3 != 0)
  {
    _sntprintf(szCPU,sizeof(szCPU),TEXT("%08X"),s3);
    // _sntscanf(szMACAddressEx, CountArray(szMACAddressEx), TEXT("%08X"),s3);
  }
  else  if(dwVolumeSerialNumber != 0)
  {
    _sntprintf(szCPU,sizeof(szCPU),TEXT("%08X"),dwVolumeSerialNumber);
    // _sntscanf(szMACAddressEx, CountArray(szMACAddressEx), TEXT("%08X"),s3);
  }
// CString str;
// str.Format(TEXT("%s-%s,%ld,%ld,%ld"),szMACAddressEx,szMACAddress,s1,s2,s3);
  //AfxMessageBox(str,0,0);
  _sntprintf(szMACAddressEx,sizeof(szMACAddressEx),TEXT("%s%s"),szMACAddress,szCPU);
  //AfxMessageBox(szMACAddressEx,0,0);
  CWHEncrypt::MD5Encrypt(szMACAddressEx,szMachineID);
...
bool CWHService::GetModuleVersion(LPCTSTR pszModuleName, DWORD & dwVersionInfo)
{
  ...
  if(GetFileVersionInfo(pszModuleName,dwFileHandle,sizeof(cbInfoBuffer),cbInfoBuffer)==FALSE)
  {
    return false;
  }
  ...
  if(VerQueryValue(cbInfoBuffer,TEXT("\\"),(VOID **)&pFixedFileInfo,&uQuerySize)==FALSE)
  {
    return false;
  }
