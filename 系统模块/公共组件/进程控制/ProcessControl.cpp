...
bool CProcessControl::SetProcessControlSink(IUnknownEx * pIUnknownEx)
{
  ...
  if(pIUnknownEx!=NULL)
  {
    ...
    //成功判断
    if(m_pIProcessControlSink==NULL)
    {
      return false;
    }
  }
...
bool CProcessControl::SendData(HWND hWndServer, WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
  ...
  if(pData!=NULL)
  {
    ...
    if(wDataSize>sizeof(IPCBuffer.cbBuffer))
    {
      return false;
    }
...
BOOL CProcessControl::OnCopyData(CWnd * pWnd, COPYDATASTRUCT * pCopyDataStruct)
{
  ...
  if(pCopyDataStruct->cbData<sizeof(IPC_Head))
  {
    return FALSE;
  }
  ...
  if((pHead->wVersion!=IPC_VER)||(pHead->wPacketSize!=pCopyDataStruct->cbData))
  {
    return FALSE;
  }
