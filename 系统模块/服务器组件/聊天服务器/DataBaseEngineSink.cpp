    ...
    if(pLogonUserID->wLogonComand&LOGON_COMMAND_GROUPINFO)
    {
      ...
      if(lResultCode==DB_SUCCESS && m_AccountsDBModule.GetInterface())
      {
        ...
        while(m_AccountsDBModule->IsRecordsetEnd()==false)
        {
          ...
          //退出判断
          if(UserGroupInfo.wGroupCount>=GROUP_MAX_COUNT)
          {
            break;
          }
...
bool CDataBaseEngineSink::OnRequestTrumpet(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  try
  {
    ...
    m_PlatformDBAide.AddParameter(TEXT("@dwUserID"),pTrumpet->dwSourceUserID);
    ...
    TrumpetResult.dwSourceUserID = pTrumpet->dwSourceUserID;
    TrumpetResult.cbFontAttri = pTrumpet->cbFontAttri;
    TrumpetResult.cbFontSize = pTrumpet->cbFontSize;
    TrumpetResult.dwFontColor = pTrumpet->dwFontColor;
    lstrcpyn(TrumpetResult.szNickName, pTrumpet->szNickName, CountArray(TrumpetResult.szNickName));
    lstrcpyn(TrumpetResult.szFontName, pTrumpet->szFontName, CountArray(TrumpetResult.szFontName));
    lstrcpyn(TrumpetResult.szMessageContent, pTrumpet->szMessageContent, CountArray(TrumpetResult.szMessageContent));
