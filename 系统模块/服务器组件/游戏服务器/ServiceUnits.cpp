...
bool CServiceUnits::ConcludeService()
{
  ...
  //数据引擎
  if(m_RecordDataBaseEngine.GetInterface()!=NULL)
  {
    m_RecordDataBaseEngine->ConcludeService();
  }
  if(m_KernelDataBaseEngine.GetInterface()!=NULL)
  {
    m_KernelDataBaseEngine->ConcludeService();
  }
  ...
  {
    m_MatchServiceManager.CloseInstance();
  }
...
bool CServiceUnits::InitializeService()
{
  ...
  if(LoadDataBaseParameter(pszDataBaseAddr,pszDataBaseName,m_DataBaseParameter)==false)
  {
    return false;
  }
  ...
  //数据引擎
  IUnknownEx * pIDataBaseEngineRecordSink;
  IUnknownEx * pIDataBaseEngineKernelSink;

  pIDataBaseEngineRecordSink=QUERY_OBJECT_INTERFACE(m_RecordDataBaseSink,IUnknownEx);
  pIDataBaseEngineKernelSink=QUERY_OBJECT_INTERFACE(m_KernelDataBaseSink,IUnknownEx);
  ...
  if(m_RecordDataBaseEngine->SetDataBaseEngineSink(pIDataBaseEngineRecordSink)==false)
  {
    return false;
  }
  if(m_KernelDataBaseEngine->SetDataBaseEngineSink(pIDataBaseEngineKernelSink)==false)
  {
    return false;
  }
  ...
  if(m_MatchServiceManager.GetInterface()!=NULL)
  {
    if(m_MatchServiceManager->CreateGameMatch(m_GameMatchOption.cbMatchType)==false)
    {
      return false;
    }
  }
  ...
  //数据回调
  m_RecordDataBaseSink.m_pInitParameter=&m_InitParameter;
  m_RecordDataBaseSink.m_pGameParameter=&m_GameParameter;
  m_RecordDataBaseSink.m_pDataBaseParameter=&m_DataBaseParameter;
  m_RecordDataBaseSink.m_pGameServiceAttrib=&m_GameServiceAttrib;
  m_RecordDataBaseSink.m_pGameServiceOption=&m_GameServiceOption;
  m_RecordDataBaseSink.m_pIGameServiceManager=m_GameServiceManager.GetInterface();
  m_RecordDataBaseSink.m_pIDataBaseEngineEvent=QUERY_OBJECT_PTR_INTERFACE(pIAttemperEngine,IDataBaseEngineEvent);


  //数据回调
  m_KernelDataBaseSink.m_pInitParameter=&m_InitParameter;
  m_KernelDataBaseSink.m_pGameParameter=&m_GameParameter;
  m_KernelDataBaseSink.m_pDataBaseParameter=&m_DataBaseParameter;
  m_KernelDataBaseSink.m_pGameServiceAttrib=&m_GameServiceAttrib;
  m_KernelDataBaseSink.m_pGameServiceOption=&m_GameServiceOption;
  m_KernelDataBaseSink.m_pIGameServiceManager=m_GameServiceManager.GetInterface();
  m_KernelDataBaseSink.m_pIDataBaseEngineEvent=QUERY_OBJECT_PTR_INTERFACE(pIAttemperEngine,IDataBaseEngineEvent);
  m_KernelDataBaseSink.m_pIDBCorrespondManager=(IDBCorrespondManager*)m_DBCorrespondManager.QueryInterface(IID_IDBCorrespondManager,VER_IDBCorrespondManager);
...
LRESULT CServiceUnits::OnUIControlRequest(WPARAM wParam, LPARAM lParam)
{
  ...
  switch(DataHead.wIdentifier)
  {
    case UI_CORRESPOND_RESULT:    //协调成功
    {
      ...
      if(DataHead.wDataSize!=sizeof(CP_ControlResult))
      {
        return 0;
      }
...
//重读代理
void CServiceUnits::LoadGate()
{
  m_AttemperEngineSink.LoadGateIP();
}
//////////////////////////////////////////////////////////////////////////////////
