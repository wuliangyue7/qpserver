    ...
    tagGameServiceAttrib *pGameServiceAttrib,tagGameServiceOption *pGameServiceOption,CTableFrameArray &TableFrameArray)
{
  ...
  if(pIMainServiceFrame==NULL || pIKernelDataBaseEngine==NULL || pGameServiceOption==NULL || pGameServiceAttrib==NULL || pITimerEngine==NULL)
  {
    return false;
  }
...
//  if(m_DistributeManage.GetCount()<m_pGameServiceOption->wMinDistributeUser) return false;
  ...
  while(true)
  {
    ...
    if(m_DistributeManage.GetRealCount()==0)
    {
      return false;
    }
    ...
    if(wDistributeCount < m_pGameServiceAttrib->wChairCount)
    {
      break;
    }
  ...
  for(WORD i=0; i<m_pGameServiceOption->wTableCount; i++)
  {
    ...
    if(pTableFrame->IsGameStarted() || wSitUserCount==m_pGameServiceAttrib->wChairCount || wSitUserCount==0)
    {
      continue;
    }
...
bool CTimeRankingsMatch::OnEventDataBase(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)
{
  switch(wRequestID)
  {
    ...
    case DBO_GR_MATCH_RANK:     //比赛名次
    {
      ...
      if(wDataSize%sizeof(DBO_GR_MatchRank)!=0)
      {
        return false;
      }
...
bool CTimeRankingsMatch::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
  ...
  {
    if(m_cbCurMatch==0xFF || m_cbMatchStatus== MATCH_STATUS_FINISH)
    {
      return true;
    }
    ...
    if(m_cbMatchStatus==MATCH_STATUS_NOT_STARTED)
    {
      if((m_TimeMatchStart==CurTime))
      {
        ...
        if(m_pIMainServiceFrame!=NULL)
        {
          m_pIMainServiceFrame->OnEventMatchStart(m_lInitScore);
        }
