...
BEGIN_MESSAGE_MAP(CGameServerDlg, CDialog)
  ...
  ON_BN_CLICKED(IDC_BT_LOAD_GATE_IP, OnBnClickedLoadGate)
...
VOID CGameServerDlg::UpdateServerTitle(enServiceStatus ServiceStatus)
{
  ...
  //房间名字
  if(m_bOptionSuccess==false)
  {
    pszServerName=TEXT("游戏服务器");
  }
  else
  {
...
VOID CGameServerDlg::OnBnClickedOpenServer()
{
  ...
  //配置房间
  CDlgServerItem DlgServerItem;
  if(DlgServerItem.OpenGameServer()==false)
  {
    return;
  }
...
VOID CGameServerDlg::OnBnClickedCreateServer()
{
  ...
  if(DlgServerWizard.CreateGameServer()==false)
  {
    return;
  }
...
//重载代理配置
VOID CGameServerDlg::OnBnClickedLoadGate()
{
  m_ServiceUnits.LoadGate();
  return;
}
//命令处理
