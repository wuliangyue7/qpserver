...
#define IDI_UPDATE_ONLINE_COUNT     (IDI_MAIN_MODULE_START+12)          //在线人数
#define IDI_LOAD_STORAGE_CONFIG_INFO     (IDI_MAIN_MODULE_START+13)          //库存数据
//////////////////////////////////////////////////////////////////////////////////
//时间定义 秒

#define TIME_LOAD_ANDROID_USER    15L                 //加载机器
#define TIME_DISTRIBUTE_ANDROID   5L                  //分配用户
...
CAttemperEngineSink::CAttemperEngineSink()
{
  ...
  m_wGateCount = 0;
  ZeroMemory(m_dwGataIP,sizeof(m_dwGataIP));
  ...
  m_DataConfigProperty.cbPropertyCount = 0;
  ZeroMemory(m_DataConfigProperty.PropertyInfo,sizeof(m_DataConfigProperty.PropertyInfo));
  ZeroMemory(&m_StorageConfigInfo,sizeof(m_StorageConfigInfo));
...
bool CAttemperEngineSink::OnAttemperEngineStart(IUnknownEx * pIUnknownEx)
{
  ...
  m_pITimerEngine->SetTimer(IDI_UPDATE_ONLINE_COUNT,15*1000L,TIMES_INFINITY,NULL);
  m_pITimerEngine->SetTimer(IDI_LOAD_STORAGE_CONFIG_INFO,10*1000L,1,0);
#else
  ...
  m_pITimerEngine->SetTimer(IDI_UPDATE_ONLINE_COUNT,60*1000L,TIMES_INFINITY,NULL);
  m_pITimerEngine->SetTimer(IDI_LOAD_STORAGE_CONFIG_INFO,20*1000L,1,0);
#endif
...
bool CAttemperEngineSink::OnEventTimer(DWORD dwTimerID, WPARAM wBindParam)
{
  try
  {
    ...
    if((dwTimerID>=IDI_MAIN_MODULE_START)&&(dwTimerID<=IDI_MAIN_MODULE_FINISH))
    {
      ...
      switch(dwTimerID)
      {
        case IDI_LOAD_ANDROID_USER:   //加载机器
        {
          ...
          if(m_AndroidUserManager.GetAndroidCount()<50)
          {
            //加载机器
            m_pIDBCorrespondManager->PostDataBaseRequest(0L,DBR_GR_LOAD_ANDROID_PARAMETER,0L,NULL,0L);
          }
        ...
        case IDI_DISTRIBUTE_ANDROID:  //分配机器
        {
          ...
          if(m_AndroidUserManager.GetAndroidCount()>0 && m_pGameServiceOption->wServerType!=GAME_GENRE_MATCH)
          {
            ...
            srand(dwCurrentTime+m_AndroidUserManager.GetAndroidCount());
            //变量定义
            ...
            if(bAllowAndroidSimulate==true && bAllowAvertCheatMode==false)
            {
              ...
              WORD wTableCount=m_pGameServiceOption->wTableCount/2;
              if(m_pGameServiceOption->wServerType == GAME_GENRE_GOLD && wTableCount>1)
              {
                LONGLONG lMinEnterScore=m_pGameServiceOption->lMinEnterScore;
                if(lMinEnterScore<=1000)
                {
                  wTableCount=m_pGameServiceOption->wTableCount;
                }
                else if(lMinEnterScore>1000 && lMinEnterScore<=50000)
                {
                  wTableCount=m_pGameServiceOption->wTableCount/2;
                  if(m_pGameServiceOption->wTableCount>=50)
                  {
                    wTableCount = wTableCount + 20;
                  }
                  else if(m_pGameServiceOption->wTableCount>=40)
                  {
                    wTableCount = wTableCount + 15;
                  }
                  else if(m_pGameServiceOption->wTableCount>=30)
                  {
                    wTableCount = wTableCount + 10;
                  }
                }
              }
              //机器处理
              if(AndroidSimulate.wFreeUserCount>0)
              {
                ...
                {
                  //随机桌子
                  WORD wTableID=rand()%(__max(wTableCount,1));
                  ...
                  //分配判断
                  if(TableUserInfo.wTableUserCount>1)
                  {
                    continue;
                  }
                  if((wUserSitCount>=TableUserInfo.wMinUserCount)&&(m_pGameServiceAttrib->wChairCount<MAX_CHAIR))
                  {
                    continue;
                  }
            ...
            if(bAllowAndroidAttend==true)
            {
              ...
              if(bAllowAvertCheatMode)
              {
                ...
                for(WORD j=0; j<AndroidPassivity.wFreeUserCount; j++)
                {
                  ...
                  if(InsertDistribute(pIAndroidUserItem->GetMeUserItem())==true)
                  {
                    return true;
                  }
                }
                ...
                if(AndroidPassivity.wFreeUserCount>0)
                {
                  ...
                  if(m_pGameServiceAttrib->wChairCount >= MAX_CHAIR)
                  {
                    for(INT_PTR i=0; i<(m_pGameServiceOption->wTableCount); i++)
                    {
                      ...
                      {
                        CString mstr;
                        mstr.Format(TEXT("百人游戏 wUserSitCount:%d, %d"),wUserSitCount,AndroidPassivity.wFreeUserCount);
                        CTraceService::TraceString(mstr,TraceLevel_Exception);
                      }
                      else
                      {
                        if(AndroidPassivity.wFreeUserCount<6)
                        {
                          return true;
                        }
                      }
                      ...
                      if(wUserSitCount>m_pGameServiceAttrib->wChairCount-13)
                      {
                        return true;
                      }
                  ...
                  else
                  {
                    for(INT_PTR i=0; i<m_pGameServiceOption->wTableCount; i++)
                    {
                      ...
                      if((wUserSitCount>=TableUserInfo.wMinUserCount)&&(rand()%100>30))
                      {
                        continue;
                      }
                      if(wUserSitCount==m_pGameServiceAttrib->wChairCount)
                      {
                        continue;
                      }
                      if(TableUserInfo.wTableUserCount==0)
                      {
                        if((wUserSitCount>0)&&(wUserSitCount<TableUserInfo.wMinUserCount))
                        {
                        }
                        else
                        {
              ...
              else
              {
                ...
                if(AndroidInitiative.wFreeUserCount>0)
                {
                  for(INT_PTR i=0; i<m_pGameServiceOption->wTableCount; i++)
                  {
                    ...
                    if(m_pGameServiceAttrib->wChairCount<MAX_CHAIR)
                    {
                      if(wUserSitCount>(TableUserInfo.wMinUserCount-1) && (rand()%100>30))
                      {
                        continue;
                      }
                      if(wUserSitCount==m_pGameServiceAttrib->wChairCount)
                    ...
                    else
                    {
                      ...
                      {
                        CString mstr;
                        mstr.Format(TEXT("22百人游戏 wUserSitCount:%d, %d"),wUserSitCount,AndroidInitiative.wFreeUserCount);
                        CTraceService::TraceString(mstr,TraceLevel_Exception);
                      }
                      if(wUserSitCount>75)
                      {
            ...
            for(INT_PTR i=nIndex; i<m_pGameServiceOption->wTableCount+nIndex; ++i)
            {
              ...
              //用户过虑
              bool bRand = ((rand()%100)>80);
              if(TableUserInfo.wTableAndroidCount==0)
              {
                continue;
              }
              if((TableUserInfo.wTableUserCount>0)&&(bAllowAndroidAttend==true) && bRand)
              {
                continue;
              }
              if(TableUserInfo.wTableAndroidCount>=TableUserInfo.wMinUserCount && bRand)
              {
                continue;
              }
              ...
              for(WORD j=0; j<pTableFrame->GetChairCount(); j++)
              {
                ...
                tagTimeInfo* pTimeInfo=pIServerUserItem->GetTimeInfo();
                if(pTimeInfo==NULL)
                {
                  continue ;
                }
                if((pTimeInfo->dwEnterTableTimer>0)&&(TableUserInfo.wTableUserCount>1))
                {
                  if((dwCurrentTime-pTimeInfo->dwEnterTableTimer)<1600)
                  {
                    continue;
                  }
                }
        ...
        {
          //数量判断
          if(m_SystemMessageActive.GetCount()==0)
          {
            return true;
          }
        ...
        case IDI_UPDATE_ONLINE_COUNT: //在线人数
        {
          DBO_GR_UpdateOnLineCount UpdateInfo;
          ZeroMemory(&UpdateInfo,sizeof(DBO_GR_UpdateOnLineCount));
          UpdateInfo.wServerID = m_pGameServiceOption->wServerID;
          UpdateInfo.wKindID =m_pGameServiceOption->wKindID;
          UpdateInfo.dwOnLineCount = m_ServerUserManager.GetUserItemCount();
          UpdateInfo.dwMBOnLineCount = 0;
          WORD wEnumIndex=0;
          ...
          while(pIServerUserItem!=NULL)
          {
            ...
            {
              UpdateInfo.dwMBOnLineCount++;
            }
            //枚举用户
            pIServerUserItem= m_ServerUserManager.EnumUserItem(wEnumIndex++);
          }

          m_pIDBCorrespondManager->PostDataBaseRequest(0L,DBR_GR_UPDATE_ONLINE_COUNT,0L,&UpdateInfo,sizeof(UpdateInfo));

          return true;
        }
        case IDI_LOAD_STORAGE_CONFIG_INFO:  //库存数据
        {
          m_pIDBCorrespondManager->PostDataBaseRequest(0L,DBR_GR_LOAD_STORAGE_CONFIG_INFO,0L,NULL,0);
          return true;
        }
      }
    }
    ...
    if((dwTimerID>=IDI_MATCH_MODULE_START)&&(dwTimerID<IDI_MATCH_MODULE_FINISH))
    {
      if(m_pIMatchServiceManager!=NULL)
      {
        m_pIMatchServiceManager->OnEventTimer(dwTimerID,wBindParam);
      }
      return true;
    }
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  switch(wRequestID)
  {
    ...
    case DBO_GR_GAME_PROPERTY_ITEM:   //道具信息
    {
      return OnDBGamePropertyInfo(dwContextID,pData,wDataSize);
    }
    case DBO_GR_GAME_PROPERTY_LIST_RESULT:    //道具结果
    {
    ...
    case DBR_GR_LOAD_STORAGE_CONFIG_INFO_RESULT:    //加载库存数据
    {
      return OnDBLoadStorageConfigInfo(dwContextID,pData,wDataSize);
    }
  }
...
{
  //CString mstr;
  //mstr.Format(TEXT("OnEventTCPNetworkRead %d,%d,%d"),Command.wMainCmdID,Command.wSubCmdID,wDataSize);
  //CTraceService::TraceString(mstr,TraceLevel_Debug);
  switch(Command.wMainCmdID)
  {
    ...
    case MDM_FORWARD_COMMAND: //代理命令
    {
      switch(Command.wSubCmdID)
      {
          //注册IP
        case SUB_FORWARD_CLIENTIP:
        {
          ASSERT(wDataSize>=sizeof(Sub_Forward_ClinetIP));
          if(wDataSize<sizeof(Sub_Forward_ClinetIP))
          {
            return true;
          }
          Sub_Forward_ClinetIP * pForwardClientIP = (Sub_Forward_ClinetIP *)pData;
          if(pForwardClientIP->dwClientIP==0)
          {
            return true;
          }
          if(pForwardClientIP->dwClientIP==INADDR_NONE)
          {
            return true;
          }
          //变量定义
          ...
          //一样就不管了
          if(pForwardClientIP->dwClientIP == pBindParameter->dwClientAddr)
          {
            //TCHAR szClientAddr[16]=TEXT("");
            //BYTE * pClientAddr=(BYTE *)&pBindParameter->dwClientAddr;
            //_sntprintf(szClientAddr,CountArray(szClientAddr),TEXT("%d.%d.%d.%d"),pClientAddr[0],pClientAddr[1],pClientAddr[2],pClientAddr[3]);
            //TCHAR szClientAddr2[16]=TEXT("");
            //BYTE * pClientAddr2=(BYTE *)&pForwardClientIP->dwClientIP;
            //_sntprintf(szClientAddr2,CountArray(szClientAddr2),TEXT("%d.%d.%d.%d"),pClientAddr2[0],pClientAddr2[1],pClientAddr2[2],pClientAddr2[3]);

            //CString Msg;
            //Msg.Format(L"IP - %d=%s : %d-%s-%s",pBindParameter->dwClientAddr,szClientAddr,pForwardClientIP->dwClientIP,szClientAddr,pForwardClientIP->szClientIP);
            //CTraceService::TraceString(Msg,TraceLevel_Normal);
            ////CTraceService::TraceString(L"if (pForwardClientIP->dwClientIP == pBindParameter->dwClientAddr) ",TraceLevel_Normal);
            return true;
          }

          if(pBindParameter != NULL)
          {
            //检查真实ip是否为代理服务器，是就更换ip
            if(CheckGateIP(pBindParameter->dwClientAddr))
            {
              //TCHAR szClientAddr[16]=TEXT("");
              //BYTE * pClientAddr=(BYTE *)&pBindParameter->dwClientAddr;
              //_sntprintf(szClientAddr,CountArray(szClientAddr),TEXT("%d.%d.%d.%d"),pClientAddr[0],pClientAddr[1],pClientAddr[2],pClientAddr[3]);
              ...
              //CString Msg;
              //Msg.Format(L"网关修改IP - %d=%s : %d-%s-%s",pBindParameter->dwClientAddr,szClientAddr,pForwardClientIP->dwClientIP,szClientAddr,pForwardClientIP->szClientIP);
              //CTraceService::TraceString(Msg,TraceLevel_Normal);
              ////注册IP
              pBindParameter->dwClientAddr = pForwardClientIP->dwClientIP;
...
//游戏消息
bool CAttemperEngineSink::SendGlobalMessage(LPCTSTR lpszMessage, WORD wType)
{
  //构造消息
  ...
  //SendMessage.wClientID=CLIENT_ID;

  //WORD wSendSize = sizeof(SendMessage)-sizeof(SendMessage.szSystemMessage)+CountStringBuffer(SendMessage.szSystemMessage);

  //数据属性
  WORD wHeadSize=sizeof(SendMessage)-sizeof(SendMessage.szSystemMessage);
  WORD wSendSize=wHeadSize+CountStringBuffer(SendMessage.szSystemMessage);
...
bool CAttemperEngineSink::SendDataBatchToMobileUser(WORD wCmdTable, WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  while(wEnumIndex<m_ServerUserManager.GetUserItemCount())
  {
    ...
    if(!pITargetUserItem->IsMobileUser())
    {
      continue;
    }
    ...
    {
      if(wCmdTable!=INVALID_WORD && wTagerTableID != wCmdTable)
      {
        continue;
      }
    }
    ...
    {
      if(!bRecviceGameChat || !bRecviceRoomChat)
      {
        continue;
      }
    }
    ...
    {
      //消息过滤
      if(wMainCmdID==MDM_GR_USER && wSubCmdID==SUB_GR_USER_ENTER && wCmdTable==INVALID_TABLE)
      {
        continue;
      }
      ...
      if(wCmdTable!=INVALID_TABLE)
      {
        ...
        if(wCmdTable==0)
        {
          if((wTagerDeskPos < 0)||(wTagerDeskPos > 2*wTagerDeskCount))
          {
            continue;
          }
        }
        ...
        {
          if((wTagerDeskPos < wLastPagePos-2*wTagerDeskCount)||(wTagerDeskPos > wLastPagePos+wTagerDeskCount))
          {
            continue;
          }
        }
        ...
        {
          if((wTagerDeskPos < wCmdTable-wEverySendPage*wTagerDeskCount)||(wTagerDeskPos > wCmdTable+wEverySendPage*wTagerDeskCount+wTagerDeskCount-1))
          {
            continue;
          }
        }
...
bool CAttemperEngineSink::OnEventUserItemScore(IServerUserItem * pIServerUserItem, BYTE cbReason)
{
  ...
  if((CServerRule::IsImmediateWriteScore(m_pGameServiceOption->dwServerRule)==true)&&(pIServerUserItem->IsVariation()==true))
  {
    ...
    //{
    //  if (pUserInfo->lScore+WriteGameScore.VariationInfo.lScore<0)
    //  {
    //    CString mstr;
    //    mstr.Format(TEXT("OnEventUserItemScore %ld,%I64d,%I64d"),pIServerUserItem->GetUserID(),pUserInfo->lScore,WriteGameScore.VariationInfo.lScore);
    //    CTraceService::TraceString(mstr,TraceLevel_Debug);

    //    WriteGameScore.VariationInfo.lScore=-pUserInfo->lScore;
    //  }
    //}
...
bool CAttemperEngineSink::OnDBLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  //最低分数
  if((m_pGameServiceOption->lMinEnterScore!=0L)&&(pDBOLogonSuccess->lScore+pDBOLogonSuccess->lInsure<m_pGameServiceOption->lMinEnterScore))
  {
    //发送失败
  ...
  //最高分数
  if((m_pGameServiceOption->lMaxEnterScore!=0L)&&(pDBOLogonSuccess->lScore+pDBOLogonSuccess->lInsure>m_pGameServiceOption->lMaxEnterScore))
  {
    //发送失败
    TCHAR szMsg[128]=TEXT("");
  ...
  UserInfo.wTeamID = INVALID_WORD;
  UserInfo.wSortID=INVALID_WORD;
  UserInfo.cbUserStatusEx=US_NULL;
  UserInfo.dwMatchUser=pDBOLogonSuccess->dwUserRight;

  //积分信息
  UserInfo.lScore=pDBOLogonSuccess->lScore;
  UserInfo.lGrade=pDBOLogonSuccess->lGrade;
  UserInfo.lToDayGradeScore=pDBOLogonSuccess->lToDayGradeScore;
  UserInfo.lTolGrade=pDBOLogonSuccess->lTolGrade;
  UserInfo.lTolToDayGrade=pDBOLogonSuccess->lTolToDayGrade;
  ...
  if(m_bCollectUser==true)
  {
    ...
    if(pUserInfo != NULL)
    {
      memcpy(&UserEnter.userInfo, pUserInfo, sizeof(tagUserInfo));
    }
...
bool CAttemperEngineSink::OnDBGameAndroidInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<wHeadSize)||(wDataSize!=(wHeadSize+pGameAndroidInfo->wAndroidCount*sizeof(pGameAndroidInfo->AndroidAccountsInfo[0]))))
  {
    return false;
  }
  ...
  {
    DWORD dwUserCount = m_ServerUserManager.GetUserItemCount();
    m_AndroidUserManager.InsertAndroidInfo(pGameAndroidInfo->AndroidAccountsInfo,pGameAndroidInfo->wAndroidCount,pGameAndroidInfo->dwBatchID,dwUserCount);
  }
...
bool CAttemperEngineSink::OnDBGameAndroidParameter(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<wHeadSize)||(wDataSize!=(wHeadSize+pGameAndroidParameter->wParameterCount*sizeof(pGameAndroidParameter->AndroidParameter[0]))))
  {
    return false;
  }
...
{
  //效验参数
  ASSERT(wDataSize%sizeof(DBO_GR_GamePropertyItem)==0);
  if(wDataSize%sizeof(DBO_GR_GamePropertyItem)!=0)
  {
    return false;
  }

  //变量定义
  WORD wItemCount=wDataSize/sizeof(DBO_GR_GamePropertyItem);
  DBO_GR_GamePropertyItem * pGamePropertyItem=(DBO_GR_GamePropertyItem *)pData;

  //更新数据
  for(WORD i=0; i<wItemCount; i++)
  {
    tagPropertyInfo *pPropertyInfo;
    pPropertyInfo = m_DataConfigProperty.PropertyInfo+m_DataConfigProperty.cbPropertyCount;
    pPropertyInfo->wIndex = (pGamePropertyItem+i)->wIndex;
    lstrcpyn(pPropertyInfo->szName,(pGamePropertyItem+i)->szName,CountArray(pPropertyInfo->szName));
    pPropertyInfo->wKind = (pGamePropertyItem+i)->wKind;
    pPropertyInfo->dPropertyCash = (pGamePropertyItem+i)->dPropertyCash;
    pPropertyInfo->lPropertyGold = (pGamePropertyItem+i)->lPropertyGold;
    pPropertyInfo->lPropertyUserMedal = (pGamePropertyItem+i)->lPropertyUserMedal;
    pPropertyInfo->lPropertyLoveLiness = (pGamePropertyItem+i)->lPropertyLoveLiness;
    pPropertyInfo->wUseArea = (pGamePropertyItem+i)->wUseArea;
    pPropertyInfo->wServiceArea = (pGamePropertyItem+i)->wServiceArea;
    pPropertyInfo->cbSuportMobile = (BYTE)((pGamePropertyItem+i)->cbSuportMobile);
    lstrcpyn(pPropertyInfo->szRegulationsInfo,(pGamePropertyItem+i)->szRegulationsInfo,CountArray(pPropertyInfo->szRegulationsInfo));
    pPropertyInfo->lSendLoveLiness = (pGamePropertyItem+i)->lSendLoveLiness;
    pPropertyInfo->lRecvLoveLiness = (pGamePropertyItem+i)->lRecvLoveLiness;
    pPropertyInfo->lUseResultsGold = (pGamePropertyItem+i)->lUseResultsGold;
    pPropertyInfo->wScoreMultiple = (pGamePropertyItem+i)->wUseResultsValidTimeScoreMultiple;
    pPropertyInfo->wRecommend = (pGamePropertyItem+i)->wRecommend;
    m_DataConfigProperty.cbPropertyCount++;
  }
...
{
  //变量定义
  DBO_GR_GamePropertyListResult * pGamePropertyInfo=(DBO_GR_GamePropertyListResult *)pData;
  //设置道具
  if(pGamePropertyInfo->cbSuccess)
  {
    //设置管理
    m_GamePropertyManager.SetGamePropertyInfo(m_DataConfigProperty.PropertyInfo,m_DataConfigProperty.cbPropertyCount);
  }
...
bool CAttemperEngineSink::OnDBGamePropertyBuff(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if((wDataSize<wHeadSize)||(wDataSize!=(wHeadSize+pGamePropertyBuff->cbBuffCount*sizeof(pGamePropertyBuff->PropertyBuff[0]))))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnDBPropertySuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  tagPropertyInfo* pPropertyInfo = m_GamePropertyManager.SearchPropertyItem(pPropertySuccess->wPropertyIndex);
  ...
  //负分清零
  if(pPropertyInfo->wKind == PROP_KIND_NEGATIVE_SCORE_CLEAR)
  {
  ...
  //逃跑清零
  else if(pPropertyInfo->wKind == PROP_KIND_ESCAPE_CLEAR)
  {
    //用户信息
    tagUserInfo * pUserInfo = pITargetUserItem->GetUserInfo();
    if(pUserInfo==NULL)
  ...
  //VIP卡
  else if(pPropertyInfo->wKind == PROP_KIND_VIP)
  {
    //用户信息
    tagUserInfo * pUserInfo = pITargetUserItem->GetUserInfo();
    if(pUserInfo==NULL)
  ...
  //喇叭判断
  if(pPropertyInfo->wKind==PROP_KIND_TRUMPET|| pPropertyInfo->wKind==PROP_KIND_TYPHON)
  {
    return true;
  }
...
bool CAttemperEngineSink::OnDBPropertyFailure(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(DBO_GR_PropertyFailure))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnDBGamePropertyBuy(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  //构造结构
  CMD_GR_S_PropertyBuyResult PropertySuccess;
  ZeroMemory(&PropertySuccess,sizeof(CMD_GR_S_PropertyBuyResult));
  PropertySuccess.dwPropID = pPropertyBuyResult->dwPropID;
  PropertySuccess.dwPropCount = pPropertyBuyResult->dwPropCount;
  PropertySuccess.lInsureScore = pPropertyBuyResult->lInsureScore;
  PropertySuccess.lUserMedal = pPropertyBuyResult->lUserMedal;
  PropertySuccess.lLoveLiness = pPropertyBuyResult->lLoveLiness;
  PropertySuccess.dCash = pPropertyBuyResult->dCash;
  PropertySuccess.cbSuccessed = pPropertyBuyResult->cbSuccessed;
  PropertySuccess.cbCurrMemberOrder = pPropertyBuyResult->cbCurrMemberOrder;
  lstrcpyn(PropertySuccess.szNotifyContent,pPropertyBuyResult->szNotifyContent,CountArray(PropertySuccess.szNotifyContent));

  WORD wSendSize=sizeof(PropertySuccess)-sizeof(PropertySuccess.szNotifyContent)+CountStringBuffer(PropertySuccess.szNotifyContent);
  m_pITCPNetworkEngine->SendData(dwContextID,MDM_GR_PROPERTY, SUB_GR_GAME_PROPERTY_BUY_RESULT,&PropertySuccess,wSendSize);
...
bool CAttemperEngineSink::OnDBQueryPropertyUse(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if(m_pGameServiceOption->wServerType&GAME_GENRE_GOLD)
  {
    ...
    if(pISourceUserItem)
    {
      pISourceUserItem->ModifyUserInsure(pPropertyUseResult->lUseResultsGold, 0, 0);
    }
  }
...
//加载库存数据
bool CAttemperEngineSink::OnDBLoadStorageConfigInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  DBO_GR_StorageConfigInfo* pStorageConfigInfo = (DBO_GR_StorageConfigInfo*)pData;

  m_StorageConfigInfo.lStorageCurrent = pStorageConfigInfo->lStorageCurrent;
  m_StorageConfigInfo.lStorageMax = pStorageConfigInfo->lStorageMax;
  m_StorageConfigInfo.lStorageStart = pStorageConfigInfo->lStorageStart;
  m_StorageConfigInfo.nStorageDeduct = pStorageConfigInfo->nStorageDeduct;
  return true;
}
//购买结果
...
bool CAttemperEngineSink::OnDBPCCheckInReward(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize!=sizeof(DBO_GR_CheckInReward))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnDBUserTaskInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize>sizeof(DBO_GR_TaskInfo))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnDBUserTaskResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  if(wDataSize>sizeof(DBO_GR_TaskResult))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPSocketMainManagerService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    ...
    case SUB_CS_S_PROPERTY_TRUMPET:  //喇叭消息
    {
      ...
      m_pITCPNetworkEngine->SendDataBatch(MDM_GR_PROPERTY,SUB_GR_PROPERTY_TRUMPET,pData,wDataSize,BG_MOBILE);
...
bool CAttemperEngineSink::OnTCPSocketMainAndroidService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case SUB_CS_S_ADDPARAMETER:   //添加参数
    {
      ...
      if(sizeof(CMD_CS_S_AddParameter)!=wDataSize)
      {
        return false;
      }
    ...
    case SUB_CS_S_MODIFYPARAMETER:  //修改参数
    {
      ...
      if(sizeof(CMD_CS_S_ModifyParameter)!=wDataSize)
      {
        return false;
      }
    ...
    case SUB_CS_S_DELETEPARAMETER:  //删除参数
    {
      ...
      if(sizeof(CMD_CS_S_DeleteParameter)!=wDataSize)
      {
        return false;
      }
...
bool CAttemperEngineSink::OnTCPNetworkMainProperty(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  switch(wSubCmdID)
  {
    case SUB_GR_QUERY_PROPERTY:   //查询道具
    {
      return OnTCPNetworkSubQueryProperty(pData,wDataSize,dwSocketID);
    }
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonUserID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize<sizeof(CMD_GR_LogonUserID))
  {
    return false;
  }
  ...
  if(pBindParameter->dwClientAddr!=0L)
  {
    DWORD dwUserID=pLogonUserID->dwUserID-pLogonUserID->lCode+1258;
    pLogonUserID->dwUserID = dwUserID;
  }
  //重复判断
  ...
  LONG lCode=8623+m_pGameServiceOption->wKindID+m_pGameServiceOption->wServerID*2;
  if(pBindParameter->dwClientAddr!=0L)
  {
    BYTE cbReg=0;
    if(pLogonUserID->lCode != lCode)
    {
      cbReg=1;
    }
    if(cbReg>0)
    {
      //发送失败
      SendLogonFailure(TEXT("很抱歉，请通过正常流程进入游戏！"),LOGON_FAIL_SERVER_INVALIDATION,dwSocketID);
      return true;
    }
  }
  //切换判断
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonMobile(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if((pBindParameter==NULL)||(pIBindUserItem!=NULL))
  {
    CTraceService::TraceString(TEXT("if ((pBindParameter==NULL)||(pIBindUserItem!=NULL))"),TraceLevel_Debug);
    ASSERT(FALSE);
    return false;
  }
  ...
  if(((m_pGameServiceOption->wServerKind&SERVER_GENRE_PASSWD)!=0) && (LOWORD(dwSocketID)<INDEX_ANDROID))
  {
    ...
    if(lstrcmp(pLogonMobile->szServerPasswd,m_pGameServiceOption->szServerPasswd)!=0)
    {
      //发送失败
      SendLogonFailure(TEXT("抱歉，您输入的房间密码不正确，请重新输入！"),0,dwSocketID);
      CTraceService::TraceString(TEXT("if(((m_pGameServiceOption->wServerKind&SERVER_GENRE_PASSWD)!=0) && (LOWORD(dwSocketID)<INDEX_ANDROID))"),TraceLevel_Debug);
      return true;
    }
  }
  ...
  if(pIServerUserItem!=NULL)
  {
    if((pIServerUserItem->IsAndroidUser() && (pBindParameter->dwClientAddr!=0L))
       || (!pIServerUserItem->IsAndroidUser() && (pBindParameter->dwClientAddr==0L)))
    {
      CTraceService::TraceString(TEXT(" 该帐号已在此房间游戏，且不允许踢出，请咨询管理员"),TraceLevel_Debug);
      SendRoomMessage(dwSocketID, TEXT("该帐号已在此房间游戏，且不允许踢出，请咨询管理员！"), SMT_CHAT|SMT_EJECT|SMT_GLOBAL|SMT_CLOSE_ROOM,(pBindParameter->dwClientAddr==0L));
      return false;
    }
  }
  ...
  if(pBindParameter->dwClientAddr!=0L && strPassword.GetLength()==0)
  {
    CTraceService::TraceString(TEXT(" 很抱歉，您的登录密码错误，不允许继续进入"),TraceLevel_Debug);
    //发送失败
    SendLogonFailure(TEXT("很抱歉，您的登录密码错误，不允许继续进入！"),LOGON_FAIL_SERVER_INVALIDATION,dwSocketID);
    return true;
  }
  ...
  //{
  //  CTraceService::TraceString(TEXT("if (PerformCheckVersion(0L,0L,dwClientVersion,dwSocketID)==false) "),TraceLevel_Debug);
  //  return true;
  //}
  ...
  {
    CTraceService::TraceString(TEXT(" 切换判断"),TraceLevel_Debug);
  ...
  {
    CTraceService::TraceString(TEXT(" /比赛效验"),TraceLevel_Debug);
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubLogonAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize<=sizeof(CMD_GR_LogonAccounts))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserRule(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(CServerRule::IsForfendGameRule(m_pGameServiceOption->dwServerRule)==true||m_pGameServiceOption->wServerType==GAME_GENRE_MATCH)
  {
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserLookon(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_UserLookon))
  {
    return false;
  }
  ...
  if(pUserLookon->wChairID>=m_pGameServiceAttrib->wChairCount)
  {
    return false;
  }
  if(pUserLookon->wTableID>=(WORD)m_TableFrameArray.GetCount())
  {
    return false;
  }
  ...
  WORD wChairID=pIServerUserItem->GetChairID();
  BYTE cbUserStatus=pIServerUserItem->GetUserStatus();
  if((wTableID==pUserLookon->wTableID)&&(wChairID==pUserLookon->wChairID)&&(cbUserStatus==US_LOOKON))
  {
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserSitDown(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_UserSitDown))
  {
    return false;
  }
  ...
  if(wRequestTableID>=m_TableFrameArray.GetCount())
  {
    ...
    {
      //游戏状态
      if((m_TableFrameArray[i]->IsGameStarted()==true)&&(bDynamicJoin==false))
      {
        continue;
      }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserStandUp(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(CServerRule::IsAllowAvertCheatMode(m_pGameServiceOption->dwServerRule)&&(m_pGameServiceAttrib->wChairCount < MAX_CHAIR))
  {
    ...
    if(pUserStandUp->wTableID==INVALID_TABLE)
    {
      return true;
    }
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserChat(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  //效验参数
  if(wDataSize>sizeof(CMD_GR_C_UserChat))
  {
    return false;
  }
  if(wDataSize<(sizeof(CMD_GR_C_UserChat)-sizeof(pUserChat->szChatString)))
  {
    return false;
  }
  if(wDataSize!=(sizeof(CMD_GR_C_UserChat)-sizeof(pUserChat->szChatString)+pUserChat->wChatLength*sizeof(pUserChat->szChatString[0])))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserExpression(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_UserExpression))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubWisperChat(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  //效验参数
  if(wDataSize<(sizeof(CMD_GR_C_WisperChat)-sizeof(pWisperChat->szChatString)))
  {
    return false;
  }
  if(wDataSize!=(sizeof(CMD_GR_C_WisperChat)-sizeof(pWisperChat->szChatString)+pWisperChat->wChatLength*sizeof(pWisperChat->szChatString[0])))
  {
    return false;
  }
  ...
  if((CServerRule::IsForfendWisperOnGame(m_pGameServiceOption->dwServerRule)==true)&&(pISendUserItem->GetMasterOrder()==0))
  {
    ...
    //规则判断
    if((bForfend==true)&&(pIRecvUserItem->GetMasterOrder()!=0))
    {
      bForfend=false;
    }
    if((bForfend==true)&&(pIRecvUserItem->GetMasterOrder()!=0))
    ...
    if((bForfend==true)&&(pISendUserItem->GetUserStatus()!=US_PLAYING))
    {
      bForfend=false;
    }
    if((bForfend==true)&&(pIRecvUserItem->GetUserStatus()!=US_PLAYING))
    {
      bForfend=false;
    }
    if((bForfend==true)&&((wTableIDSend==INVALID_TABLE)||(wTableIDSend!=wTableIDRecv)))
    {
      bForfend=false;
    }
  ...
  if(pIRecvUserItem->IsAndroidUser()==true)
  {
    ...
    do
    {
      ...
      {
        if(pTempServerUserItem==pISendUserItem)
        {
          continue;
        }
        if(pTempServerUserItem==pIRecvUserItem)
        {
          continue;
        }
...
bool CAttemperEngineSink::OnTCPNetworkSubWisperExpression(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_WisperExpression))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryProperty(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  //获取用户
  WORD wBindIndex=LOWORD(dwSocketID);
  IServerUserItem * pIServerUserItem=GetBindUserItem(wBindIndex);
  ...
  //枚举数据
  for(DWORD i=0; i<m_DataConfigProperty.cbPropertyCount; i++)
  {
    //发送数据
    if((wSendSize+sizeof(tagPropertyInfo))>sizeof(cbDataBuffer))
    {
      m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GR_PROPERTY,SUB_GR_QUERY_PROPERTY_RESULT,cbDataBuffer,wSendSize);
      wSendSize=0;
    }

    CopyMemory(cbDataBuffer+wSendSize,&(m_DataConfigProperty.PropertyInfo[i]),sizeof(tagPropertyInfo));
    wSendSize+=sizeof(tagPropertyInfo);
  }
  ...
  //发送消息
  m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GR_PROPERTY,SUB_GR_QUERY_PROPERTY_RESULT_FINISH);
...
{
  //效验参数
  ASSERT(wDataSize==sizeof(CMD_GR_C_GamePropertyBuy));
  if(wDataSize!=sizeof(CMD_GR_C_GamePropertyBuy))
  {
    return false;
  }

  //变量定义
  CMD_GR_C_GamePropertyBuy * pPropertyBuy=(CMD_GR_C_GamePropertyBuy *)pData;

  //数据效验
  ASSERT(pPropertyBuy->wItemCount>0);
  if(pPropertyBuy->wItemCount==0)
  {
    return false;
  }

  //变量定义
  DBR_GR_GamePropertyBuy PropertyRequest;
  ...
  PropertyRequest.dwSourceUserID=pPropertyBuy->dwUserID;
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyBackpack(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_BackpackProperty))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyUse(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_PropertyUse))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubQuerySendPresent(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_QuerySendPresent))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubPropertyPresent(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_PropertyPresent))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubGetSendPresent(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_GetSendPresent))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserInviteReq(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_UserInviteReq))
  {
    return false;
  }
  ...
  //效验状态
  if(pUserInviteReq->wTableID==INVALID_TABLE)
  {
    return true;
  }
  if(pIServerUserItem->GetTableID()!=pUserInviteReq->wTableID)
  {
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserRepulseSit(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_UserRepulseSit))
  {
    return false;
  }
  ...
  if(pRepulseIServerUserItem==NULL)
  {
    return true;
  }
  if(pRepulseIServerUserItem->GetUserID() != pUserRepulseSit->dwRepulseUserID)
  {
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMemberKickUser(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_KickUser))
  {
    return false;
  }
  ...
  if((pIServerUserItem==NULL)||(pIServerUserItem->GetMemberOrder()<=pITargetUserItem->GetMemberOrder()))
  {
    return false;
  }
  ...
  if(CUserRight::CanKillOutUser(pIServerUserItem->GetUserRight())==false)
  {
    return false;
  }
  ...
  if(wTargerTableID != INVALID_TABLE)
  {
    ...
    if(pTableFrame->PerformStandUpAction(pITargetUserItem)==false)
    {
      return true;
    }
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubUserChairReq(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  for(INT_PTR i=0; i<(m_pGameServiceOption->wTableCount); i++)
  {
    ...
    //过滤同桌
    if(nTableIndex == pIServerUserItem->GetTableID())
    {
      continue;
    }
    ...
    //获取桌子
    CTableFrame * pTableFrame=m_TableFrameArray[nTableIndex];
    if((pTableFrame->IsGameStarted()==true)&&(bDynamicJoin==false))
    {
      continue;
    }
    ...
    if(pTableFrame->GetChairCount()==pTableFrame->GetNullChairCount())
    {
      continue;
    }
...
bool CAttemperEngineSink::OnTCPNetworkSubChairUserInfoReq(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  {
    //获取用户
    if(pUserInfoReq->wChairID != INVALID_CHAIR && wIndex != pUserInfoReq->wChairID)
    {
      continue;
    }
...
bool CAttemperEngineSink::OnTCPNetworkSubGrowLevelQuery(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_GrowLevelQueryInfo))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubPurchaseMember(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_PurchaseMember))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubExchangeScoreByIngot(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_ExchangeScoreByIngot))
  {
    return false;
  }
  ...
  if(pExchangeScore->lExchangeIngot<=0)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubExchangeScoreByBeans(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_ExchangeScoreByBeans))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryInsureInfo(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_QueryInsureInfoRequest))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubEnableInsureRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_EnableInsureRequest))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubSaveScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_SaveScoreRequest))
  {
    return false;
  }
  ...
  if(pSaveScoreRequest->lSaveScore<=0L)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubTakeScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_TakeScoreRequest))
  {
    return false;
  }
  ...
  if(pTakeScoreRequest->lTakeScore<=0L)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubTransferScoreRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GP_C_TransferScoreRequest))
  {
    return false;
  }
  ...
  if(pTransferScoreRequest->lTransferScore<=0L)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryUserInfoRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_QueryUserInfoRequest))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubCheckInQueryRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_CheckInQueryInfo))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubCheckInDoneRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_CheckInDone))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubTakeTaskRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_TakeTask))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubTaskRewardRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_TaskReward))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubLoadTaskInfoRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_LoadTaskInfo))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubGiveUpTaskRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_C_TakeGiveUp))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMemberQueryInfo(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_MemberQueryInfo))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMemberDayPresent(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_MemberDayPresent))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMemberDayGift(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_MemberDayGift))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubBaseEnsureTakeRequest(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_BaseEnsureTake))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryOption(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if((pIServerUserItem==NULL)||(pIServerUserItem->GetMasterOrder()==0))
  {
    return false;
  }
  ...
  {
    OptionCurrent.dwRuleMask|=SR_ALLOW_DYNAMIC_JOIN;
  }
  if(m_pGameServiceAttrib->cbAndroidUser==TRUE)
  {
    OptionCurrent.dwRuleMask|=SR_ALLOW_ANDROID_ATTEND;
  }
  ...
  {
    OptionCurrent.dwRuleMask|=SR_RECORD_GAME_SCORE;
  }
  if((m_pGameServiceOption->wServerType&(GAME_GENRE_GOLD|GAME_GENRE_MATCH))==0)
  {
    OptionCurrent.dwRuleMask|=SR_IMMEDIATE_WRITE_SCORE;
  }

  //扩展属性
  OptionCurrent.ServerOptionInfo.lStorageStart = m_StorageConfigInfo.lStorageStart;
  OptionCurrent.ServerOptionInfo.lStorageCurrent = m_StorageConfigInfo.lStorageCurrent;
  OptionCurrent.ServerOptionInfo.lStorageMax = m_StorageConfigInfo.lStorageMax;
  OptionCurrent.ServerOptionInfo.nStorageDeduct = m_StorageConfigInfo.nStorageDeduct;
...
bool CAttemperEngineSink::OnTCPNetworkSubOptionServer(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_ServerOption))
  {
    return false;
  }
  ...
  bool bModifyStorageConfig=false;
  if(m_StorageConfigInfo.nStorageDeduct != pServerOptionInfo->nStorageDeduct ||
     m_StorageConfigInfo.lStorageMax != pServerOptionInfo->lStorageMax ||
     m_StorageConfigInfo.lStorageStart != pServerOptionInfo->lStorageStart)
  {
    bModifyStorageConfig=true;
  }
  //削减值合法判断
  if(pServerOptionInfo->nStorageDeduct>=0&& pServerOptionInfo->nStorageDeduct<=1000)
  {
    m_StorageConfigInfo.nStorageDeduct = pServerOptionInfo->nStorageDeduct;
  }

  m_StorageConfigInfo.lStorageMax = pServerOptionInfo->lStorageMax;
  m_StorageConfigInfo.lStorageStart = pServerOptionInfo->lStorageStart;
  m_StorageConfigInfo.lStorageCurrent = pServerOptionInfo->lStorageCurrent;
...
bool CAttemperEngineSink::OnTCPNetworkSubManagerKickUser(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(CMasterRight::CanKillUser(pIServerUserItem->GetMasterRight())==false)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubLimitUserChat(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_LimitUserChat))
  {
    return false;
  }
  ...
  {
    if(CMasterRight::CanLimitRoomChat(pIServerUserItem->GetMasterRight())==false)
    {
      return false;
    }
  ...
  {
    if(CMasterRight::CanLimitGameChat(pIServerUserItem->GetMasterRight())==false)
    {
      return false;
    }
  ...
  {
    if(CMasterRight::CanLimitWisper(pIServerUserItem->GetMasterRight())==false)
    {
      return false;
    }
  ...
  {
    if(CMasterRight::CanLimitUserChat(pIServerUserItem->GetMasterRight())==false)
    {
      return false;
    }
...
bool CAttemperEngineSink::OnTCPNetworkSubKickAllUser(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize > sizeof(CMD_GR_KickAllUser))
  {
    return false;
  }
  ASSERT(wDataSize==CountStringBuffer(pKillAllUser->szKickMessage));
  if(wDataSize!=CountStringBuffer(pKillAllUser->szKickMessage))
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubDismissGame(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(wDataSize!=sizeof(CMD_GR_DismissGame))
  {
    return false;
  }
  ...
  if(CMasterRight::CanDismissGame(pIServerUserItem->GetMasterRight())==false)
  {
    return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubWarningUser(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(CMasterRight::CanSendWarning(pIServerUserItem->GetMasterRight())==false)
  {
    return false;
  }
  ...
  if(pIServerUserItem->GetMasterOrder()<pITargetUserItem->GetMasterOrder())
  {
    return false;
  }
...
VOID CAttemperEngineSink::OnEventUserLogon(IServerUserItem * pIServerUserItem, bool bAlreadyOnLine)
{
  ...
  if(pIServerUserItem->IsMobileUser()==false)
  {
    ...
    //道具配置
    SendPropertyConfig(pBindParameter->dwSocketID);
    ...
    if(m_pIMatchServiceManager!=NULL)
    {
      m_pIMatchServiceManager->OnEventUserLogonFinish(pIServerUserItem);
    }
...
VOID CAttemperEngineSink::OnEventUserLogout(IServerUserItem * pIServerUserItem, DWORD dwLeaveReason)
{
  ...
  if(pIServerUserItem->IsAndroidUser()==true)
  {
    ...
    LeaveGameServer.VariationInfo.lToDayGradeScore=0;
    LeaveGameServer.VariationInfo.lTolGrade=0;
    LeaveGameServer.VariationInfo.lTolToDayGrade=0;
...
bool CAttemperEngineSink::PerformTaskProgress(IServerUserItem * pIServerUserItem,tagUserTaskEntry * pUserTaskEntry,DWORD dwWinCount,DWORD dwLostCount,DWORD dwDrawCount)
{
  ...
  if(pIServerUserItem==NULL || pUserTaskEntry==NULL)
  {
    return false;
  }

  //状态校验
  if(pUserTaskEntry->pTaskParameter==NULL)
  {
    return true;
  }
  ...
  if(pUserTaskEntry->pTaskParameter->wKindID!=m_pGameServiceOption->wKindID)
  {
    return true;
  }
...
bool CAttemperEngineSink::SendUserInfoPacket(IServerUserItem * pIServerUserItem, DWORD dwSocketID,bool bSendAndroidFalg)
{
  ...
  pUserInfoHead->wChairID=pUserInfo->wChairID;
  pUserInfoHead->cbUserStatus=pUserInfo->cbUserStatus;
  pUserInfoHead->cbUserStatusEx=pUserInfo->cbUserStatusEx;
  pUserInfoHead->wSortID=pUserInfo->wSortID;
  ...
  ////随机更改机器人属性
  //if (pIServerUserItem->IsAndroidUser() && (rand()%100<80))
  //{
  //  if (pUserInfo->dwWinCount>80) pUserInfoHead->dwWinCount=rand()%(pUserInfo->dwWinCount/5);
  //  if (pUserInfo->dwLostCount>50)pUserInfoHead->dwLostCount=rand()%(pUserInfo->dwLostCount/5);
  //  if (pUserInfo->dwDrawCount>30)pUserInfoHead->dwDrawCount=rand()%(pUserInfo->dwDrawCount/5);
  //  if (pUserInfo->dwFleeCount>20)pUserInfoHead->dwFleeCount=rand()%(pUserInfo->dwFleeCount/5);
  //  if (pUserInfo->dwExperience>2000000)pUserInfoHead->dwExperience=rand()%(pUserInfo->dwExperience/100)+20000;
  //}

  //用户积分
  pUserInfoHead->lGrade=0;//pUserInfo->lGrade;
...
bool CAttemperEngineSink::SendPropertyConfig(DWORD dwSocketID)
{
  //网络数据
  WORD wSendSize=0;
  BYTE cbDataBuffer[SOCKET_TCP_PACKET];
  ...
  for(DWORD i=0; i<m_DataConfigProperty.cbPropertyCount; i++)
  {
    ...
    if((wSendSize+sizeof(tagPropertyInfo))>sizeof(cbDataBuffer))
    {
      SendData(dwSocketID,MDM_GR_CONFIG,SUB_GR_CONFIG_PROPERTY,cbDataBuffer,wSendSize);
      wSendSize=0;
    }
...
WORD CAttemperEngineSink::GetPropertyType(WORD wPropertyIndex)
{
  switch(wPropertyIndex)
  {
    ...
    case PROPERTY_ID_FOUR_CARD:
    case PROPERTY_ID_SCORE_CLEAR:
    case PROPERTY_ID_ESCAPE_CLEAR:
    case PROPERTY_ID_TRUMPET:
    case PROPERTY_ID_TYPHON:
    case PROPERTY_ID_GUARDKICK_CARD:
    case PROPERTY_ID_POSSESS:
    case PROPERTY_ID_BLUERING_CARD:
    case PROPERTY_ID_YELLOWRING_CARD:
    case PROPERTY_ID_WHITERING_CARD:
    case PROPERTY_ID_REDRING_CARD:
...
{
  //人数校验
  if(m_DistributeManage.GetCount()<m_pGameServiceOption->wMinDistributeUser)
  {
    return false;
  }
  ...
  while(true)
  {
    ...
    {
      //人数校验
      if(m_DistributeManage.GetCount()<m_pGameServiceOption->wMinPartakeGameUser)
      {
        break;
      }
      ...
      if(wDistributeCount < wChairCount)
      {
        break;
      }
...
{
  if(wPageTableCount > m_pGameServiceOption->wTableCount)
  {
    wPageTableCount=m_pGameServiceOption->wTableCount;
  }
...
bool CAttemperEngineSink::SendVisibleTableUserInfoToMobileUser(IServerUserItem * pIServerUserItem,WORD wTablePos)
{
  ...
  while(wEnumIndex<m_ServerUserManager.GetUserItemCount())
  {
    ...
    if(pIUserItem==pIServerUserItem)
    {
      continue;
    }
    ...
    if(wTablePos!=INVALID_WORD && !bViewModeAll)
    {
      ...
      if(wTablePos==0)
      {
        if((wUserPagePos < 0)||(wUserPagePos > 2*wTagerDeskCount))
        {
          continue;
        }
      }
      ...
      {
        if((wUserPagePos < wLastPagePos-2*wTagerDeskCount)||(wUserPagePos > wLastPagePos+wTagerDeskCount))
        {
          continue;
        }
      }
      ...
      {
        if((wUserPagePos < wTablePos-wEverySendPage*wTagerDeskCount)||(wUserPagePos > wTablePos+wEverySendPage*wTagerDeskCount+wTagerDeskCount-1))
        {
          continue;
        }
      }
...
bool CAttemperEngineSink::SendUserInfoPacketBatchToPage(IServerUserItem * pIServerUserItem)
{
  ...
  while(wEnumIndex<m_ServerUserManager.GetUserItemCount())
  {
    ...
    if(pIUserItem->IsMobileUser()==false)
    {
      continue;
    }
    ...
    if(!bViewModeAll)
    {
      ...
      if(wDestPos > wMaxDeskPos)
      {
        wDestPos = wMaxDeskPos;
      }

      if(wTagerDeskPos==0)
      {
        if((wDestPos < 0)||(wDestPos > 2*wTagerDeskCount))
        {
          continue;
        }
      }
      ...
      {
        if((wDestPos < wLastPagePos-2*wTagerDeskCount)||(wDestPos > wLastPagePos+wTagerDeskCount))
        {
          continue;
        }
      }
      ...
      {
        if((wDestPos < wTagerDeskPos-wEverySendPage*wTagerDeskCount)||(wDestPos > wTagerDeskPos+wEverySendPage*wTagerDeskCount+wTagerDeskCount-1))
        {
          continue;
        }
      }
...
//更新库存数据
void CAttemperEngineSink::UpdateStorageConfigInfo()
{
  DBO_GR_StorageConfigInfo StorageConfigInfo;
  ZeroMemory(&StorageConfigInfo,sizeof(DBO_GR_StorageConfigInfo));
  StorageConfigInfo.lStorageCurrent = m_StorageConfigInfo.lStorageCurrent;
  StorageConfigInfo.lStorageMax = m_StorageConfigInfo.lStorageMax;
  StorageConfigInfo.lStorageStart = m_StorageConfigInfo.lStorageStart;
  StorageConfigInfo.nStorageDeduct = m_StorageConfigInfo.nStorageDeduct;
  StorageConfigInfo.wKindID = m_pGameServiceOption->wKindID;
  StorageConfigInfo.wServerID = m_pGameServiceOption->wServerID;

  m_pIDBCorrespondManager->PostDataBaseRequest(0L,DBR_GR_UPDATE_STORAGE_CONFIG_INFO,0L,&StorageConfigInfo,sizeof(DBO_GR_StorageConfigInfo));

}
//用户起立
void CAttemperEngineSink::SetUserStandUp(int nIndex)
{
  //NativeInfoRecode("GameServerClose.log","KindID:%d,ServerID:%d nIndex:%d",m_pGameServiceOption->wKindID,m_pGameServiceOption->wServerID,nIndex);
  try
  {
    for(INT_PTR i=0; i<m_TableFrameArray.GetCount(); i++)
    {
    ...
    INT_PTR nTableCount=m_TableFrameArray.GetCount();
    DWORD dwOnLineCount=m_ServerUserManager.GetUserItemCount();
    WORD wUserIndex=0;
    IServerUserItem * pIServerUserItem=NULL;
    while(wUserIndex<dwOnLineCount)
    {
      pIServerUserItem=m_ServerUserManager.EnumUserItem(wUserIndex++);
      if(pIServerUserItem==NULL)
      {
        break;
      }
      ...
      SendRoomMessage(pIServerUserItem,TEXT("本游戏房间维护，退出游戏房间。"),SMT_CLOSE_ROOM);
    }
  }
  catch(...)
  {
    CTraceService::TraceString(TEXT("SetUserStandUp"),TraceLevel_Normal);
  }

}
//游戏结束
bool CAttemperEngineSink::TableGameEnd(WORD wTableID, WORD wTeamID)
{
  return true;
}
//游戏结束
bool CAttemperEngineSink::WriteUserReward(IServerUserItem * pIServerUserItem, __int64 lGold, WORD wType)
{
  return true;
}
void CAttemperEngineSink::LoadGateIP()
{
  m_wGateCount = 0;
  ZeroMemory(m_dwGataIP,sizeof(m_dwGataIP));
  ...
  _sntprintf(szIniFile,CountArray(szIniFile),TEXT("%s\\GateParameter.ini"),szWorkDir);

  //读取代理数量
  WORD GateCount = GetPrivateProfileInt(TEXT("GateInfo"),TEXT("GateCount"),0,szIniFile);
  CString Msg;
  if(GateCount>0)
  {
    Msg.Format(TEXT("加载代理文件 GateParameter.ini 加载[%d]条代理信息"), GateCount);
    CTraceService::TraceString(Msg,TraceLevel_Normal);
  }
  ...
  {
    szKeyName.Empty();
    szKeyName.Format(TEXT("GateIP_%d"), i);
    ZeroMemory(szIP, sizeof(TCHAR)*15);

    GetPrivateProfileString(L"GateInfo",szKeyName,L"8.8.8.8", szIP, sizeof(szIP),szIniFile);
    CT2CA strServerDomain(szIP);
    m_dwGataIP[i] = inet_addr(strServerDomain);


    Msg.Format(TEXT("%s - %d"), szIP, m_dwGataIP[i]);
    CTraceService::TraceString(Msg,TraceLevel_Normal);
  }
  m_wGateCount = GateCount;
}
//检查是否是代理IP
bool CAttemperEngineSink::CheckGateIP(DWORD dwClientIP)
{
  //校验合法性
  if((m_wGateCount >= 100) || (m_wGateCount <=0))
  {
    return false;
  }
  for(int i=0; i<m_wGateCount; i++)
  {
    if(m_dwGataIP[i] == dwClientIP)
    {
      return true;
    }
  }
