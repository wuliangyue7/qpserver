...
protected:
  ...
  tagStorageConfigInfo            m_StorageConfigInfo;               //库存配置
...
protected:
  ...
  //网关信息
protected:
  DWORD             m_dwGataIP[100];          //代理地址IP
  WORD              m_wGateCount;           //代理数量
...
public:
  ...
  //发送消息
  virtual bool SendGlobalMessage(LPCTSTR lpszMessage, WORD wType);
...
public:
  ...
  //获取库存配置数据
  virtual tagStorageConfigInfo* GetStorageConfigInfo()
  {
    return &m_StorageConfigInfo;
  }
  //比赛组结束
  virtual bool TableGameEnd(WORD wTableID, WORD wTeamID);
  virtual bool WriteUserReward(IServerUserItem * pIServerUserItem, __int64 lGold, WORD wType);
...
protected:
  ...
  //加载库存数据
  bool OnDBLoadStorageConfigInfo(DWORD dwContextID, VOID * pData, WORD wDataSize);
...
protected:
  ...
  //查询道具
  bool OnTCPNetworkSubQueryProperty(VOID * pData, WORD wDataSize, DWORD dwSocketID);
  //购买道具
...
protected:
  ...
  bool SendPropertyConfig(DWORD dwSocketID);
...
protected:
  ...
  //更新库存数据
  void UpdateStorageConfigInfo();
  //用户起立
  void SetUserStandUp(int nIndex);
...
protected:
  ...
  //检查网关IP
  bool CheckGateIP(DWORD dwClientIP);
