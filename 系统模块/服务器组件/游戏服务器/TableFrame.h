...
public:
  ...
  {
    return m_pGameServiceOption->cbCustomRule;
  };
  //比赛配置
...
public:
  ...
  {
    return m_cbStartMode;
  }
  //开始模式
  virtual VOID SetStartMode(BYTE cbStartMode)
  {
    m_cbStartMode=cbStartMode;
  }
...
public:
  //房间机器库银
  virtual LONGLONG GetStorageStart();
  //库存相关
  virtual void SetStorageStart(LONGLONG lStorageStart);
  //库存相关
  virtual LONGLONG GetStorageMax();
  virtual void SetStorageMaxt(LONGLONG lStorageMax);
  //库存相关
  virtual LONGLONG GetStorageCurrent();
  virtual int GetStorageDeduct();
  virtual void SetStorageDeduct(int nStorageDeduct);
...
public:
  ...
  //房间库存变动
  bool StorageScoreChange(WORD wChairID,tagScoreInfo &ScoreInfo);
};
