...
bool CDataBaseEngineSink::OnDataBaseEngineRequest(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  switch(wRequestID)
  {
    ...
    break;
    case DBR_GR_UPDATE_ONLINE_COUNT:  //在线人数
    {
      bSucceed = OnRequestUpdateOnlineCount(dwContextID,pData,wDataSize,dwUserID);
    }
    break;
    case DBR_GR_LOAD_STORAGE_CONFIG_INFO: //库存数据
    {
      bSucceed = OnRequestLoadStorageConfigInfo(dwContextID,pData,wDataSize,dwUserID);
    }
    break;
    case DBR_GR_UPDATE_STORAGE_CONFIG_INFO: //更新库存数据
    {
      bSucceed = OnRequestUpdateStorageConfigInfo(dwContextID,pData,wDataSize,dwUserID);
    }
    break;
  }
  ...
  {
    CString str2;
    str2.Format(TEXT("OnDataBaseEngineRequest %d,%ld,%d"),wRequestID,dwUserID,bSucceed);
...
bool CDataBaseEngineSink::OnRequestLogonUserID(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID,BOOL bMatch)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_LogonUserID))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestLogonMobile(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_LogonMobile))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestLogonAccounts(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_LogonAccounts))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestWriteGameScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_WriteGameScore))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestLeaveGameServer(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_LeaveGameServer))
    {
      return false;
    }
    ...
    long lWinCount = pLeaveGameServer->VariationInfo.dwWinCount;
    long lLostCount = pLeaveGameServer->VariationInfo.dwLostCount;
    long lDrawCount = pLeaveGameServer->VariationInfo.dwDrawCount;
    long lFleeCount = pLeaveGameServer->VariationInfo.dwFleeCount;
    long lExperience = pLeaveGameServer->VariationInfo.dwExperience;
    long lPlayTimeCount = pLeaveGameServer->VariationInfo.dwPlayTimeCount;
    if(pClientAddr[0]==0)
    {
      ////离开时随机更改机器人属性
      __time64_t tNowTime = (DWORD)time(NULL);
      CTime tCNowTime(tNowTime);
      if(tCNowTime.GetDayOfWeek()==1 || tCNowTime.GetDayOfWeek()==4)
      {
        lWinCount = -lWinCount;
    ...
    m_GameDBAide.AddParameter(TEXT("@lVariationWinCount"),lWinCount);
    m_GameDBAide.AddParameter(TEXT("@lVariationLostCount"),lLostCount);
    m_GameDBAide.AddParameter(TEXT("@lVariationDrawCount"),lDrawCount);
    m_GameDBAide.AddParameter(TEXT("@lVariationFleeCount"),lFleeCount);
    ...
    m_GameDBAide.AddParameter(TEXT("@lVariationExperience"),lExperience);
    m_GameDBAide.AddParameter(TEXT("@lVariationLoveLiness"),pLeaveGameServer->VariationInfo.lLoveLiness);
    m_GameDBAide.AddParameter(TEXT("@dwVariationPlayTimeCount"),lPlayTimeCount);
...
bool CDataBaseEngineSink::OnRequestGameScoreRecord(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    //效验参数
    if(wDataSize>sizeof(DBR_GR_GameScoreRecord))
    {
      return false;
    }
    if(wDataSize<(sizeof(DBR_GR_GameScoreRecord)-sizeof(pGameScoreRecord->GameScoreRecord)))
    {
      return false;
    }
    if(wDataSize!=(sizeof(DBR_GR_GameScoreRecord)-sizeof(pGameScoreRecord->GameScoreRecord)+pGameScoreRecord->wRecordCount*sizeof(pGameScoreRecord->GameScoreRecord[0])))
    {
      return false;
    }
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_RecordDrawInfo"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestLoadGameColumn(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    for(WORD i=0; i<CountArray(GameColumnInfo.ColumnItemInfo); i++)
    {
      ...
      if(GameColumnInfo.cbColumnCount>=CountArray(GameColumnInfo.ColumnItemInfo))
      {
        break;
      }
...
bool CDataBaseEngineSink::OnRequestLoadAndroidUser(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(sizeof(DBR_GR_LoadAndroidUser)!=wDataSize)
  {
    return false;
  }
  ...
  try
  {
    ...
    for(WORD i=0; i<CountArray(GameAndroidInfo.AndroidAccountsInfo); i++)
    {
      ...
      if(GameAndroidInfo.wAndroidCount>=CountArray(GameAndroidInfo.AndroidAccountsInfo))
      {
        break;
      }
...
bool CDataBaseEngineSink::OnRequestLoadAndroidParameter(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    DWORD dwLeaveMinInterval=3600;
    DWORD dwLeaveMaxInterval=7300;
    DWORD dwRandInterval=18000+(rand()%50)*1523;
    //读取信息
    for(WORD i=0; i<CountArray(GameAndroidParameter.AndroidParameter); i++)
    {
      ...
      if(GameAndroidParameter.wParameterCount>=CountArray(GameAndroidParameter.AndroidParameter))
      {
        break;
      }
      ...
      dwLeaveMinInterval=m_TreasureDBAide.GetValue_DWORD(TEXT("LeaveMinInterval"));
      dwLeaveMaxInterval=m_TreasureDBAide.GetValue_DWORD(TEXT("LeaveMaxInterval"));
      if(dwLeaveMinInterval<1000)
      {
        dwLeaveMinInterval = dwRandInterval;
        dwLeaveMaxInterval += dwRandInterval;
      }
      if(dwLeaveMaxInterval<=dwLeaveMinInterval)
      {
        dwLeaveMaxInterval = dwLeaveMinInterval+dwRandInterval;
      }
      GameAndroidParameter.AndroidParameter[i].dwLeaveMinInterval=dwLeaveMinInterval;
      GameAndroidParameter.AndroidParameter[i].dwLeaveMaxInterval=dwLeaveMaxInterval;
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_LoadAndroidConfigure"),TraceLevel_Exception);
    //变量定义
    DBO_GR_GameAndroidParameter GameAndroidParameter;
    ZeroMemory(&GameAndroidParameter,sizeof(GameAndroidParameter));
...
bool CDataBaseEngineSink::OnRequestLoadGameProperty(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    DBO_GR_GamePropertyItem  * pGamePropertyItem=NULL;
    while(m_PlatformDBModule->IsRecordsetEnd()==false)
    {
      //发送信息
      if((wPacketSize+sizeof(DBO_GR_GamePropertyItem))>(sizeof(cbBuffer)-sizeof(NTY_DataBaseEvent)))
      {
        m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_GAME_PROPERTY_ITEM,dwContextID,cbBuffer,wPacketSize);
        wPacketSize=0;
      }

      //读取信息
      pGamePropertyItem=(DBO_GR_GamePropertyItem *)(cbBuffer+wPacketSize);
	  ...
	  pGamePropertyItem->cbSuportMobile = m_PlatformDBAide.GetValue_WORD(TEXT("SuportMobile"));
      ...
      //设置位移
      wPacketSize+=sizeof(DBO_GR_GamePropertyItem);
    ...
    //变量定义
    DBO_GR_GamePropertyListResult GamePropertyResult;
    ...
    //发送消息
    m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBO_GR_GAME_PROPERTY_LIST_RESULT,dwContextID,&GamePropertyResult,sizeof(GamePropertyResult));
...
bool CDataBaseEngineSink::OnRequestUserEnableInsure(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_UserEnableInsure))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestUserSaveScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_UserSaveScore))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestUserTakeScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_UserTakeScore))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestUserTransferScore(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_UserTransferScore))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestQueryInsureInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_QueryInsureInfo))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestQueryTransferUserInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_QueryTransferUserInfo))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestTaskGiveUp(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_TaskGiveUp))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestTaskTake(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_TaskTake))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestTaskReward(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_TaskReward))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestTaskQueryInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_TaskQueryInfo))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestPurchaseMember(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_PurchaseMember))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestExchangeScoreByIngot(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_ExchangeScoreByIngot))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestExchangeScoreByBeans(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_ExchangeScoreByBeans))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestBuyGameProperty(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  {
    //效验参数
    ASSERT(wDataSize==sizeof(DBR_GR_GamePropertyBuy));
    if(wDataSize!=sizeof(DBR_GR_GamePropertyBuy))
    {
      return false;
    }

    //请求处理
    DBR_GR_GamePropertyBuy * pPropertyBuy=(DBR_GR_GamePropertyBuy *)pData;
    ...
    dwUserID = pPropertyBuy->dwSourceUserID;
    //构造参数
    m_PlatformDBAide.ResetParameter();
...
bool CDataBaseEngineSink::OnRequestPropertyBackpack(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_QueryBackpack))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestPropertyUse(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_PropertyUse))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestLoadGameTrumpet(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GP_LoadUserGameTrumpet"),TraceLevel_Exception);
    //变量定义
    DBO_GR_UserGameTrumpet UserGameTrumpet;
    ZeroMemory(&UserGameTrumpet,sizeof(UserGameTrumpet));
...
bool CDataBaseEngineSink::OnRequestSendTrumpet(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("OnRequestSendTrumpet"),TraceLevel_Exception);
    //变量定义
    DBO_GR_Send_Trumpet SendTrumpetResult= {0};
...
bool CDataBaseEngineSink::OnRequestMemberQueryInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_MemberQueryInfo))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestMemberDayPresent(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_MemberDayPresent))
    {
      return false;
    }
    ...
    if(lResultCode==DB_SUCCESS)
    {
      pDBOMemberInfoResult.lGameScore = m_PlatformDBAide.GetValue_BYTE(TEXT("Score"));
    }
...
bool CDataBaseEngineSink::OnRequestMemberDayGift(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_MemberDayGift))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestCheckInQueryInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_CheckInQueryInfo))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestCheckInDone(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_CheckInDone))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestTakeBaseEnsure(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_TakeBaseEnsure))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestWriteUserGameData(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize1>=wDataSize && wDataSize!=wDataSize1+wHeadSize1)
    {
      return false;
    }
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_WriteUserGameData"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchSignup(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchSignup))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchUnSignup(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchUnSignup))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchSignupStart(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_SignupStart))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchSignupStart"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchQueryQualify(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_QueryMatchQualify))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchQueryQualify"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchBuySafeCard(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchBuySafeCard))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchBuySafeCard"),TraceLevel_Exception);
    //构造结构
    DBO_GR_MatchBuyResult MatchBuyResult;
    ZeroMemory(&MatchBuyResult,sizeof(MatchBuyResult));
...
bool CDataBaseEngineSink::OnRequestMatchStart(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchStart))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchOver(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchOver))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchCancel(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchCancel))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchCancel"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchReward(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchReward))
  {
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchEliminate(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchEliminate))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchEliminate"),TraceLevel_Exception);
    return false;
  }
...
bool CDataBaseEngineSink::OnRequestMatchUserRevive(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchUserRevive))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchUserRevive"),TraceLevel_Exception);
    //构造结构
    DBO_GR_MatchReviveResult MatchReviveResult;
    ZeroMemory(&MatchReviveResult,sizeof(MatchReviveResult));
...
bool CDataBaseEngineSink::OnRequestMatchQueryReviveInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  if(wDataSize!=sizeof(DBR_GR_MatchQueryRevive))
  {
    return false;
  }
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_MatchQueryReviveInfo"),TraceLevel_Exception);
    //构造结果
    DBO_GR_MatchReviveInfo MatchReviveInfo;
    ZeroMemory(&MatchReviveInfo,sizeof(MatchReviveInfo));
...
bool CDataBaseEngineSink::OnRequestQueryGrowLevelParameter(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_GrowLevelQueryInfo))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestManageUserRight(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_ManageUserRight))
    {
      return false;
    }
...
bool CDataBaseEngineSink::OnRequestManageMatchRight(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  ...
  catch(IDataBaseException * pIException)
  {
    //错误信息
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_ManageMatchRight"),TraceLevel_Exception);
    //错误处理
...
bool CDataBaseEngineSink::OnRequestUnlockAndroidUser(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    if(wDataSize!=sizeof(DBR_GR_UnlockAndroidUser))
    {
      return false;
    }
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GR_UnlockAndroidUser"),TraceLevel_Exception);
    return false;
  }
...
//在线人数
bool CDataBaseEngineSink::OnRequestUpdateOnlineCount(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    //效验参数
    ASSERT(wDataSize==sizeof(DBO_GR_UpdateOnLineCount));
    if(wDataSize!=sizeof(DBO_GR_UpdateOnLineCount))
    {
      return false;
    }

    //请求处理
    DBO_GR_UpdateOnLineCount * pUpdateOnLineCount=(DBO_GR_UpdateOnLineCount *)pData;

    //构造参数
    m_TreasureDBAide.ResetParameter();
    m_TreasureDBAide.AddParameter(TEXT("@dwKindID"),pUpdateOnLineCount->wKindID);
    m_TreasureDBAide.AddParameter(TEXT("@dwServerID"),pUpdateOnLineCount->wServerID);
    m_TreasureDBAide.AddParameter(TEXT("@dwOnLineCountSum"),pUpdateOnLineCount->dwOnLineCount);
    m_TreasureDBAide.AddParameter(TEXT("@dwMBCountSum"),pUpdateOnLineCount->dwMBOnLineCount);

    //执行查询
   // LONG lResultCode=m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_UpdateOnLineCount"),false);
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GP_UpdateOnLineCount"),TraceLevel_Exception);
    return false;
  }
  return true;
}
//库存数据
bool CDataBaseEngineSink::OnRequestLoadStorageConfigInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    ...
    //用户帐户
    m_TreasureDBAide.ResetParameter();
    m_TreasureDBAide.AddParameter(TEXT("@ServerID"),m_pGameServiceOption->wServerID);

    //执行查询
    LONG lResultCode=m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_LoadRoomStorage"),true);

    //执行成功
    if(lResultCode==DB_SUCCESS)
    {
      StorageConfigInfo.lStorageStart=m_TreasureDBAide.GetValue_LONGLONG(TEXT("StorageStart"));
      StorageConfigInfo.lStorageCurrent=m_TreasureDBAide.GetValue_LONGLONG(TEXT("ServiceCurrent"));
      StorageConfigInfo.lStorageMax=m_TreasureDBAide.GetValue_LONGLONG(TEXT("StorageMax"));
      StorageConfigInfo.nStorageDeduct=m_TreasureDBAide.GetValue_LONGLONG(TEXT("StorageDeduct"));
    }

    m_pIDataBaseEngineEvent->OnEventDataBaseResult(DBR_GR_LOAD_STORAGE_CONFIG_INFO_RESULT,dwContextID,&StorageConfigInfo,sizeof(DBO_GR_StorageConfigInfo));
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GP_LoadRoomStorage"),TraceLevel_Exception);
...
//库存数据
bool CDataBaseEngineSink::OnRequestUpdateStorageConfigInfo(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
  try
  {
    //效验参数
    ASSERT(wDataSize==sizeof(DBO_GR_StorageConfigInfo));
    if(wDataSize!=sizeof(DBO_GR_StorageConfigInfo))
    {
      return false;
    }
    ...
    m_TreasureDBAide.ResetParameter();
    m_TreasureDBAide.AddParameter(TEXT("@ServerID"),m_pGameServiceOption->wServerID);
    m_TreasureDBAide.AddParameter(TEXT("@StorageDeduct"),pStorageConfigInfo->nStorageDeduct);
    m_TreasureDBAide.AddParameter(TEXT("@StorageStart"),pStorageConfigInfo->lStorageStart);
    m_TreasureDBAide.AddParameter(TEXT("@StorageMax"),pStorageConfigInfo->lStorageMax);
    m_TreasureDBAide.AddParameter(TEXT("@ServiceCurrent"),pStorageConfigInfo->lStorageCurrent);

    //执行查询
    LONG lResultCode=m_TreasureDBAide.ExecuteProcess(TEXT("GSP_GP_ModifyRoomStorage"),true);
  ...
  catch(IDataBaseException * pIException)
  {
    //输出错误
    CTraceService::TraceString(pIException->GetExceptionDescribe(),TraceLevel_Exception);
    CTraceService::TraceString(TEXT("GSP_GP_ModifyRoomStorage"),TraceLevel_Exception);
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
  if(dwErrorCode==DB_SUCCESS)
  {
    ...
    if((m_pGameServiceOption->wServerType&GAME_GENRE_GOLD)!=0)
    {
      ...
      //m_LogonSuccess.lToDayGradeScore=m_GameDBAide.GetValue_LONGLONG(TEXT("ToDayGradeScore"));
      //m_LogonSuccess.lTolGrade=m_GameDBAide.GetValue_LONGLONG(TEXT("TolGrade"));
      //m_LogonSuccess.lTolToDayGrade=m_GameDBAide.GetValue_LONGLONG(TEXT("TolToDayGrade"));
    }
