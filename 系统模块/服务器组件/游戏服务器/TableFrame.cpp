...
bool CTableFrame::StartGame()
{
  ...
  {
    bStart=m_pITableFrameHook->OnEventGameStart(m_wChairCount);
  }
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus)
{
  ...
  if(m_pITableFrameHook!=NULL)
  {
    m_pITableFrameHook->OnEventGameEnd(0, NULL, cbGameStatus);
  }
  ...
  if(m_bGameStarted==false)
  {
    LONGLONG lMaxTableUserScore=1;
    if((m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)&&(m_pGameServiceAttrib->wChairCount<MAX_CHAIR))
    {
      for(WORD i=0; i<m_wChairCount; i++)
      {
        IServerUserItem * pIServerUserItem=GetTableUserItem(i);
        ...
        //最大分
        if(pIServerUserItem->GetUserScore()>lMaxTableUserScore)
        {
          if(!pIServerUserItem->IsAndroidUser())
          {
            lMaxTableUserScore=pIServerUserItem->GetUserScore();
          }
        }
      }
      for(WORD i=0; i<m_wChairCount; i++)
      {
        ...
        if(pIServerUserItem->GetUserScore()<(lMaxTableUserScore/60))
        {
    ...
    for(WORD i=0; i<m_wChairCount; i++)
    {
      ...
      {
        //机器处理
        if((pIServerUserItem->IsAndroidUser()==true)&&(wStandUpCount<=wRandCount))
        {
          //绑定信息
...
bool CTableFrame::ConcludeTable()
{
  ...
  if((m_bGameStarted==false)&&(m_bTableStarted==true))
  {
    ...
    if(m_cbStartMode==START_MODE_PAIR_READY)
    {
      m_bTableStarted=false;
    }
    if(m_cbStartMode==START_MODE_ALL_READY)
    {
      m_bTableStarted=false;
    }
  }
...
bool CTableFrame::WriteUserScore(WORD wChairID, tagScoreInfo & ScoreInfo, DWORD dwGameMemal, DWORD dwPlayGameTime)
{
  ...
  if(pIServerUserItem!=NULL)
  {
    ...
    if((m_pGameServiceOption->wServerType == GAME_GENRE_GOLD)&&(pIServerUserItem->IsAndroidUser()==false))
    {
      //防止负分
      LONGLONG lUserScore = pIServerUserItem->GetUserScore();
      if(lUserScore>0 && ScoreInfo.lScore<0)
      {
        if((ScoreInfo.lScore+lUserScore)<0)
        {
          CString mstr;
          mstr.Format(TEXT("WriteUserScore %ld,%I64d,%I64d"),pIServerUserItem->GetUserID(),lUserScore,ScoreInfo.lScore);
          CTraceService::TraceString(mstr,TraceLevel_Debug);
          ScoreInfo.lScore = -lUserScore;
        }
      }
      //库存计算
      if(ScoreInfo.lScore != 0)
      {
        StorageScoreChange(wChairID,ScoreInfo);
      }
    }
...
SCORE CTableFrame::QueryConsumeQuota(IServerUserItem * pIServerUserItem)
{
  ...
  if((lUserConsumeQuota<0L)||(lUserConsumeQuota>pIServerUserItem->GetUserScore()-lMinEnterScore))
  {
    return 0L;
  }
...
{
  if(wEnumIndex>=m_LookonUserItemArray.GetCount())
  {
    return NULL;
  }
...
//发送消息
bool CTableFrame::SendGlobalMessage(LPCTSTR lpszMessage, WORD wType)
{
  m_pIMainServiceFrame->SendGlobalMessage(lpszMessage,wType);
  return true;
}
//设置接口
...
bool CTableFrame::SendChatMessage(IServerUserItem * pIServerUserItem,IServerUserItem * pITargetServerUserItem,DWORD dwChatColor, LPCTSTR lpszChatString, LPTSTR lpszDescribeString)
{
  ...
  if(wDescribeLength>LEN_USER_CHAT)
  {
    wDescribeLength=LEN_USER_CHAT;
  }
  WORD wChatLength = lstrlen(lpszChatString) + 1;
  if(wChatLength>LEN_USER_CHAT)
  {
    wChatLength=LEN_USER_CHAT;
  }
...
bool CTableFrame::OnEventUserOffLine(IServerUserItem * pIServerUserItem)
{
  ...
  if(cbUserStatus!=US_LOOKON)
  {
    ...
    if(pIServerUserItem!=GetTableUserItem(wChairID))
    {
      return false;
    }
...
bool CTableFrame::OnEventTimer(DWORD dwTimerID, WPARAM dwBindParameter)
{
  ...
  if(dwTimerID >= IDI_START_OVERTIME && dwTimerID <=IDI_START_OVERTIME_END)
  {
    ...
    //
    if(wChair != (WORD)(dwTimerID-IDI_START_OVERTIME))
    {
      return false;
    }
...
{
  //游戏处理
  if(m_pITableFrameSink->OnFrameMessage(wSubCmdID,pData,wDataSize,pIServerUserItem)==true)
  {
    return true;
  }
  ...
  switch(wSubCmdID)
  {
    case SUB_GF_GAME_OPTION:  //游戏配置
    {
      ...
      if(wDataSize!=sizeof(CMD_GF_GameOption))
      {
        return false;
      }
      ...
      if(m_pGameServiceOption->wServerType==GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType==MATCH_TYPE_IMMEDIATE)
      {
        ...
        if(cbUserStatus==US_PLAYING || cbUserStatus==US_OFFLINE)
        {
          ...
          {
            m_pITableFrameHook->OnEventUserReturnMatch(pIServerUserItem);
          }
        }
      }
    ...
    case SUB_GF_USER_EXPRESSION:  //用户表情
    {
      ...
      if(wDataSize!=sizeof(CMD_GF_C_UserExpression))
      {
        return false;
      }
    ...
    case SUB_GF_LOOKON_CONFIG:    //旁观配置
    {
      ...
      if(wDataSize<sizeof(CMD_GF_LookonConfig))
      {
        return false;
      }
      ...
      if(pLookonConfig->dwUserID!=0L)
      {
        for(INT_PTR i=0; i<m_LookonUserItemArray.GetCount(); i++)
        {
          ...
          if(pILookonUserItem->GetUserID()!=pLookonConfig->dwUserID)
          {
            continue;
          }
          if(pILookonUserItem->GetChairID()!=pIServerUserItem->GetChairID())
          {
            continue;
          }
      ...
      else
      {
        ...
        if(bAllowLookon==m_bAllowLookon[pIServerUserItem->GetChairID()])
        {
          return true;
        }
...
bool CTableFrame::InitializationFrame(WORD wTableID, tagTableFrameParameter & TableFrameParameter)
{
  ...
  if(m_pITableFrameSink->Initialization(pITableFrame)==false)
  {
    return false;
  }
  ...
  if(pIServerUserItem==pITableUserItem)
  {
    ...
    try
    {
      if(pIServerUserItem->IsAndroidUser())
      {
        ...
        //游戏时间
        DWORD dwCurrentTime=(DWORD)time(NULL);
        TimeInfo->dwEnterTableTimer=dwCurrentTime;
      }
    }
    catch(...)
    {
      CTraceService::TraceString(TEXT("PerformStandUpAction1."),TraceLevel_Debug);
    }
    return true;
  }
  else
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
  ...
  //规则效验
  if((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)==0 && EfficacyEnterTableScoreRule(wChairID,pIServerUserItem)==false)
  {
    return false;
  }
  if((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)==0 && EfficacyIPAddress(pIServerUserItem)==false)
  {
    return false;
  }
  if((m_pGameServiceOption->wServerType&GAME_GENRE_MATCH)==0 && EfficacyScoreRule(pIServerUserItem)==false)
  {
    return false;
  }
  ...
  {
    CTraceService::TraceString(TEXT("PerformSitDownAction1."),TraceLevel_Debug);
  }
  return true;
}
...
//房间机器库银
LONGLONG CTableFrame::GetStorageStart()
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
    return 0;
  }

  return pStorageConfigInfo->lStorageStart;
}
//库存相关
void CTableFrame::SetStorageStart(LONGLONG lStorageStart)
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  pStorageConfigInfo->lStorageStart = lStorageStart;
}
//库存相关
LONGLONG CTableFrame::GetStorageMax()
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  return pStorageConfigInfo->lStorageMax;
}
void CTableFrame::SetStorageMaxt(LONGLONG lStorageMax)
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  pStorageConfigInfo->lStorageMax = lStorageMax;
}
//库存相关
LONGLONG CTableFrame::GetStorageCurrent()
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  return pStorageConfigInfo->lStorageCurrent;
}
int CTableFrame::GetStorageDeduct()
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  return pStorageConfigInfo->nStorageDeduct;
}
void CTableFrame::SetStorageDeduct(int nStorageDeduct)
{
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  {
  ...
  pStorageConfigInfo->nStorageDeduct = nStorageDeduct;
}
//房间库存变动
bool CTableFrame::StorageScoreChange(WORD wChairID,tagScoreInfo &ScoreInfo)
{
  //库存配置信息
  tagStorageConfigInfo* pStorageConfigInfo = m_pIMainServiceFrame->GetStorageConfigInfo();
  if(pStorageConfigInfo == NULL)
  ...
  //统计变量
  LONGLONG lRobotScoreResult = 0;
  LONGLONG lStorageDeduct = 0;
  ...
  if(pIServerUserItem->IsAndroidUser()==false)
  {
    if(ScoreInfo.cbType != SCORE_TYPE_NULL)
    {
      lRobotScoreResult += ScoreInfo.lScore;
    }
  }

  //库银的增量是玩家的相反数
  LONGLONG llAddStorage = -lRobotScoreResult;

  //统计库银
  if(llAddStorage>0)
  {
    lStorageDeduct = (LONGLONG)(llAddStorage*pStorageConfigInfo->nStorageDeduct)/1000;
  }
  //CString str;
  //str.Format(TEXT("RobotStorageStart:%I64d,llAddStorage:%I64d,lStorageDeduct:%I64d"),pStorageConfigInfo->lStorageCurrent,llAddStorage,lStorageDeduct);
  pStorageConfigInfo->lStorageCurrent += llAddStorage-lStorageDeduct;
