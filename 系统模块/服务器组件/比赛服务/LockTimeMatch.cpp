...
void CLockTimeMatch::OnStartService()
{
  ...
  {
    if(m_pMatchOption->cbMatchStatus==MS_FREE)
    {
      m_MatchStatus=MatchStatus_Free;
    }
    if(m_pMatchOption->cbMatchStatus==MS_MATCHEND)
    {
      m_MatchStatus=MatchStatus_End;
    }
    if(m_pMatchOption->cbMatchStatus==MS_MATCHING)
    {
...
bool CLockTimeMatch::OnEventDataBase(WORD wRequestID, IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize,DWORD dwContextID)
{
  switch(wRequestID)
  {
    case DBO_GR_MATCH_QUERY_RESULT:     //查询结果
    {
      ...
      if(wDataSize!=sizeof(DBO_GR_MatchQueryResult))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_BUY_RESULT:     //购买结果
    {
      ...
      if(wDataSize>sizeof(DBO_GR_MatchBuyResult))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_REVIVE_INFO:      //复活信息
    {
      ...
      if(wDataSize!=sizeof(DBO_GR_MatchReviveInfo))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_REVIVE_RESULT:    //复活结果
    {
      ...
      if(wDataSize>sizeof(DBO_GR_MatchReviveResult))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_SIGNUP_RESULT:    //报名结果
    {
      ...
      if(wDataSize>sizeof(DBO_GR_MatchSingupResult))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_UNSIGNUP_RESULT:    //退赛结果
    {
      ...
      //系统取消
      if(pMatchSignupResult->dwReason==UNSIGNUP_REASON_SYSTEM)
      {
        return true;
      }
    ...
    case DBO_GR_MATCH_RANK_LIST:      //比赛排行
    {
      ...
      if(wDataSize>sizeof(DBO_GR_MatchRankList))
      {
        return false;
      }
      ...
      for(WORD wIndex=0; wIndex<pMatchRankList->wUserCount; wIndex++)
      {
        ...
        if(pIRewardUserItem!=NULL && pIRewardUserItem->IsAndroidUser()==true)
        {
          continue;
        }
      ...
      while(true)
      {
        ...
        if(pIRewardUserItem==NULL)
        {
          break;
        }
        if(pIRewardUserItem->IsAndroidUser()==true)
        {
          continue;
        }
    ...
    case DBO_GR_MATCH_REWARD_RESULT:    //奖励结果
    {
      ...
      if(wDataSize!=sizeof(DBO_GR_MatchRewardResult))
      {
        return false;
      }
    ...
    case DBO_GR_MATCH_CANCEL_RESULT:    //取消结果
    {
      ...
      if(wDataSize!=sizeof(DBO_GR_MatchCancelResult))
      {
        return false;
      }
...
bool CLockTimeMatch::OnEventSocketMatch(WORD wSubCmdID, VOID * pData, WORD wDataSize, IServerUserItem * pIServerUserItem, DWORD dwSocketID)
{
  switch(wSubCmdID)
  {
    case SUB_GR_MATCH_FEE:  //比赛费用
    {
      ...
      if((cbSignupMode&m_pMatchOption->cbSignupMode)==0)
      {
        return false;
      }
    ...
    case SUB_GR_MATCH_BUY_SAFECARD: //购买保险
    {
      ...
      if(wDataSize!=sizeof(CMD_GR_Match_BuySafeCard))
      {
        return false;
      }
...
bool CLockTimeMatch::OnEventUserLogonFinish(IServerUserItem * pIServerUserItem)
{
  ...
  if(m_MatchStatus>=MatchStatus_Free && pIServerUserItem->IsAndroidUser()==false)
  {
    ...
    if(m_MatchStatus==MatchStatus_End)
    {
      cbMatchStatus=MS_MATCHEND;
    }
    if(m_MatchStatus==MatchStatus_WaitEnd)
    {
      cbMatchStatus=MS_WAITEND;
    }
    if(m_MatchStatus==MatchStatus_Playing)
    {
      cbMatchStatus=MS_MATCHING;
    }
    if(m_MatchStatus==MatchStatus_WaitPlay)
    {
      cbMatchStatus=MS_WAITPLAY;
    }
    ...
    {
      //加入分组
      if(cbUserMatchStatus==MUS_SIGNUPED)
      {
        InsertDistribute(pIServerUserItem);
      }
...
bool CLockTimeMatch::OnEventUserJoinMatch(IServerUserItem * pIServerUserItem, BYTE cbReason,DWORD dwSocketID)
{
  ...
  //加入分组
  if(InsertDistribute(pIServerUserItem)==false)
  {
    return false;
  }
...
bool CLockTimeMatch::PerformDistribute()
{
  ...
  //人数校验
  if(m_DistributeManage.GetCount()<m_pMatchOption->wMinDistributeUser)
  {
    return false;
  }
...
bool CLockTimeMatch::InsertDistribute(IServerUserItem * pIServerUserItem)
{
  ...
  //比赛状态
  if(m_MatchStatus!=MatchStatus_WaitPlay && m_MatchStatus!=MatchStatus_Playing)
  {
    return false;
  }
  ...
  //参赛校验
  if(VerifyUserEnterMatch(pIServerUserItem)==false)
  {
    return false;
  }
