...
CTCPSocketServiceThread::CTCPSocketServiceThread()
{
  ...
  m_nTCPSocketStatus=SOCKET_STATUS_IDLE;
...
DWORD CTCPSocketServiceThread::PerformConnect(DWORD dwServerIP, WORD wPort)
{
  ...
  ASSERT(m_nTCPSocketStatus==SOCKET_STATUS_IDLE);
  ...
  try
  {
    ...
    if(m_nTCPSocketStatus!=SOCKET_STATUS_IDLE)
    {
      throw CONNECT_EXCEPTION;
    }
    if((dwServerIP==INADDR_NONE)||(dwServerIP==0))
    {
      throw CONNECT_EXCEPTION;
    }
    ...
    //设置变量
    m_nTCPSocketStatus=SOCKET_STATUS_WAIT;
...
DWORD CTCPSocketServiceThread::PerformSendData(WORD wMainCmdID, WORD wSubCmdID)
{
  ...
  if(m_nTCPSocketStatus!=SOCKET_STATUS_CONNECT)
  {
    return false;
  }
  ...
  return SendBuffer(cbDataBuffer, sizeof(TCP_Head));
}
...
DWORD CTCPSocketServiceThread::PerformSendData(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  ...
  //发送数据
  //return SendBuffer(cbDataBuffer,wSendSize);
  return SendBuffer(cbDataBuffer, sizeof(TCP_Head) + wDataSize);
}
...
//映射数据
BOOL CTCPSocketServiceThread::MappedBuffer(BYTE pcbDataBuffer[], WORD wDataSize)
{
  //变量定义
...
//逆映射数据
BOOL CTCPSocketServiceThread::UnMappedBuffer(BYTE pcbDataBuffer[], WORD wDataSize)
{
  //变量定义
  BYTE* pcbBuffer = pcbDataBuffer;
  TCP_Info* pInfo = (TCP_Info*)pcbDataBuffer;
  ...
  try
  {
    ...
    while(m_wRecvSize>=sizeof(TCP_Head))
    {
      ...
      else
      {
        //处理数据
        CTCPSocketService * pTCPSocketStatusService=CONTAINING_RECORD(this,CTCPSocketService,m_TCPSocketServiceThread);
        if(pTCPSocketStatusService->OnSocketRead(Command,pDataBuffer,wDataSize)==false)
        {
...
LRESULT CTCPSocketServiceThread::OnSocketNotifyConnect(WPARAM wParam, LPARAM lParam)
{
  ...
  //设置状态
  m_nTCPSocketStatus=SOCKET_STATUS_CONNECT;
