...
VOID CDataBaseAide::AddParameter(LPCTSTR pszItem, SYSTEMTIME & SystemTime, ParameterDirectionEnum ParameterDirection)
{
  ...
  WORD wMilliSeconds = SystemTime.wMilliseconds;
  SystemTime.wMilliseconds = 0;

  double dWithoutms;
  SystemTimeToVariantTime(&SystemTime, &dWithoutms) ;
...
#define ONETHOUSANDMILLISECONDS  .0000115740740740
  ...
  m_pIDataBase->AddParameter(pszItem,adDouble,ParameterDirection,sizeof(DOUBLE),CDBVarValue((DOUBLE)dVariantTime));
  return;
}
