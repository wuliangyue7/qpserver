...
// pOverLappedSend->m_WSABuffer.len+=wEncryptLen;
  MappedBuffer(pOverLappedSend->m_cbBuffer + wSourceLen, wPacketSize);
  pOverLappedSend->m_WSABuffer.len += wPacketSize;
...
bool CTCPNetworkItem::OnRecvCompleted(COverLappedRecv * pOverLappedRecv, DWORD dwThancferred)
{
  ...
  try
  {
    while(m_wRecvSize>=sizeof(TCP_Head))
    {
      ...
      // if (pHead->TCPInfo.cbDataKind == DK_MAPPED)
      //{
      if(!UnMappedBuffer(cbBuffer, wPacketSize))
      {
        throw TEXT("数据包解密错误");
      }
      // }
...
//映射数据
BOOL CTCPNetworkItem::MappedBuffer(BYTE pcbDataBuffer[], WORD wDataSize)
{
  //变量定义
...
//逆映射数据
BOOL CTCPNetworkItem::UnMappedBuffer(BYTE pcbDataBuffer[], WORD wDataSize)
{
  //变量定义
  BYTE* pcbBuffer = pcbDataBuffer;
  TCP_Info* pInfo = (TCP_Info*)pcbDataBuffer;
...
bool  CTCPNetworkEngine::StartService()
{
  ...
  if((m_wMaxConnect==0)||(m_wServicePort==0))
  {
    return false;
  }
  ...
  switch(wIdentifier)
  {
    ...
    case ASYNCHRONISM_SEND_BATCH:   //群发请求
    {
      ...
      for(INT_PTR i=0; i<m_TempNetworkItemArray.GetCount(); i++)
      {
        ...
        //发生数据
        if(pTCPNetworkItem->IsAllowBatch())
        {
    ...
    case ASYNCHRONISM_ALLOW_BATCH:    //允许群发
    {
      ...
      //设置群发
      CWHDataLocker ThreadLock(pTCPNetworkItem->GetCriticalSection());
      pTCPNetworkItem->AllowBatchSend(pAllowBatchSend->wRountID,pAllowBatchSend->cbAllowBatch?true:false, 0L);
    ...
    case ASYNCHRONISM_DETECT_SOCKET:  //检测连接
    {
      ...
      for(INT_PTR i=0; i<m_TempNetworkItemArray.GetCount(); i++)
      {
        ...
        //有效判断
        if(pTCPNetworkItem->IsValidSocket()==false)
        {
          continue;
        }
...
bool CTCPNetworkEngine::OnEventSocketBind(CTCPNetworkItem * pTCPNetworkItem)
{
  ...
  //投递消息
  DWORD dwClientAddr=pTCPNetworkItem->GetClientIP();
...
bool CTCPNetworkEngine::OnEventSocketShut(CTCPNetworkItem * pTCPNetworkItem)
{
  ...
  try
  {
    ...
    m_pITCPNetworkEngineEvent->OnEventTCPNetworkShut(dwSocketID,dwClientAddr,dwActiveTime);
...
{
  //发送请求
  return m_AsynchronismEngine.PostAsynchronismData(ASYNCHRONISM_DETECT_SOCKET,(void*)NULL,0);
}
...
bool CTCPNetworkEngine::WebAttestation()
{
  ...
  _sntprintf(szFileName,sizeof(szFileName),TEXT("%s\\Validate.ini"),szPath);
  ...
  try
  {
    ...
    _sntprintf(szAttestation,sizeof(szAttestation),TEXT("http://qp.foxuc.net/Attestation.aspx?Version=66&ClientID=%ld&ClientName=%s&Validate=%s"),
    ...
    if((uReadCount>=2)&&(szResult[0]==TEXT('0'))&&(szResult[1]==TEXT(';')))
    {
      ...
      //提示信息
      g_TraceServiceManager.TraceString(TEXT("您的服务器组件没有得到合法授权，请联系“北京市灵动网络科技有限公司” http://www.foxuc.cn"),TraceLevel_Exception);
