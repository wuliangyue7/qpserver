...
bool CMatchInfoManager::InsertGameMatchOption(tagCreateMatchInfo * pCreateMatchInfo,tagGameMatchOption & GameMatchResult)
{
  ...
  try
  {
    ...
    for(INT i=0; i<CountArray(pCreateMatchInfo->cbMatchRule); i++)
    {
      _stprintf(&szMatchRule[i*2],TEXT("%02X"),pCreateMatchInfo->cbMatchRule[i]);
    }
...
bool CMatchInfoManager::ReadGameMatchOption(CDataBaseAide & GameMatchDBAide, tagGameMatchOption & GameMatchResult)
{
  ...
  if(szMatchRule[0]!=0)
  {
    ...
    for(INT i=0; i<nMatchRuleSize; i++)
    {
      ...
      {
        GameMatchResult.cbMatchRule[i]+=(cbChar2-TEXT('0'));
      }
      if((cbChar2>=TEXT('A'))&&(cbChar2<=TEXT('F')))
      {
        GameMatchResult.cbMatchRule[i]+=(cbChar2-TEXT('A')+0x0A);
      }
      if((cbChar1>=TEXT('0'))&&(cbChar1<=TEXT('9')))
      {
        GameMatchResult.cbMatchRule[i]+=(cbChar1-TEXT('0'))*0x10;
      }
      if((cbChar1>=TEXT('A'))&&(cbChar1<=TEXT('F')))
      {
        GameMatchResult.cbMatchRule[i]+=(cbChar1-TEXT('A')+0x0A)*0x10;
      }
    }
  }
