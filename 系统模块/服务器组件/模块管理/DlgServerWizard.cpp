...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
  ...
  {
    SetDlgItemText(IDC_SERVER_NAME,m_pGameServiceOption->szServerName);
  }
  ...
  //房间配置
  if(m_pGameServiceOption->lCellScore!=0)
  {
    SetDlgItemInt(IDC_CELL_SCORE,(LONG)m_pGameServiceOption->lCellScore);
  }
  if(m_pGameServiceOption->wMaxPlayer!=0)
  {
    SetDlgItemInt(IDC_MAX_PLAYER,(LONG)m_pGameServiceOption->wMaxPlayer);
  }
  if(m_pGameServiceOption->lRestrictScore!=0)
  {
    SetDlgItemInt(IDC_RESTRICT_SCORE,(LONG)m_pGameServiceOption->lRestrictScore);
  }
  ...
  if(m_pGameServiceOption->wNodeID!=0)
  {
    SetDlgItemInt(IDC_NODE_ID,m_pGameServiceOption->wNodeID);
  }
  if(m_pGameServiceOption->wSortID!=0)
  {
    SetDlgItemInt(IDC_SORT_ID,m_pGameServiceOption->wSortID);
  }
  if((wGameKindID!=0)&&(wGameKindID!=wAttribKindID))
  {
    SetDlgItemInt(IDC_KIND_ID,m_pGameServiceOption->wKindID);
  }
  ...
  if(pServerType->GetCurSel()==LB_ERR)
  {
    ...
    //类型定义
    if((wServerType==0)&&(bGoldDataBase==true))
    {
      wServerType=GAME_GENRE_GOLD;
    }
    if((wServerType==0)&&(bGoldDataBase==false)&&(bDefaultDataBase==true))
    {
      wServerType=GAME_GENRE_SCORE;
    }
...
VOID CDlgServerOptionItem1::InitCtrlWindow()
{
  ...
  {
    if((m_pGameServiceAttrib->wSupporType&wServerType[i])==0)
    {
      continue;
    }
...
VOID CDlgServerOptionItem1::UpdateDataBaseName()
{
  ...
  switch((WORD)pServerType->GetItemData(pServerType->GetCurSel()))
  {
    ...
    default:
    {
      ...
      //名字判断
      if(lstrcmpi(szDataBase,szTreasureDB)==0)
      {
        SetDlgItemText(IDC_DATABASE_NAME,TEXT(""));
      }
      if(lstrcmpi(szDataBase,m_pGameServiceAttrib->szDataBaseName)==0)
      {
        SetDlgItemText(IDC_DATABASE_NAME,TEXT(""));
      }
...
BOOL CDlgServerOptionItem2::OnInitDialog()
{
  ...
  if(m_pGameServiceAttrib->wChairCount >= MAX_CHAIR)
  {
    CServerRule::SetForfendGameRule(m_pGameServiceOption->dwServerRule, true);
  }
  if(m_pGameServiceAttrib->wChairCount >= MAX_CHAIR)
  {
    ((CButton *)GetDlgItem(IDC_FORFEND_GAME_RULE))->EnableWindow(FALSE);
  }
  ...
  if(m_pGameServiceAttrib->wChairCount >= MAX_CHAIR)
  {
    CServerRule::SetForfendLockTable(m_pGameServiceOption->dwServerRule, true);
  }
  if(m_pGameServiceAttrib->wChairCount >= MAX_CHAIR)
  {
    ((CButton *)GetDlgItem(IDC_FORFEND_LOCK_TABLE))->EnableWindow(FALSE);
  }
...
bool CDlgServerWizardItem1::SaveInputInfo()
{
  ...
  if(pGameModuleInfo->dwNativeVersion!=pGameModuleInfo->dwServerVersion)
  {
    ...
    //提示消息
    if(AfxMessageBox(szString,MB_ICONWARNING|MB_YESNO|MB_DEFBUTTON2)!=IDYES)
    {
      return false;
    }
  }
...
bool CDlgServerWizardItem1::LoadDBModuleItem()
{
  ...
  if(m_ModuleInfoManager.LoadGameModuleInfo(m_ModuleInfoBuffer)==true)
  {
    ...
    while(Position!=NULL)
    {
      ...
      if(pGameModuleInfo!=NULL)
      {
        m_ModuleListControl.InsertModuleInfo(pGameModuleInfo);
      }
    }
  }
...
bool CDlgServerWizardItem2::ActiveOptionItem(WORD wIndex)
{
  ...
  if(m_wActiveIndex<m_wItemCount)
  {
    pItemOption=m_pOptionItem[m_wActiveIndex];
  }
  ...
  if(pItemOption!=NULL)
  {
    pItemOption->ShowWindow(SW_HIDE);
  }

  //设置选择
  if(m_TabCtrl.GetCurSel()!=wIndex)
  {
    m_TabCtrl.SetCurSel(wIndex);
  }
...
VOID CDlgServerWizardItem2::OnTcnSelchangeTabCtrl(NMHDR * pNMHDR, LRESULT * pResult)
{
  ...
  {
    case ITEM_SERVER_OPTION_1:
    {
      ActiveOptionItem(0);
      break;
    }
    case ITEM_SERVER_OPTION_2:
    {
      ActiveOptionItem(1);
      break;
    }
    case ITEM_SERVER_OPTION_3:
    {
      ActiveOptionItem(2);
      break;
    }
    case ITEM_SERVER_OPTION_CUSTOM:
    {
      ActiveOptionItem(3);
      break;
    }
...
bool CDlgServerWizard::ActiveWizardItem(WORD wIndex)
{
  ...
  if(m_wActiveIndex<CountArray(m_pWizardItem))
  {
    ...
    //保存数据
    if((wIndex>m_wActiveIndex)&&(m_pWizardItem[m_wActiveIndex]->SaveItemData()==false))
    {
      return false;
    }
  }
  ...
  if(pItemWizard!=NULL)
  {
    pItemWizard->ShowWindow(SW_HIDE);
  }
...
VOID CDlgServerWizard::OnBnClickedFinish()
{
  ...
  if(m_pWizardItem[m_wActiveIndex]->SaveItemData()==false)
  {
    return;
  }
  ...
  {
    if(ServerInfoManager.InsertGameServer(&GameServerCreate,GameServerResult)==false)
    {
      return;
    }
  ...
  {
    //修改房间
    if(ServerInfoManager.ModifyGameServer(&GameServerCreate,GameServerResult)==false)
    {
      return;
    }
