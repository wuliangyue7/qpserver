...
bool CDlgServerItem::LoadDBServerItem()
{
  ...
  if(ServerInfoManager.LoadGameServerInfo(szMachineID,m_ServerInfoBuffer)==true)
  {
    ...
    while(Position!=NULL)
    {
      ...
      {
        m_ServerListControl.InsertServerInfo(pGameServerInfo);
      }
    }
...
VOID CDlgServerItem::OnBnClickedDeleteServer()
{
  ...
  if(pGameServerInfo!=NULL)
  {
    ...
    //提示消息
    if(AfxMessageBox(szString,MB_ICONQUESTION|MB_YESNO)!=IDYES)
    {
      return;
    }
    ...
    //删除房间
    CServerInfoManager ServerInfoManager;
    if(ServerInfoManager.DeleteGameServer(pGameServerInfo->wServerID)==false)
    {
      return;
    }
