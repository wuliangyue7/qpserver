...
bool CDlgMatchItem::ShowMatchItem(const CRect & rcRect, CWnd * pParentWnd)
{
  ...
  if(m_hWnd==NULL)
  {
    CreateMatchItem(rcRect,pParentWnd);
  }
  ...
  if(pComboBox!=NULL)
  {
    ...
    if(m_pMatchInfoBuffer!=NULL)
    {
      ...
      for(INT_PTR i=0; i<m_pMatchInfoBuffer->m_GameMatchOptionArray.GetCount(); i++)
      {
        ...
        if(pGameMatchOption->dwMatchID==m_pGameMatchOption->dwMatchID)
        {
          continue;
        }
...
bool CDlgMatchSignup1::SaveInputInfo()
{
  ...
  if(IsDlgButtonChecked(IDC_CHECK_SIGNUP_FEE))
  {
    ...
    //缴费区域
    if(IsDlgButtonChecked(IDC_RADIO_SERVER))
    {
      m_pGameMatchOption->cbDeductArea=DEDUCT_AREA_SERVER;
    }
    if(IsDlgButtonChecked(IDC_RADIO_WEBSITE))
    {
      m_pGameMatchOption->cbDeductArea=DEDUCT_AREA_WEBSITE;
    }

    //费用类型
    if(IsDlgButtonChecked(IDC_RADIO_GOLD))
    {
      m_pGameMatchOption->cbFeeType=FEE_TYPE_GOLD;
    }
    if(IsDlgButtonChecked(IDC_RADIO_MEDAL))
    {
      m_pGameMatchOption->cbFeeType=FEE_TYPE_MEDAL;
    }
  }
  else
  {
  ...
  if(IsDlgButtonChecked(IDC_CHECK_MATCH_USER))
  {
    ...
    if(pComboBox!=NULL && pComboBox->GetCurSel()!=LB_ERR)
    {
      ...
      {
        m_pGameMatchOption->dwFromMatchID=pGameMatchOption->dwMatchID;
      }
    }
...
{
  //排名方式
  if(IsDlgButtonChecked(IDC_RADIO_TOTAL_GRADES))
  {
    m_pGameMatchOption->cbRankingMode=RANKING_MODE_TOTAL_GRADES;
  }
  if(IsDlgButtonChecked(IDC_RADIO_SPECIALLY_GRADES))
  {
    m_pGameMatchOption->cbRankingMode=RANKING_MODE_SPECIAL_GRADES;
  }
  ...
  if(m_pGameMatchOption->cbRankingMode==RANKING_MODE_SPECIAL_GRADES)
  {
    ...
    //筛选模式
    if(IsDlgButtonChecked(IDC_RADIO_BEST_GRADES))
    {
      m_pGameMatchOption->cbFilterGradesMode=FILTER_GRADES_MODE_BEST;
    }
    if(IsDlgButtonChecked(IDC_RADIO_AVERAGE_GRADES))
    {
      m_pGameMatchOption->cbFilterGradesMode=FILTER_GRADES_MODE_AVERAGE;
    }
    if(IsDlgButtonChecked(IDC_RADIO_AVERAGEEX_GRADES))
    {
      m_pGameMatchOption->cbFilterGradesMode=FILTER_GRADES_MODE_AVERAGEEX;
    }
  }
...
VOID CDlgMatchReward::OnBnClickedAddReward()
{
  ...
  //获取配置
  if(GetMatchRewardOption(MatchRewardInfo,wStartRankID,wEndRankID)==false)
  {
    return;
  }
...
VOID CDlgMatchReward::OnLvnItemchangedListReward(NMHDR *pNMHDR, LRESULT *pResult)
{
  ...
  if(pNMLV->iItem!=LB_ERR)
  {
    ...
    if(pMatchRewardInfo!=NULL)
    {
      SetMatchRewardOption(pMatchRewardInfo);
    }
  }
...
bool CDlgMatchWizardStep1::SaveInputInfo()
{
  ...
  for(int i=0; i<CountArray(m_pDlgMatchItem); i++)
  {
    if(m_pDlgMatchItem[i]->SaveInputInfo() == false)
    {
      return false;
    }
  }
...
VOID CDlgMatchWizardStep1::InitBaseOption()
{
  ...
  if(pComboBox!=NULL)
  {
    ...
    if(m_pGameMatchOption->dwMatchID!=0)
    {
...
{
  //保存输入
  if(m_pDlgMatchItem->SaveInputInfo() == false)
  {
    return false;
  }
...
{
  //隐藏窗口
  if(m_pDlgMatchItem!=NULL)
  {
    m_pDlgMatchItem->ShowWindow(SW_HIDE);
  }
...
VOID CDlgMatchWizard::OnBnClickedFinish()
{
  ...
  {
    if(MatchInfoManager.InsertGameMatchOption(&CreateMatchInfo,GameMatchResult)==false)
    {
      return;
    }
  ...
  {
    //修改房间
    if(MatchInfoManager.ModifyGameMatchOption(&CreateMatchInfo,GameMatchResult)==false)
    {
      return;
    }
