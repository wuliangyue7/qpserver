...
bool CAttemperEngineSink::OnEventControl(WORD wIdentifier, VOID * pData, WORD wDataSize)
{
  switch(wIdentifier)
  {
    case CT_LOAD_DB_GAME_LIST:    //加载列表
    {
      ...
      //道具配置
      m_pIDataBaseEngine->PostDataBaseRequest(DBR_GP_LOAD_GAME_PROPERTY,0,NULL,0);
...
bool CAttemperEngineSink::OnEventTCPNetworkBind(DWORD dwClientAddr, DWORD dwSocketID)
{
  ...
  if(LOWORD(dwSocketID)>=m_pInitParameter->m_wMaxConnect)
  {
    return false;
  }
      ...
      switch(Command.wSubCmdID)
      {
        ...
        case SUB_FORWARD_CLIENTIP:
        {
          ...
          if(pForwardClientIP->dwClientIP == pBindParameter->dwClientAddr)
          {
            ...
            //CTraceService::TraceString(szClientAddr,TraceLevel_Normal);
            //CTraceService::TraceString(szClientAddr2,TraceLevel_Normal);
...
bool CAttemperEngineSink::OnTCPSocketMainRemoteService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
  switch(wSubCmdID)
  {
    case SUB_CS_S_SEARCH_CORRESPOND:  //协调查找
    {
      ...
      if(wDataSize!=(sizeof(CMD_CS_S_SearchCorrespond)-sizeof(pSearchCorrespond->UserRemoteInfo)+pSearchCorrespond->wUserCount*sizeof(pSearchCorrespond->UserRemoteInfo[0])))
      {
        return false;
      }
      ...
      if((m_pBindParameter+LOWORD(pSearchCorrespond->dwSocketID))->dwSocketID!=pSearchCorrespond->dwSocketID)
      {
        return true;
      }
      ...
      for(WORD i=0; i<pSearchCorrespond->wUserCount; i++)
      {
        ...
        if(SearchCorrespond.wUserCount>=CountArray(SearchCorrespond.UserRemoteInfo))
        {
          break;
        }
...
bool CAttemperEngineSink::OnTCPNetworkMainPCUserService(WORD wSubCmdID, VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  switch(wSubCmdID)
  {
    ...
    case SUB_GP_MODIFY_UNDER_WRITE:     //修改签名
    {
      ...
      if(wDataSize!=(sizeof(CMD_GP_ModifyUnderWrite)-sizeof(pModifyUnderWrite->szUnderWrite)+CountStringBuffer(pModifyUnderWrite->szUnderWrite)))
      {
        return false;
      }
    ...
    case SUB_GP_EXCHANGE_SCORE_BYINGOT:   //兑换游戏币
    {
      ...
      if(wDataSize!=sizeof(CMD_GP_ExchangeScoreByIngot))
      {
        return false;
      }
    ...
    case SUB_GP_EXCHANGE_SCORE_BYBEANS:   //兑换游戏币
    {
      ...
      if(wDataSize!=sizeof(CMD_GP_ExchangeScoreByBeans))
      {
        return false;
      }
  ...
  switch(wSubCmdID)
  {
    case SUB_GP_QUERY_PROPERTY:     //加载道具
    {
      ...
      for(DWORD i=0; i<m_DataConfigProperty.cbPropertyCount; i++)
      {
        ...
        {
          m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_PROPERTY,SUB_GP_QUERY_PROPERTY_RESULT,cbDataBuffer,wSendSize);
          wSendSize=0;
        }
    ...
    {
      //效验参数
      ASSERT(wDataSize==sizeof(CMD_GP_C_PropertyBuy));
      if(wDataSize!=sizeof(CMD_GP_C_PropertyBuy))
      {
        return false;
      }

      //变量定义
      CMD_GP_C_PropertyBuy * pPropertyBuy=(CMD_GP_C_PropertyBuy *)pData;
...
bool CAttemperEngineSink::OnTCPNetworkSubPCLogonGameID(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  lstrcpyn(LogonGameID.szSessionKey,pLogonGameID->szSessionKey,CountArray(LogonGameID.szSessionKey));
...
bool CAttemperEngineSink::OnTCPNetworkSubPCLogonAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  lstrcpyn(LogonAccounts.szSessionKey,pLogonAccounts->szSessionKey,CountArray(LogonAccounts.szSessionKey));
  //投递请求
...
bool CAttemperEngineSink::OnTCPNetworkSubPCRegisterAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  if(pRegisterAccounts->nCode != 9078+pRegisterAccounts->wFaceID*2)
  {
    //变量定义
    ...
    //构造数据
    LogonFailure.lResultCode=5;
    lstrcpyn(LogonFailure.szDescribeString,TEXT("请通过正常流程注册游戏帐号!"),CountArray(LogonFailure.szDescribeString));
    ...
    CString str;
    str.Format(TEXT("Code %d,%s"),pRegisterAccounts->nCode,pRegisterAccounts->szMachineID);
  ...
  _sntprintf(szResPassword,sizeof(szResPassword),TEXT("%s68y56p99@QTc.DV*F"),pRegisterAccounts->szAccounts);
  CWHEncrypt::MD5Encrypt(szResPassword,szNewDataEncrypt);
  if(_tcscmp(szNewDataEncrypt,pRegisterAccounts->szDataEncrypt)!=0)
  {
    CTraceService::TraceString(TEXT("register fail"),TraceLevel_Exception);
    return false;
  }
  //变量定义
  ...
  RegisterAccounts.wAgentID = pRegisterAccounts->wAgentID;
  ...
  lstrcpyn(RegisterAccounts.szSessionKey,pRegisterAccounts->szSessionKey,CountArray(RegisterAccounts.szSessionKey));
  //投递请求
...
bool CAttemperEngineSink::OnTCPNetworkSubMBLogonAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  {
    CString mstr;
    mstr.Format(TEXT("OnTCPNetworkSubMBLogonAccounts %d, %d"),wDataSize,sizeof(CMD_MB_LogonAccounts));
  ...
  {
    CTraceService::TraceString(TEXT("OnTCPNetworkSubMBLogonAccounts CheckPlazaVersion"),TraceLevel_Exception);
    return true;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMBRegisterAccounts(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  {
    CString mstr;
    mstr.Format(TEXT("OnTCPNetworkSubMBRegisterAccounts %d, %d"),wDataSize,sizeof(CMD_MB_RegisterAccounts));
    CTraceService::TraceString(mstr,TraceLevel_Exception);
    //return false;
  }
...
bool CAttemperEngineSink::OnTCPNetworkSubMBLogonOtherPlatform(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  {
    CString mstr;
    mstr.Format(TEXT("OnTCPNetworkSubMBLogonOtherPlatform %d, %d"),wDataSize,sizeof(CMD_MB_LogonOtherPlatform));
    CTraceService::TraceString(mstr,TraceLevel_Exception);
    //return false;
  }
  ...
  lstrcpyn(LogonOtherPlatform.szSessionKey,pLogonOtherPlatform->szSessionKey,CountArray(LogonOtherPlatform.szSessionKey));
  //投递请求
...
bool CAttemperEngineSink::OnTCPNetworkSubMBLogonVisitor(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
  ...
  {
    CString mstr;
    mstr.Format(TEXT("OnTCPNetworkSubMBLogonVisitor %d, %d"),wDataSize,sizeof(CMD_MB_LogonVisitor));
    CTraceService::TraceString(mstr,TraceLevel_Exception);
    //return false;
  }
  ...
  lstrcpyn(LogonVisitor.szSessionKey,pLogonVisitor->szSessionKey,CountArray(LogonVisitor.szSessionKey));
  //投递请求
...
bool CAttemperEngineSink::OnDBPCLogonSuccess(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  //道具配置
  SendPropertyConfig(dwContextID);
...
bool CAttemperEngineSink::OnDBGamePropertyInfo(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  for(WORD i=0; i<wItemCount; i++)
  {
    ...
    pPropertyInfo->cbSuportMobile = (pGamePropertyItem+i)->cbSuportMobile;
...
bool CAttemperEngineSink::OnDBGamePropertyListResult(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  //设置道具
  if(pGamePropertyInfo->cbSuccess == false)
  {
    m_DataConfigProperty.cbPropertyCount = 0;
    ZeroMemory(&m_DataConfigProperty.PropertyInfo,sizeof(m_DataConfigProperty.PropertyInfo));
...
bool CAttemperEngineSink::OnDBGamePropertyBuy(DWORD dwContextID, VOID * pData, WORD wDataSize)
{
  ...
  //构造结构
  CMD_GP_S_PropertySuccess PropertySuccess;
  ...
  m_pITCPNetworkEngine->SendData(dwContextID,MDM_GP_PROPERTY,SUB_GP_PROPERTY_BUY_RESULT,&PropertySuccess,sizeof(CMD_GP_S_PropertySuccess));
...
//道具配置
VOID CAttemperEngineSink::SendPropertyConfig(DWORD dwSocketID)
{
  //网络数据
  WORD wSendSize=0;
  BYTE cbDataBuffer[SOCKET_TCP_PACKET];
  ...
  for(DWORD i=0; i<m_DataConfigProperty.cbPropertyCount; i++)
  {
    ...
    if((wSendSize+sizeof(tagPropertyInfo))>sizeof(cbDataBuffer))
    {
      m_pITCPNetworkEngine->SendData(dwSocketID,MDM_GP_LOGON,SUB_GP_PROPERTY_CONFIG,cbDataBuffer,wSendSize);
      wSendSize=0;
    }
...
{
  //CString mstr;
  //mstr.Format(TEXT("SendMobileServerInfo %d"),wModuleID);
  //CTraceService::TraceString(mstr,TraceLevel_Exception);
  //网络数据
  WORD wSendSize=0;
  BYTE cbDataBuffer[SOCKET_TCP_PACKET];
  ...
  for(DWORD i=0; i<m_ServerListManager.GetGameKindCount(); i++)
  {
    ...
    if(pGameKindItem->m_GameKind.wKindID==wModuleID)
    {
      CopyMemory(cbDataBuffer+wSendSize,&pGameKindItem->m_GameKind,sizeof(tagGameKind));
      wSendSize+=sizeof(tagGameKind);
      //CTraceService::TraceString(TEXT("if (pGameKindItem->m_GameKind.wKindID==wModuleID)"),TraceLevel_Exception);
    }
  }
...
VOID CAttemperEngineSink::SendMobileServerInfo(DWORD dwSocketID, WORD wModuleID)
{
  ...
  for(DWORD i=0; i<m_ServerListManager.GetGameServerCount(); i++)
  {
    ...
    if(pGameServerItem->m_GameServer.wKindID==wModuleID)
    {
      CopyMemory(cbDataBuffer+wSendSize,&pGameServerItem->m_GameServer,sizeof(tagGameServer));
      wSendSize+=sizeof(tagGameServer);
      //CTraceService::TraceString(TEXT("if (pGameServerItem->m_GameServer.wKindID==wModuleID)"),TraceLevel_Exception);
    }
  }
...
VOID CAttemperEngineSink::RandVerifyCode(LPTSTR pszVerifyCode, UINT nMaxCount)
{
  ...
  if(pszVerifyCode==NULL || nMaxCount<LEN_VERIFY_CODE)
  {
    return;
  }
...
{
  //长度验证
  if(pszValidateCode==NULL || pszValidateCodeBak==NULL)
  {
    return false;
  }
  ...
  if(CompareString(LOCALE_SYSTEM_DEFAULT,NORM_IGNORECASE,pszValidateCode,nTargetLen,pszValidateCodeBak,nSourceLen)!=CSTR_EQUAL)
  {
    return false;
  }
