...
#define ANDROID_UNLOAD_TIME     3*60                //不加载时间
#define ANDROID_UNIN_TIME     2*60                //不进入时间
...
CAndroidUserManager::CAndroidUserManager()
{
  ...
  m_dwTimeValue=1;
  //起初基数
  m_nNeedAndroidCount[0] = 70;
  m_nNeedAndroidCount[1] = 60;
  m_nNeedAndroidCount[2] = 50;
  m_nNeedAndroidCount[3] = 50;
  m_nNeedAndroidCount[4] = 70;
  m_nNeedAndroidCount[5] = 80;
  m_nNeedAndroidCount[6] = 85;
  m_nNeedAndroidCount[7] = 85;
  m_nNeedAndroidCount[8] = 70;
  m_nNeedAndroidCount[9] = 80;
  m_nNeedAndroidCount[10] = 70;
  return;
}
...
//插入机器
bool CAndroidUserManager::InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[],WORD wAndroidCount,DWORD dwBatchID,DWORD dwUserCount)
{
  //if (wAndroidCount==0) return true;
  //重处理机器人
  if(m_pGameServiceOption->wServerType==GAME_GENRE_GOLD)
  {
    ControlAndroidCount(AndroidAccountsInfo,wAndroidCount,dwUserCount);
    int nActiveCount =m_AndroidUserItemMap.GetCount();
    if(nActiveCount<40)
    {
      CString mstr;
      mstr.Format(TEXT("nActiveCount:%d,wAndroidCount:%d"),nActiveCount,wAndroidCount);
...
bool CAndroidUserManager::DeleteAndroidUserItem(DWORD dwAndroidID,bool bStockRetrieve)
{
  ...
  if((pAndroidUserItem==NULL)||(pAndroidUserItem->m_wRoundID!=HIWORD(dwAndroidID)))
  {
    return false;
  }
...
IAndroidUserItem * CAndroidUserManager::CreateAndroidUserItem(tagAndroidItemConfig & AndroidItemConfig)
{
  ...
  pAndroidUserItem->SetKindID(m_pGameServiceOption->wKindID);
...
bool CAndroidUserManager::OnEventTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter)
{
  ...
  if(dwTimerID>=IDI_ANDROID_IN && dwTimerID<IDI_ANDROID_IN+MAX_BATCH)
  {
    ...
    if(bLogonSuccessed==true)
    {
      ...
      int nAddUserCount =m_AndroidUserItemMap.GetCount();
      if(nAddUserCount<50)
      {
        dwElapse=1+rand()%5;
      }
    ...
    else
    {
      if(pAndroidParameter->bIsValided==false)
      {
        ...
        if(wParameterIndex==GetAndroidParameterIndex(pAndroidParameter))
        {
          ...
          CString str;
          str.Format(TEXT("本批次机器人全部退出,该批次将被删除, 批次ID=%d,%d"),pAndroidParameter->AndroidParameter.dwBatchID,nActiveCount);
          CTraceService::TraceString(str,TraceLevel_Info);
      ...
      {
        if((nActiveCount-pAndroidParameter->AndroidParameter.dwAndroidCount)<36)
        {
        ...
        CString str;
        str.Format(TEXT("本批次机器人全部退出,, 批次ID=%d,%d"),pAndroidParameter->AndroidParameter.dwBatchID,nActiveCount);
        CTraceService::TraceString(str,TraceLevel_Info);
...
WORD CAndroidUserManager::GetAndroidUserInfo(tagAndroidUserInfo & AndroidUserInfo, DWORD dwServiceMode)
{
  ...
  while(Position!=NULL)
  {
    ...
    //离开判断
    if(pAndroidUserItem->m_bWaitLeave==true)
    {
      continue;
    }
...
CAndroidUserItem * CAndroidUserManager::ActiveAndroidUserItem(tagAndroidItemConfig & AndroidItemConfig)
{
  ...
  if((AndroidItemConfig.pAndroidParameter->dwSwitchMinInnings!=0L)&&(AndroidItemConfig.pAndroidParameter->dwSwitchMaxInnings!=0L))
  {
    ...
    if(pAndroidUserItem->m_AndroidService.dwSwitchTableInnings<5)
    {
      dwSwitchMinInnings = rand()%15;
      pAndroidUserItem->m_AndroidService.dwSwitchTableInnings += dwSwitchMinInnings;
    }
  }
  else
  {
    pAndroidUserItem->m_AndroidService.dwSwitchTableInnings=ADNDROID_PLAY_DRAW;
  }
...
void CAndroidUserManager::ControlAndroidCount(tagAndroidAccountsInfo AndroidAccountsInfo[], WORD &wStockCount,DWORD dwUserCount)
{
  if(wStockCount == 0 /*|| dwUserCount<10*/)
  {
    return ;
  }
  SYSTEMTIME time;
  ...
  //变量
  int nNeedUserCount = 0;
  int nNewUserCount = 0;

  WORD wMaxPlayer = m_pGameServiceOption->wTableCount*m_pGameServiceAttrib->wChairCount;

  int nRandCount=20;
  if(m_pGameServiceAttrib->wChairCount < MAX_CHAIR)
  {
    nRandCount=30;
    if(m_AndroidUserItemMap.GetCount()>wMaxPlayer-30 && wMaxPlayer>80 && m_pGameServiceAttrib->wChairCount>1)
    {
      nRandCount=wMaxPlayer-75;
    }
  }
  else
  {
    if(dwUserCount>=wMaxPlayer-30)
    {
      nRandCount=10;
    }
  }
  ...
  if(time.wHour>=1&&time.wHour<3)
  {
    nNeedUserCount = m_nNeedAndroidCount[0]+rand()%nRandCount;
  }
  else if(time.wHour>=3&&time.wHour<5)
  {
    nNeedUserCount = m_nNeedAndroidCount[1]+rand()%nRandCount;
  }
  else if(time.wHour>=5&&time.wHour<7)
  {
    nNeedUserCount = m_nNeedAndroidCount[2]+rand()%nRandCount;
  }
  else if(time.wHour>=7&&time.wHour<9)
  {
    nNeedUserCount = m_nNeedAndroidCount[3]+rand()%nRandCount;
  }
  else if(time.wHour>=9&&time.wHour<11)
  {
    nNeedUserCount = m_nNeedAndroidCount[4]+rand()%nRandCount;
  }
  else if(time.wHour>=11&&time.wHour<13)
  {
    nNeedUserCount = m_nNeedAndroidCount[5]+rand()%nRandCount;
  }
  else if(time.wHour>=13&&time.wHour<15)
  {
    nNeedUserCount = m_nNeedAndroidCount[6]+rand()%nRandCount;
  }
  else if(time.wHour>=15&&time.wHour<17)
  {
    nNeedUserCount = m_nNeedAndroidCount[7]+rand()%nRandCount;
  }
  else if(time.wHour>=17&&time.wHour<19)
  {
    nNeedUserCount = m_nNeedAndroidCount[8]+rand()%nRandCount;
  }
  else if(time.wHour>=19&&time.wHour<23)
  {
    nNeedUserCount = m_nNeedAndroidCount[9]+rand()%nRandCount;
  }
  else
  {
    nNeedUserCount = m_nNeedAndroidCount[10]+rand()%nRandCount;
  }


  if(nNeedUserCount==0)
  {
    ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
    wStockCount = 0;

    CTraceService::TraceString(TEXT("获取时段人数异常"),TraceLevel_Exception);

    return;
  }

  //机器人过多
  if(m_pGameServiceOption->wTableCount>1)
  {
    wMaxPlayer += RESERVE_USER_COUNT/2;
  }
  if(nNeedUserCount+dwUserCount>wMaxPlayer)
  {
    nNeedUserCount = wMaxPlayer-dwUserCount;
    if(nNeedUserCount<0)
    {
      ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
      wStockCount = 0;
  ...
  if(dwUserCount>m_pGameServiceOption->wTableCount*m_pGameServiceAttrib->wChairCount)
  {
    nNeedUserCount = wMaxPlayer-dwUserCount;
    if(nNeedUserCount<0)
    {
  ...
  //需要新增数
  const int nAddUserCount = (int)(nNeedUserCount - m_AndroidUserItemMap.GetCount());
  if(nAddUserCount<=0)
  {
    ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
    wStockCount = 0;
  ...
  //过滤当前已有的
  tagAndroidAccountsInfo *newAndroidAccountsInfo = new tagAndroidAccountsInfo[wStockCount];
  if(newAndroidAccountsInfo==NULL)
  {
    return;
  }
  ZeroMemory(newAndroidAccountsInfo,sizeof(newAndroidAccountsInfo));
  for(int i=0; i<wStockCount; i++)
  {
    CAndroidUserItem * pAndroidUserItem=NULL;
    m_AndroidUserItemMap.Lookup(newAndroidAccountsInfo[i].dwUserID,pAndroidUserItem);
    if(pAndroidUserItem==NULL)
    {
      newAndroidAccountsInfo[nNewUserCount++] = AndroidAccountsInfo[i];
    }
  }
  ...
  if(nNewUserCount==0)
  {
    ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
    wStockCount = 0;
    if(newAndroidAccountsInfo)
    {
      delete []newAndroidAccountsInfo;
    }
    return;
  }

  //多余的随机去掉
  if(nNewUserCount>nAddUserCount)
  {
    int nDropCount = nNewUserCount - nAddUserCount;
    for(int i=0; i<nDropCount; i++)
    {
      ...
      int nDropIndex = rand()%nNewUserCount;
      newAndroidAccountsInfo[nDropIndex] = newAndroidAccountsInfo[--nNewUserCount];
    }

    //更新数据
    ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
    CopyMemory(AndroidAccountsInfo,newAndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*nAddUserCount);

    wStockCount = nAddUserCount;
  ...
  else
  {
    //更新数据
    ZeroMemory(AndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*wStockCount);
    CopyMemory(AndroidAccountsInfo,newAndroidAccountsInfo,sizeof(tagAndroidAccountsInfo)*nNewUserCount);

    wStockCount = nNewUserCount;
  }
...
//获取当前数目
INT CAndroidUserManager::GetCurrentNeedCount()
{
  SYSTEMTIME time;
  GetLocalTime(&time);
  ...
  if(time.wHour>=1&&time.wHour<3)
  {
    nNeedUserCount = m_nNeedAndroidCount[0];
  }
  else if(time.wHour>=3&&time.wHour<5)
  {
    nNeedUserCount = m_nNeedAndroidCount[1];
  }
  else if(time.wHour>=5&&time.wHour<7)
  {
    nNeedUserCount = m_nNeedAndroidCount[2];
  }
  else if(time.wHour>=7&&time.wHour<9)
  {
    nNeedUserCount = m_nNeedAndroidCount[3];
  }
  else if(time.wHour>=9&&time.wHour<11)
  {
    nNeedUserCount = m_nNeedAndroidCount[4];
  }
  else if(time.wHour>=11&&time.wHour<13)
  {
    nNeedUserCount = m_nNeedAndroidCount[5];
  }
  else if(time.wHour>=13&&time.wHour<15)
  {
    nNeedUserCount = m_nNeedAndroidCount[6];
  }
  else if(time.wHour>=15&&time.wHour<17)
  {
    nNeedUserCount = m_nNeedAndroidCount[7];
  }
  else if(time.wHour>=17&&time.wHour<19)
  {
    nNeedUserCount = m_nNeedAndroidCount[8];
  }
  else if(time.wHour>=19&&time.wHour<23)
  {
    nNeedUserCount = m_nNeedAndroidCount[9];
  }
  else
  {
    nNeedUserCount = m_nNeedAndroidCount[10];
  }

  return nNeedUserCount;
}
