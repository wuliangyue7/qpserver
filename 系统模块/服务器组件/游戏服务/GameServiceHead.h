...
struct tagAndroidService
{
  ...
  DWORD                           dwSitDownTime;                      //坐下时间
  DWORD                           dwStandUpTime;                      //起来时间
};
...
public:
  ...
  //用户成绩
  virtual SCORE GetUserToDayGradeScore()=NULL;
  //用户成绩
  virtual SCORE GetUserTolGrade()=NULL;
  //用户成绩
  virtual SCORE GetUserTolToDayGrade()=NULL;
  //用户银行
...
public:
  ...
  //设置状态
  virtual bool SetUserStatusEx(BYTE cbUserStatusEx, WORD wTableID, WORD wChairID)=NULL;
  virtual BYTE GetUserStatusEx()=NULL;
  //用户组 I D
  virtual bool  SetTeamID(WORD wTeamID)=NULL;
  //用户排名
  virtual bool  SetSortID(WORD wSortID)=NULL;
...
public:
  ...
  //发送消息
  virtual bool SendGlobalMessage(LPCTSTR lpszMessage, WORD wType)=NULL;
...
public:
  //房间机器库银
  virtual LONGLONG GetStorageStart()=NULL;
  //库存相关
  virtual void SetStorageStart(LONGLONG lStorageStart)=NULL;
  //库存相关
  virtual LONGLONG GetStorageMax()=NULL;
  virtual void SetStorageMaxt(LONGLONG lStorageMax)=NULL;
  //库存相关
  virtual LONGLONG GetStorageCurrent()=NULL;
  virtual int GetStorageDeduct()=NULL;
  virtual void SetStorageDeduct(int nStorageDeduct)=NULL;
...
public:
  ...
  //获取库存配置数据
  virtual tagStorageConfigInfo* GetStorageConfigInfo() = NULL;
  //比赛组结束
  virtual bool TableGameEnd(WORD wTableID, WORD wTeamID) = NULL;
  virtual bool WriteUserReward(IServerUserItem * pIServerUserItem, __int64 lGold, WORD wType) = NULL;
};
...
public:
  ...
  //插入机器
  virtual bool InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[],WORD wAndroidCount,DWORD dwBatchID,DWORD dwUserCount)=NULL;
