...
CServerUserItem::CServerUserItem()
{
  ...
  m_UserInfo.wTeamID = INVALID_WORD;
...
//用户组 I D
bool CServerUserItem::SetTeamID(WORD wTeamID)
{
  m_UserInfo.wTeamID=wTeamID;

  return true;
}
//用户排名
bool CServerUserItem::SetSortID(WORD wSortID)
{
  m_UserInfo.wSortID=wSortID;

  return true;
}
//设置状态
bool CServerUserItem::SetUserStatusEx(BYTE cbUserStatusEx, WORD wTableID, WORD wChairID)
{
  //效验状态
  ASSERT(m_UserInfo.dwUserID!=0L);
  if(m_UserInfo.dwUserID==0L)
  ...
  m_UserInfo.wTableID=wTableID;
  m_UserInfo.wChairID=wChairID;
  m_UserInfo.cbUserStatusEx=cbUserStatusEx;
...
bool CServerUserItem::WriteUserScore(SCORE lScore, SCORE lGrade, SCORE lRevenue, SCORE lIngot, BYTE cbScoreType, DWORD dwPlayTimeCount,DWORD dwWinExperience)
{
  ...
  m_UserInfo.lToDayGradeScore+=lGrade;
  m_UserInfo.lTolGrade+=lGrade;
  m_UserInfo.lTolToDayGrade+=lGrade;
  ...
  m_RecordInfo.lToDayGradeScore+=lGrade;
  m_RecordInfo.lTolGrade+=lGrade;
  m_RecordInfo.lTolToDayGrade+=lGrade;
  ...
  m_VariationInfo.lToDayGradeScore+=lGrade;
  m_VariationInfo.lTolGrade+=lGrade;
  m_VariationInfo.lTolToDayGrade+=lGrade;
...
bool CServerUserManager::DeleteUserItem(IServerUserItem * pIServerUserItem)
{
  ...
  if((pIServerUserItem==NULL)||(pIServerUserItem->GetUserStatus()!=US_NULL))
  {
    return false;
  }
