...
protected:
  ...
  DWORD             m_dwTimeValue;            //平均时间
  INT                             m_nNeedAndroidCount[11];            //需要机器
...
public:
  ...
  //插入机器
  virtual bool InsertAndroidInfo(tagAndroidAccountsInfo AndroidAccountsInfo[],WORD wAndroidCount,DWORD dwBatchID,DWORD dwUserCount);
...
protected:
  ...
  //机器人时间段人数监控
  void ControlAndroidCount(tagAndroidAccountsInfo AndroidAccountsInfo[], WORD &wStockCount,DWORD dwUserCount);

  //功能函数
public:
  //获取数目
  INT  GetCurrentNeedCount();
};
